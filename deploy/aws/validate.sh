...
# Auto-detect API URL from stack if not provided
if [ -z "$API_URL" ]; then
...
if [ -z "$API_URL" ]; then
    echo "Error: API_URL not set and could not be auto-detected from stack '$STACK_NAME'"
    exit 1
fi
