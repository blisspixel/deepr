...
queue_client = QueueClient(
    account_url=f"https://{STORAGE_ACCOUNT_NAME}.queue.core.windows.net", queue_name=QUEUE_NAME, credential=credential
)
...
def response(status_code: int, body: dict) -> func.HttpResponse:
    ...
    return func.HttpResponse(json.dumps(body), status_code=status_code, headers=headers)
...
def health(req: func.HttpRequest) -> func.HttpResponse:
    ...
    return response(200, {"status": "healthy", "service": "deepr-api", "version": "2.6.0"})
...
def submit_job(req: func.HttpRequest) -> func.HttpResponse:
    ...
    return response(
        200,
        {"job": {"id": job_id, "prompt": prompt, "model": model, "status": "queued"}, "estimated_cost": estimated_cost},
    )
...
def list_jobs(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if status_filter:
        ...
        parameters = [{"name": "@status", "value": status_filter}, {"name": "@limit", "value": limit}]
    else:
...
def get_result(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if job.get("status") != "completed":
        return response(400, {"error": "Job not completed yet", "status": job.get("status")})
    ...
    try:
        ...
        return response(
            200, {"job_id": job_id, "content": content, "format": "markdown", "completed_at": job.get("completed_at")}
        )
    except Exception:
