...
## 2026-06-21 — Portraits generated (19/19, branded) + archive-on-delete

- **Generated all 19 branded portraits** via the metered provider (no local FLUX endpoint configured yet): 19/19, ~$0.76, `portrait_url` wired into each profile. (Once the user stands up a local FLUX/ComfyUI OpenAI-Images endpoint and sets `DEEPR_LOCAL_IMAGE_URL`, regeneration is $0.)
- **`deepr expert delete` now archives by default**: it tars the expert's whole canonical directory into a gitignored archive folder (`<data>/expert-archive/<slug>-<UTCstamp>.tar.gz`, sibling of the experts root so it's never mistaken for an expert) *before* removing it, so a delete is recoverable. `--purge` skips the archive for an outright delete. Archive helper lives in `expert_cleanup.py` (`archive_expert`); the delete command guards on dir existence (archive is skipped if there's nothing on disk) and still prints the vector-store cleanup hint for cloud-backed experts. Live-verified: a throwaway expert archived to `data/expert-archive/...tar.gz` then vanished from `expert list`. New test + existing delete tests green (73).
