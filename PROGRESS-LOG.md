...
- **Confirmed a second, deeper gap (bug B, not yet fixed):** expert *chat consultation* (the council's `agentic=False` perspective generations) incurred ~$0.01/expert but wrote **no** ledger entries - chat records *research* costs via `cost_safety.record_cost` but the plain answer-generation cost path bypasses the ledger. That violates "every spend source writes the ledger." Investigating/fixing next.
