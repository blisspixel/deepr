...
"""
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(f"\n{'='*70}")
        print(f"  Creating Keyboards Expert (E2E Test)")
        print(f"{'='*70}\n")
        ...
        vector_store = await provider.create_vector_store(
            name=f"test-keyboards-{datetime.utcnow().strftime('%Y%m%d%H%M%S')}",
            file_ids=[file_id]
        )
        ...
        try:
            ...
            if hasattr(updated_profile, 'beliefs') and updated_profile.beliefs:
                ...
                print(f"  ⚠ No beliefs formed yet (may need synthesis)")
            ...
            for question in test_questions:
                ...
                try:
                    ...
                    if "switch" in question.lower():
                        ...
                        has_switches = any(term in response_lower for term in ['cherry', 'red', 'brown', 'blue', 'linear', 'tactile'])
                        assert has_switches, "Response doesn't mention switch types"
                        print(f"  ✓ Mentioned switch types")
                    
                    if "benefit" in question.lower():
                        # Should mention benefits
                        has_benefits = any(term in response_lower for term in ['durability', 'typing', 'customizable', 'rollover'])
                        assert has_benefits, "Response doesn't mention benefits"
                        print(f"  ✓ Mentioned benefits")
            ...
            print(f"  - Can answer domain questions: ✓")
            ...
            if store.load(expert_name):
                ...
                print(f"  ✓ Deleted expert profile")
