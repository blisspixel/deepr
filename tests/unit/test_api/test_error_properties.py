...
error_messages_with_secrets = st.one_of(
    ...
    st.builds(
        ...
        key=st.text(
            alphabet="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789", min_size=20, max_size=40
        ),
    ),
    ...
    st.builds(
        ...
        msg=st.text(min_size=1, max_size=50),
        pwd=st.text(
            alphabet="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%", min_size=8, max_size=20
        ),
    ),
...
safe_error_messages = st.text(
    alphabet="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 .,!?-_()[]{}", min_size=1, max_size=200
)
    ...
    def test_unexpected_error_with_secrets_not_exposed(self, secret_message):
        ...
        **Validates: Requirements 3.1, 3.4**
        ...
        with app.test_client() as client:
            ...
            assert not has_secret, f"Secret pattern found in unexpected error response: {matched}"
    ...
    def test_known_secret_patterns_not_in_unexpected_response(self, secret_value):
        ...
        **Validates: Requirements 3.1, 3.4**
        ...
        with app.test_client() as client:
            ...
            assert secret_value not in response_text, f"Secret value found in response: {secret_value}"
...
class TestErrorTypeConsistencyProperty:
    ...
    **Validates: Requirements 4.2, 4.3, 4.4, 4.7**
    ...
    def test_api_handler_returns_valid_json(self, message, error_code):
        ...
        **Validates: Requirements 4.4, 4.7**
        ...
        with app.test_client() as client:
            ...
            assert response.content_type == "application/json", f"Response must be JSON, got {response.content_type}"
    ...
    def test_all_error_subclasses_have_consistent_structure(self, error_class, args, expected_code):
    ...
    @pytest.mark.parametrize(
        "error_class,args,expected_status",
        [
    ...
    def test_error_handler_returns_correct_http_status(self, error_class, args, expected_status):
    ...
    def test_error_details_preserved_in_response(self, message, details):
        ...
        **Validates: Requirements 4.4**
        ...
        with app.test_client() as client:
            ...
            assert data["details"] == details, f"Details not preserved. Expected {details}, got {data['details']}"
...
class TestCombinedErrorProperties:
    ...
    @given(
        exception_type=st.sampled_from(
            [RuntimeError, ValueError, TypeError, KeyError, AttributeError, IndexError, ZeroDivisionError]
        ),
        message=st.one_of(safe_error_messages, error_messages_with_secrets),
    )
    @settings(max_examples=50, suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture])
    def test_unexpected_errors_always_return_generic_response(self, exception_type, message):
