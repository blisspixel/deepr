...
class TestJobSubmission:
    ...
    def test_submit_job_without_prompt_returns_400(self, client):
        ...
        response = client.post("/api/jobs", json={}, content_type="application/json")
    ...
    def test_submit_job_with_empty_prompt_returns_400(self, client):
        ...
        response = client.post("/api/jobs", json={"prompt": ""}, content_type="application/json")
...
class TestJobListing:
    ...
    def test_list_jobs_returns_jobs(self, client):
        ...
        mock_jobs = [
            ResearchJob(
                id="job-1", prompt="Prompt 1", model="o4-mini-deep-research", status=JobStatus.COMPLETED, priority=3
            ),
            ResearchJob(
                id="job-2", prompt="Prompt 2", model="o4-mini-deep-research", status=JobStatus.QUEUED, priority=2
            ),
        ]
...
class TestResults:
    ...
    def test_get_result_returns_content_for_completed_job(self, client):
        ...
        client.mock_queue.get_job = AsyncMock(return_value=mock_job)
        client.mock_storage.get_report = AsyncMock(return_value=b"# Research Report\n\nContent here...")
...
class TestAPIResponseStructure:
    ...
    def test_job_list_response_structure(self, status_filter):
        ...
        mock_jobs = [
            ResearchJob(
                id="job-1", prompt="Test", model="o4-mini-deep-research", status=JobStatus.COMPLETED, priority=3
            )
        ]
