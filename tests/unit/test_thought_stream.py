...
class TestThoughtStreamBasic:
    ...
    def test_create_thought_stream(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            stream = ThoughtStream(expert_name="test_expert", verbose=True, log_dir=Path(tmpdir))
    ...
    def test_emit_thought(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            stream = ThoughtStream(expert_name="test_expert", verbose=False, log_dir=Path(tmpdir))

            thought = stream.emit(ThoughtType.PLAN_STEP, "Planning the analysis", private_payload={"step": 1})
    ...
    def test_emit_writes_to_log(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            stream.emit(ThoughtType.DECISION, "Made a decision", private_payload={"reason": "test"})

            # Log file should exist
            assert stream.log_path.exists()

            # Read and verify log content
            with open(stream.log_path) as f:
    ...
    def test_decision_method(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            stream.decision("Using vector search", confidence=0.9, evidence=["doc1.md"], reasoning="Internal reasoning")
    ...
    def test_evidence_method(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            stream.evidence(
                source_id="doc1.md", summary="Found relevant information about quantum computing", relevance=0.85
            )
    ...
    def test_tool_call_method(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            stream.tool_call(
                tool_name="web_search", args={"query": "quantum computing"}, result_summary="Found 10 results"
            )
    ...
    def test_error_method(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            stream.error("Failed to connect to API", details={"status_code": 500})
...
class TestThoughtStreamVerboseModes:
    ...
    def test_verbose_mode(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            stream = ThoughtStream(expert_name="test_expert", verbose=True, quiet=False, log_dir=Path(tmpdir))
    ...
    def test_quiet_mode_still_logs(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            stream = ThoughtStream(expert_name="test_expert", verbose=False, quiet=True, log_dir=Path(tmpdir))
...
class TestThoughtStreamMetadata:
    ...
    def test_metadata_includes_expert_name(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            stream = ThoughtStream(expert_name="quantum_expert", verbose=False, log_dir=Path(tmpdir))
    ...
    def test_custom_metadata(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            stream.emit(ThoughtType.DECISION, "Decision", custom_field="custom_value", another_field=42)
...
class TestRedactionRulesPropertyTests:
    ...
    @given(prefix=st.text(min_size=0, max_size=50), suffix=st.text(min_size=0, max_size=50))
    ...
    @given(prefix=st.text(min_size=0, max_size=30), suffix=st.text(min_size=0, max_size=30))
    @settings(max_examples=50)
...
class TestThoughtStreamDualSinkPropertyTests:
    ...
    def test_emit_writes_to_both_sinks(self, thought_type, public_text, confidence):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            stream.emit(thought_type, public_text, confidence=confidence)
    ...
    @given(
        ...
        thought_types=st.lists(st.sampled_from(list(ThoughtType)), min_size=1, max_size=20),
    )
    @settings(max_examples=30)
    ...
    def test_private_payload_in_log_only(self, public_text, private_data):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            stream.emit(ThoughtType.DECISION, public_text, private_payload=private_data if private_data else None)
...
class TestThoughtStreamVerboseQuietPropertyTests:
    ...
    def test_mode_does_not_affect_logging(self, verbose, quiet, thought_type, text):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            stream = ThoughtStream(expert_name="test_expert", verbose=verbose, quiet=quiet, log_dir=Path(tmpdir))
    ...
    @given(verbose=st.booleans(), quiet=st.booleans(), num_thoughts=st.integers(min_value=1, max_value=10))
    @settings(max_examples=30)
    ...
    @given(
        ...
        evidence_refs=st.lists(
            st.text(alphabet=st.characters(whitelist_categories=("L", "N")), min_size=1, max_size=20), max_size=3
        ),
    )
...
class TestThoughtPropertyTests:
    ...
    def test_thought_to_dict_round_trip(self, thought_type, public_text, confidence, evidence_refs):
        ...
        thought = Thought(
            thought_type=thought_type, public_text=public_text, confidence=confidence, evidence_refs=evidence_refs
        )
    ...
    def test_thought_type_value_in_dict(self, thought_type):
        ...
        thought = Thought(thought_type=thought_type, public_text="Test")
...
class TestAdversarialRedactionTests:
    ...
    @given(hex_chars=st.lists(st.sampled_from(list("0123456789abcdef")), min_size=32, max_size=32))
    @settings(max_examples=30)
