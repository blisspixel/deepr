...
class TestPathHandlerPropertyTests:
    ...
    def test_normalize_produces_absolute_path(self, path_str):
        ...
        try:
            ...
            if os.name == "nt":
                ...
                is_drive_path = (
                    len(path_str_result) >= 3
                    and path_str_result[1] == ":"
                    and path_str_result[0].isalpha()
                    and path_str_result[2] in ("\\", "/")
                )
    ...
    def test_escape_handles_spaces_consistently(self, shell):
        ...
        assert has_quotes or has_escaped_spaces or no_raw_spaces, f"Spaces should be handled: {escaped}"
    ...
    def test_safe_filename_no_invalid_chars(self, filename):
        ...
        for char in invalid_chars:
            assert char not in result, f"Safe filename should not contain '{char}': {result}"
    ...
    def test_safe_filename_length_limit(self, filename):
        ...
        result = get_safe_filename(filename)
        assert len(result) <= 255, f"Safe filename should not exceed 255 chars: {len(result)}"
...
class TestPathNormalizationPropertyTests:
    ...
    @given(
        st.lists(
            st.text(min_size=1, max_size=20, alphabet="abcdefghijklmnopqrstuvwxyz0123456789_-"), min_size=1, max_size=5
        )
    )
    @settings(max_examples=50)
