...
class TestReasoningStep:
    ...
    def test_reasoning_step_to_dict(self):
        ...
        step = ReasoningStep(step_type="hypothesis", content="Test hypothesis", confidence=0.7, sources=["doc1.md"])
        d = step.to_dict()
...
class TestEpisode:
    ...
    def test_create_episode(self):
        ...
        episode = Episode(query="What is quantum computing?", response="Quantum computing uses qubits...")
    ...
    def test_episode_with_explicit_id(self):
        ...
        episode = Episode(id="custom_id", query="Test", response="Response")
    ...
    def test_episode_from_dict(self):
        ...
        data = {
            ...
            "response": "Test response",
            "context_docs": ["doc1.md"],
            "reasoning_chain": [{"step_type": "search", "content": "Searching", "confidence": 0.8, "sources": []}],
            "user_id": "user_123",
    ...
    def test_episode_get_keywords(self):
        ...
        episode = Episode(query="What is quantum computing?", response="Quantum computing uses qubits for computation.")
...
class TestUserProfile:
    ...
    def test_user_profile_to_dict(self):
        ...
        profile = UserProfile(
            user_id="user_123", expertise_levels={"python": 0.8}, interests={"programming": 5}, interaction_count=10
        )
        d = profile.to_dict()
    ...
    def test_get_top_interests(self):
        ...
        profile = UserProfile(user_id="user_123")
        profile.interests = {"python": 10, "javascript": 5, "rust": 3, "go": 1}
    ...
    def test_get_expertise_for_query(self):
        ...
        profile = UserProfile(user_id="user_123")
        profile.expertise_levels = {"python": 0.9, "javascript": 0.5}
...
class TestMetaKnowledge:
    ...
    def test_meta_knowledge_to_dict(self):
        ...
        meta = MetaKnowledge(
            expert_name="test_expert", domains={"python": 0.9}, confidence_by_topic={"decorators": 0.8}
        )
        d = meta.to_dict()
...
class TestHierarchicalMemory:
    ...
    def test_add_episode(self, tmp_path):
        ...
        episode = Episode(query="What is Python?", response="Python is a programming language.")
    ...
    def test_consolidate(self, tmp_path):
        ...
        memory = HierarchicalMemory(expert_name="test_expert", storage_dir=tmp_path / "memory", working_capacity=4)

        # Add more episodes than capacity
        for i in range(6):
            episode = Episode(query=f"Question {i}", response=f"Answer {i}")
    ...
    def test_retrieve_from_episodic_memory(self, tmp_path):
        ...
        memory = HierarchicalMemory(expert_name="test_expert", storage_dir=tmp_path / "memory", working_capacity=2)

        # Add episodes to trigger consolidation
        for i in range(5):
            episode = Episode(query=f"Python question {i}", response=f"Python answer {i}")
            memory.add_episode(episode)
    ...
    def test_retrieve_with_tier_filter(self, tmp_path):
        ...
        results = memory.retrieve("Python", top_k=5, tiers=[MemoryTier.WORKING])
    ...
    def test_update_user_profile(self, tmp_path):
        ...
        memory.update_user_profile("user_123", "How do I use Python decorators?", response_quality=0.9)
    ...
    def test_persistence(self, tmp_path):
        ...
        memory1 = HierarchicalMemory(expert_name="test_expert", storage_dir=storage_dir, working_capacity=2)
...
class TestReconstructedContext:
    ...
    def test_reconstructed_context_to_dict(self):
        ...
        episode = Episode(query="Test", response="Response")
        context = ReconstructedContext(episode=episode, documents=[{"name": "doc1.md", "content": "Content"}])
        d = context.to_dict()
...
class TestHierarchicalMemoryReconstructContext:
    ...
    def test_reconstruct_context(self, tmp_path):
        ...
        episode = Episode(
            ...
            reasoning_chain=[ReasoningStep(step_type="search", content="Searching for Python info")],
        )
        episode_id = memory.add_episode(episode)
    ...
    def test_reconstruct_context_with_related_episodes(self, tmp_path):
        ...
        session_id = "session_123"
        for i in range(3):
            episode = Episode(query=f"Question {i}", response=f"Answer {i}", session_id=session_id)
            memory.add_episode(episode)
...
class TestEpisodePropertyTests:
    ...
    def test_episode_serialization_round_trip(self, query, response, quality_score):
        ...
        episode = Episode(query=query, response=response, quality_score=quality_score)
    ...
    @given(tags=st.lists(st.text(min_size=1, max_size=20, alphabet=string.ascii_letters), min_size=0, max_size=10))
    @settings(max_examples=30)
    def test_episode_tags_preserved(self, tags):
        """Property: Episode tags are preserved through serialization."""
        episode = Episode(query="Test", response="Response", tags=tags)
    ...
    def test_episode_reasoning_chain_preserved(self, num_steps, confidences):
        ...
        for i in range(min(num_steps, len(confidences))):
            steps.append(ReasoningStep(step_type="search", content=f"Step {i}", confidence=confidences[i]))

        episode = Episode(query="Test", response="Response", reasoning_chain=steps)
...
class TestUserProfilePropertyTests:
    ...
    def test_serialization_round_trip(self, expertise_levels):
        ...
        profile = UserProfile(user_id="test", expertise_levels=expertise_levels)
...
class TestMemoryTierConsistencyPropertyTests:
    ...
    @given(num_episodes=st.integers(min_value=1, max_value=20), working_capacity=st.integers(min_value=2, max_value=10))
    ...
    def test_tier_consistency_after_adds(self, num_episodes, working_capacity):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            memory = HierarchicalMemory(
                expert_name="test_expert", storage_dir=Path(tmp_dir) / "memory", working_capacity=working_capacity
            )
    ...
    @given(num_episodes=st.integers(min_value=5, max_value=15))
    ...
    def test_consolidation_preserves_total_count(self, num_episodes):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            memory = HierarchicalMemory(
                expert_name="test_expert", storage_dir=Path(tmp_dir) / "memory", working_capacity=4
            )
    ...
    def test_retrieval_returns_relevant_episodes(self, query_keywords):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            memory = HierarchicalMemory(expert_name="test_expert", storage_dir=Path(tmp_dir) / "memory")

            # Add episode with keywords
            keyword_str = " ".join(query_keywords)
            episode = Episode(query=f"Question about {keyword_str}", response=f"Answer about {keyword_str}")
            memory.add_episode(episode)
...
class TestHierarchicalRetrievalPropertyTests:
    ...
    @given(
        tier_filter=st.sampled_from(
            [[MemoryTier.WORKING], [MemoryTier.EPISODIC], [MemoryTier.WORKING, MemoryTier.EPISODIC]]
        )
    )
    ...
    def test_retrieval_respects_tier_filter(self, tier_filter):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            memory = HierarchicalMemory(
                expert_name="test_expert", storage_dir=Path(tmp_dir) / "memory", working_capacity=3
            )
    ...
    def test_keyword_index_contains_all_episodes(self, queries):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for query in queries:
                episode = Episode(query=query, response=f"Response to {query}")
...
class TestRelevanceScoringPropertyTests:
    ...
    @given(quality_score=st.one_of(st.none(), st.floats(min_value=0.0, max_value=1.0, allow_nan=False)))
    @settings(max_examples=20, suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture])
