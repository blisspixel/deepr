...
def test_graph_commit_envelope_v8_schema_validates_agenda_payload():
    ...
    extraction = build_semantic_claim_extraction(
        notes,
        {
            "claims": [
                {
                    "statement": "Explore replayable evidence for agenda quality.",
                    "confidence": 0.84,
            ...
            "verifications": [
                {
                    ...
                    "rationale": "The expert needs an agenda item before wider perspective writes.",
                    "uncertainty": "The useful acceptance evidence is not complete.",
                    "expected_observations": ["Future syncs surface repeated agenda-quality failures."],
                    "disconfirming_signals": ["No later trace depends on this agenda."],
                    "confidence": 0.82,
                }
            ]
        },
            ...
            "claims": [
                {
                    ...
                    "title": "Statistical variable map",
                    "confidence": 0.73,
                    "claim_kind": "concept",
                    "source_refs": [{"note_id": note["note_id"], "window_id": window["window_id"]}],
                }
            ...
            "verifications": [
                {
                    ...
                    "temporal_scope_verdict": "not_applicable",
                    "origin": "Reviewed source-note evidence.",
                    "rationale": "The concept should remain perspective state until calibrated.",
...
def test_graph_commit_envelope_v8_schema_validates_original_idea_payload():
            ...
            "verifications": [
                {
                    ...
                    "temporal_scope_verdict": "not_applicable",
                    "origin": "Reviewed source-note evidence.",
                    "rationale": "The original idea should remain perspective state until calibrated.",
    ...
    _validate(schema, payload)
    assert payload["schema_version"] == GRAPH_COMMIT_ENVELOPE_SCHEMA_VERSION
    assert payload["operations"][0]["operation"] == "promote_original_idea"
...
def test_graph_commit_apply_schema_validates_agenda_result(tmp_path):
    title = "Explore schema validation for agenda promotion."
    envelope = graph_commit_envelope(
        graph_commit_agenda_operation(title, "b" * 64),
        expert_name="Schema Expert",
    )
    store = BeliefStore("Schema Expert", storage_dir=tmp_path / "beliefs")
...
def test_graph_commit_apply_schema_validates_concept_result(tmp_path):
    name = "Statistical variable map for expert council plans"
    envelope = graph_commit_envelope(
        graph_commit_concept_operation(name, "d" * 64),
        expert_name="Schema Expert",
    )
    store = BeliefStore("Schema Expert", storage_dir=tmp_path / "beliefs")
    tracker = MetaCognitionTracker("Schema Expert", base_path=str(tmp_path / "experts"))
...
def test_graph_commit_apply_schema_validates_original_idea_result(tmp_path):
    title = "Statistician council packets"
    envelope = graph_commit_envelope(
        graph_commit_original_idea_operation(title, "f" * 64),
        expert_name="Schema Expert",
    )
    store = BeliefStore("Schema Expert", storage_dir=tmp_path / "beliefs")
    tracker = MetaCognitionTracker("Schema Expert", base_path=str(tmp_path / "experts"))
    ...
    _validate(schema, payload)
    assert payload["schema_version"] == GRAPH_COMMIT_APPLY_SCHEMA_VERSION
    assert payload["operation_results"][0]["original_idea_title"] == title
    assert payload["summary"]["status"] == "applied"
