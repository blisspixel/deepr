...
provider_names = st.sampled_from(["openai", "anthropic", "xai", "azure", "gemini", "cohere", "mistral"])
...
error_messages = st.text(
    alphabet="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 .,!?-_", min_size=0, max_size=100
)
    ...
    **Validates: Requirements 6.2, 6.4, 6.5, 6.6**
    ...
    def test_closed_to_open_on_threshold_failures(self, provider, model, threshold):
        ...
        **Validates: Requirements 6.2**
        """
        cb = CircuitBreaker(provider=provider, model=model, failure_threshold=threshold)
        ...
        assert cb.state == CircuitState.OPEN, f"Circuit should be OPEN after {threshold} failures"
    ...
    def test_open_to_half_open_after_timeout(self, provider, model, timeout):
        ...
        **Validates: Requirements 6.4**
        ...
        cb.last_state_change = datetime.now(UTC) - timedelta(seconds=timeout - 1)
        cb._check_recovery()
        assert cb.state == CircuitState.OPEN, "Circuit should remain OPEN before timeout"

        # Simulate time passing (at or after timeout)
        cb.last_state_change = datetime.now(UTC) - timedelta(seconds=timeout)
        cb._check_recovery()
        assert cb.state == CircuitState.HALF_OPEN, "Circuit should be HALF_OPEN after timeout"
    ...
    def test_half_open_to_closed_on_success(self, provider, model):
        ...
        **Validates: Requirements 6.5**
        """
        cb = CircuitBreaker(provider=provider, model=model, failure_threshold=1, recovery_timeout=1)
        ...
        cb.last_state_change = datetime.now(UTC) - timedelta(seconds=2)
        cb._check_recovery()
        assert cb.state == CircuitState.HALF_OPEN

        # Record success - should close
        cb.record_success()
        assert cb.state == CircuitState.CLOSED, "Circuit should be CLOSED after success in HALF_OPEN state"
        assert cb.failure_count == 0, "Failure count should be reset after closing"
    ...
    def test_half_open_to_open_on_failure(self, provider, model, error):
        ...
        **Validates: Requirements 6.6**
        ...
        cb.record_failure(error)
        assert cb.state == CircuitState.OPEN, "Circuit should be OPEN after failure in HALF_OPEN state"
    ...
    def test_state_machine_invariants(self, events):
        ...
        **Validates: Requirements 6.2, 6.4, 6.5, 6.6**
        """
        cb = CircuitBreaker(provider="openai", model="gpt-4o", failure_threshold=3, recovery_timeout=1)
    ...
    @given(threshold=failure_thresholds, extra_failures=st.integers(min_value=0, max_value=10))
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow])
    def test_failures_beyond_threshold_keep_circuit_open(self, threshold, extra_failures):
        ...
        **Validates: Requirements 6.2**
        """
        cb = CircuitBreaker(provider="openai", model="gpt-4o", failure_threshold=threshold)
        ...
        for i in range(extra_failures):
            ...
            assert cb.state == CircuitState.OPEN, "Circuit should remain OPEN with additional failures"
    ...
    **Validates: Requirements 6.3**
    ...
    def test_open_circuit_is_not_available(self, provider, model):
        ...
        **Validates: Requirements 6.3**
        ...
        assert cb.is_available is False, "OPEN circuit should not be available"
    ...
    def test_closed_circuit_is_available(self, provider, model):
        ...
        **Validates: Requirements 6.3**
        ...
        assert cb.state == CircuitState.CLOSED
        assert cb.is_available is True, "CLOSED circuit should be available"
    ...
    def test_half_open_circuit_is_available(self, provider, model):
        ...
        **Validates: Requirements 6.3**
        ...
        assert cb.state == CircuitState.HALF_OPEN
        assert cb.is_available is True, "HALF_OPEN circuit should be available to test recovery"
    ...
    def test_time_until_recovery_when_open(self, provider, model, timeout):
        ...
        **Validates: Requirements 6.3, 6.7**
        """
        cb = CircuitBreaker(provider=provider, model=model, failure_threshold=1, recovery_timeout=timeout)
...
class TestCircuitBreakerRegistry:
    ...
    @given(provider=provider_names, model=model_names, threshold=failure_thresholds, timeout=recovery_timeouts)
    ...
    def test_registry_uses_configured_defaults(self, provider, model, threshold, timeout):
        ...
        **Validates: Requirements 6.2, 6.4**
        """
        registry = CircuitBreakerRegistry(failure_threshold=threshold, recovery_timeout=timeout)
...
class TestCircuitBreakerProviderRouterIntegration:
    ...
    def test_router_accepts_custom_registry(self, temp_storage):
        ...
        router = AutonomousProviderRouter(storage_path=temp_storage, circuit_breaker_registry=custom_registry)
    ...
    def test_router_updates_circuit_on_failure(self, temp_storage):
        ...
        router = AutonomousProviderRouter(storage_path=temp_storage, circuit_breaker_registry=registry)
