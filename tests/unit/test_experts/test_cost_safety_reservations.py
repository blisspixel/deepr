...
class TestReservationFlow:
    ...
    def test_record_cost_settles_reservation(self, manager):
        ...
        assert manager.daily_cost == pytest.approx(0.75)
    ...
    def test_parallel_reservations_block_over_commit(self, manager):
        ...
        # First call reserves 1.0
        a_allowed, _, _, a_id = manager.check_and_reserve(session_id="s1", operation_type="r", estimated_cost=1.0)
        # Second call estimates 0.75 - should be rejected because the
        # first call's reservation is still in flight.
        b_allowed, b_reason, _, _ = manager.check_and_reserve(session_id="s2", operation_type="r", estimated_cost=0.75)
        ...
        c_allowed, _, _, _ = manager.check_and_reserve(session_id="s2", operation_type="r", estimated_cost=0.75)
...
class TestMonthlyReservationSymmetry:
    ...
    constraint (the $5/month fleet case, where daily headroom is large), N
    ...
    def test_record_cost_settles_monthly_reservation(self, manager):
        allowed, _, _, rid = manager.check_and_reserve(session_id="s1", operation_type="research", estimated_cost=1.0)
        assert allowed
        assert manager._reserved_monthly == 1.0

        manager.record_cost(session_id="s1", operation_type="research", actual_cost=0.75, reservation_id=rid)
        assert manager._reserved_monthly == 0.0
        assert manager.monthly_cost == pytest.approx(0.75)

    def test_refund_reservation_releases_monthly_without_billing(self, manager):
        _, _, _, rid = manager.check_and_reserve(session_id="s1", operation_type="research", estimated_cost=1.0)
        manager.refund_reservation(rid)
        assert manager._reserved_monthly == 0.0
    ...
    def test_parallel_reservations_block_over_commit_monthly(self, manager):
        ...
        manager.max_daily = 2.0  # roomy, never the binding constraint here
        manager.max_monthly = 1.5  # the binding reserve
        a_allowed, _, _, a_id = manager.check_and_reserve(session_id="s1", operation_type="r", estimated_cost=1.0)
        b_allowed, b_reason, _, _ = manager.check_and_reserve(session_id="s2", operation_type="r", estimated_cost=0.75)
        assert a_allowed is True
        assert b_allowed is False
...
class TestReservationTTL:
    ...
    def test_stale_reservation_is_swept_on_next_check(self, manager):
        manager.max_daily = 1.5
        _, _, _, leaked = manager.check_and_reserve(session_id="s1", operation_type="r", estimated_cost=1.0)
        assert manager._reserved_daily == 1.0
        ...
        # The next check sweeps it, so a fresh 1.0 reservation now fits.
        allowed, _, _, _ = manager.check_and_reserve(session_id="s2", operation_type="r", estimated_cost=1.0)
        assert allowed is True
    ...
    def test_live_reservation_is_not_swept(self, manager):
        manager.max_daily = 1.5
        _, _, _, fresh = manager.check_and_reserve(session_id="s1", operation_type="r", estimated_cost=1.0)
        # Still within TTL -> a parallel over-commit is still correctly blocked.
        allowed, reason, _, _ = manager.check_and_reserve(session_id="s2", operation_type="r", estimated_cost=1.0)
        assert allowed is False
        assert "Daily limit" in reason
        assert fresh in manager._reservations  # not swept

    def test_sweep_refunds_both_daily_and_monthly(self, manager):
        manager.check_and_reserve(session_id="s1", operation_type="r", estimated_cost=1.0)
        assert manager._reserved_daily == 1.0
        assert manager._reserved_monthly == 1.0
    ...
    def test_settling_clears_the_timestamp(self, manager):
        _, _, _, rid = manager.check_and_reserve(session_id="s1", operation_type="r", estimated_cost=1.0)
        assert rid in manager._reservation_started
        manager.record_cost(session_id="s1", operation_type="r", actual_cost=0.5, reservation_id=rid)
