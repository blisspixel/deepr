...
from deepr.experts.serializer import datetime_to_iso, iso_to_datetime
...
vector_store_id_strategy = st.text(
    alphabet=st.sampled_from("abcdefghijklmnopqrstuvwxyz0123456789_"), min_size=5, max_size=30
...
class TestSerializationRoundTrip:
    ...
    def test_profile_roundtrip_preserves_core_fields(
        self, name: str, vector_store_id: str, domain_velocity: str, monthly_budget: float, monthly_spending: float
    ):
    ...
    def test_profile_roundtrip_preserves_datetime_fields(
        self, name: str, vector_store_id: str, knowledge_cutoff, last_refresh
    ):
    ...
    def test_profile_roundtrip_preserves_counters(
        self, name: str, vector_store_id: str, conversations: int, research_triggered: int
    ):
...
class TestDomainVelocityThresholds:
    ...
    @given(domain_velocity=domain_velocity_strategy, days_old=st.integers(min_value=0, max_value=365))
    @settings(max_examples=100, deadline=None)
    def test_staleness_consistent_with_velocity(self, domain_velocity: str, days_old: int):
        """Property: staleness detection is consistent with domain velocity thresholds."""
        now = utc_now()
        cutoff = now - timedelta(days=days_old)

        profile = ExpertProfile(
            name="test", vector_store_id="vs_test", knowledge_cutoff_date=cutoff, domain_velocity=domain_velocity
        )
    ...
    def test_incomplete_expert_always_incomplete_status(self, domain_velocity: str):
        ...
        profile = ExpertProfile(
            name="test", vector_store_id="vs_test", knowledge_cutoff_date=None, domain_velocity=domain_velocity
        )
...
class TestBudgetManagerProperties:
    ...
    @given(budget=budget_strategy, spending=spending_strategy)
    @settings(max_examples=50, deadline=None)
    def test_remaining_budget_never_negative(self, budget: float, spending: float):
        """Property: remaining budget is never negative."""
        manager = BudgetManager(monthly_budget=budget, monthly_spending=spending)
    ...
    def test_budget_manager_serialization_roundtrip(self, budget: float, spending: float):
        ...
        original = BudgetManager(monthly_budget=budget, monthly_spending=spending, total_spending=spending * 2)
...
class TestActivityTrackerProperties:
    ...
    @given(conversations=st.integers(min_value=0, max_value=10000), research=st.integers(min_value=0, max_value=1000))
    @settings(max_examples=50, deadline=None)
    def test_activity_tracker_serialization_roundtrip(self, conversations: int, research: int):
        """Property: ActivityTracker survives serialization round-trip."""
        original = ActivityTracker(conversations=conversations, research_triggered=research)
