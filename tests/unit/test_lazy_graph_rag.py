...
class TestConcept:
    ...
    def test_create_concept(self):
        ...
        concept = Concept(text="quantum computing", concept_type="noun_phrase")
    ...
    def test_concept_with_explicit_id(self):
        ...
        concept = Concept(id="custom_id", text="test concept")
...
class TestEdge:
    ...
    def test_create_edge(self):
        ...
        edge = Edge(source_id="concept1", target_id="concept2", edge_type=EdgeType.CO_OCCURS, weight=0.8)
    ...
    def test_edge_id_property(self):
        ...
        edge = Edge(source_id="concept1", target_id="concept2", edge_type=EdgeType.DEFINED_IN)
...
class TestDocumentSection:
    ...
    def test_create_section(self):
        ...
        section = DocumentSection(document_id="doc1.md", content="This is the content", heading="Introduction", level=1)
    ...
    def test_section_id_generation(self):
        ...
        section1 = DocumentSection(document_id="doc1.md", content="Content A", start_pos=0)
        section2 = DocumentSection(document_id="doc1.md", content="Content B", start_pos=100)
...
class TestConceptExtractor:
    ...
    def test_create_extractor(self):
        ...
        extractor = ConceptExtractor(min_phrase_length=2, max_phrase_length=4)
...
class TestEdgeBuilder:
    ...
    def test_calculate_pmi(self):
        ...
        pmi = builder.calculate_pmi(c1_count=10, c2_count=10, cooccur_count=10, total_docs=100)
        assert pmi > 0  # Positive PMI for co-occurring concepts

        # No co-occurrence
        pmi_zero = builder.calculate_pmi(c1_count=10, c2_count=10, cooccur_count=0, total_docs=100)
        assert pmi_zero == 0.0
...
class TestRetrievalSufficiency:
    ...
    def test_create_sufficiency(self):
        ...
        sufficiency = RetrievalSufficiency(coverage=0.8, redundancy=0.2, citation_density=2.0, contradiction_rate=0.1)
    ...
    def test_is_sufficient(self):
        ...
        high_sufficiency = RetrievalSufficiency(
            coverage=0.9, redundancy=0.1, citation_density=3.0, contradiction_rate=0.0
        )
        high_sufficiency.compute_overall()

        low_sufficiency = RetrievalSufficiency(
            coverage=0.2, redundancy=0.8, citation_density=0.0, contradiction_rate=0.5
        )
...
class TestSufficiencyScorer:
    ...
    def test_score_with_chunks(self):
        ...
        chunks = [{"content": "Python is a programming language."}, {"content": "Python supports multiple paradigms."}]
    ...
    def test_redundancy_calculation(self):
        ...
        identical_chunks = [{"content": "Python is great."}, {"content": "Python is great."}]
...
class TestCachedSubgraph:
    ...
    def test_create_cached_subgraph(self):
        ...
        subgraph = CachedSubgraph(
            query_hash="abc123", node_ids={"node1", "node2"}, node_summaries={"node1": "Summary 1"}
        )
    ...
    def test_to_dict(self):
        ...
        subgraph = CachedSubgraph(
            query_hash="abc123", node_ids={"node1", "node2"}, prompt_blocks=["Block 1", "Block 2"]
        )
        d = subgraph.to_dict()
...
class TestSubgraphCache:
    ...
    def test_create_cache(self, tmp_path):
        ...
        cache = SubgraphCache(max_size=10, storage_path=tmp_path / "cache.json")
    ...
    def test_put_and_get(self, tmp_path):
        ...
        cache.put(query="What is Python?", node_ids={"node1", "node2"}, prompt_blocks=["Block 1"])
...
class TestKnowledgeGraph:
    ...
    def test_add_concept_merges_existing(self, tmp_path):
        ...
        merged = next(iter(graph.concepts.values()))
    ...
    def test_add_edge(self, tmp_path):
        ...
        edge = Edge(source_id=c1.id, target_id=c2.id, edge_type=EdgeType.CO_OCCURS, weight=0.8)
    ...
    def test_traverse(self, tmp_path):
        ...
        for i in range(3):
            edge = Edge(
                source_id=concepts[i].id, target_id=concepts[i + 1].id, edge_type=EdgeType.CO_OCCURS, weight=0.8
            )
            graph.add_edge(edge)
    ...
    def test_traverse_max_nodes(self, tmp_path):
        ...
        for i in range(1, 10):
            edge = Edge(source_id=concepts[0].id, target_id=concepts[i].id, edge_type=EdgeType.CO_OCCURS)
            graph.add_edge(edge)
    ...
    def test_persistence(self, tmp_path):
        ...
        graph1 = KnowledgeGraph(expert_name="test_expert", storage_dir=storage_dir)
        ...
        # Create new instance - should load from disk
        graph2 = KnowledgeGraph(expert_name="test_expert", storage_dir=storage_dir)
...
class TestLazyGraphRAG:
    ...
    def test_create_lazy_graph_rag(self, tmp_path):
        ...
        rag = LazyGraphRAG(expert_name="test_expert", storage_dir=tmp_path / "rag")
...
""",
        ...
        results = await rag.retrieve("How do neural networks relate to deep learning?", top_k=5, use_graph=True)
...
class TestConceptPropertyTests:
    ...
    def test_concept_serialization_round_trip(self, text, concept_type, frequency, tf_idf_score):
        ...
        concept = Concept(text=text.strip(), concept_type=concept_type, frequency=frequency, tf_idf_score=tf_idf_score)
    ...
    def test_concept_sets_preserved(self, original_forms, document_ids):
        ...
        concept = Concept(text="test concept", original_forms=set(original_forms), document_ids=set(document_ids))
...
class TestEdgePropertyTests:
    ...
    @given(
        ...
        edge_type=st.sampled_from(
            [EdgeType.CO_OCCURS, EdgeType.DEFINED_IN, EdgeType.DEPENDS_ON, EdgeType.MENTIONED_IN_SAME_SECTION]
        ),
    ...
    def test_edge_document_ids_preserved(self, document_ids):
        ...
        edge = Edge(
            source_id="source", target_id="target", edge_type=EdgeType.CO_OCCURS, document_ids=set(document_ids)
        )
...
class TestGraphTraversalPropertyTests:
    ...
    def test_traversal_respects_limits(self, num_concepts, max_depth, max_nodes):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            graph = KnowledgeGraph(expert_name="test_expert", storage_dir=Path(tmp_dir) / "graph")
    ...
    @given(num_concepts=st.integers(min_value=3, max_value=10))
    ...
    def test_traversal_visits_all_reachable(self, num_concepts):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for s in satellites:
                graph.add_concept(s)
                edge = Edge(source_id=center.id, target_id=s.id, edge_type=EdgeType.CO_OCCURS)
                graph.add_edge(edge)
    ...
    @given(start_indices=st.lists(st.integers(min_value=0, max_value=4), min_size=1, max_size=3, unique=True))
    @settings(max_examples=20, suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture])
...
class TestHybridRetrievalPropertyTests:
    ...
    def test_retrieval_respects_top_k(self, top_k):
        ...
        async def run_test():
            with tempfile.TemporaryDirectory() as tmp_dir:
                rag = LazyGraphRAG(expert_name="test_expert", storage_dir=Path(tmp_dir) / "rag")
    ...
    @given(num_docs=st.integers(min_value=1, max_value=10))
...
class TestCacheLRUPropertyTests:
    ...
    @given(max_size=st.integers(min_value=2, max_value=10), num_entries=st.integers(min_value=1, max_value=20))
    @settings(max_examples=30, suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture])
    def test_cache_respects_max_size(self, max_size, num_entries):
        """Property: Cache never exceeds max_size."""
        with tempfile.TemporaryDirectory() as tmp_dir:
            cache = SubgraphCache(max_size=max_size, storage_path=Path(tmp_dir) / "cache.json")
...
class TestRetrievalSufficiencyPropertyTests:
    ...
    def test_is_sufficient_consistent(self, threshold):
        ...
        sufficiency = RetrievalSufficiency(coverage=0.7, redundancy=0.2, citation_density=2.0, contradiction_rate=0.1)
...
class TestCachedSubgraphPropertyTests:
    ...
    def test_cached_subgraph_round_trip(self, query_hash, node_ids, access_count):
        ...
        subgraph = CachedSubgraph(query_hash=query_hash.strip(), node_ids=set(node_ids), access_count=access_count)
