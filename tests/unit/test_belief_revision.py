...
class TestBeliefDataclass:
    ...
    def test_belief_creation(self):
        ...
        belief = Belief(claim="Python 3.12 supports pattern matching", confidence=0.95)
...
class TestConfidenceDecay:
    ...
    def test_decay_rate_affects_speed(self):
        ...
        # Simulate 30 days old
        slow_decay.updated_at = datetime.now(UTC) - timedelta(days=30)
        fast_decay.updated_at = datetime.now(UTC) - timedelta(days=30)
    ...
    def test_is_stale_threshold(self):
        ...
        belief.updated_at = datetime.now(UTC) - timedelta(days=100)
...
class TestBeliefStore:
    ...
    def test_store_initialization(self, tmp_path):
        ...
        store = BeliefStore(expert_name="test_expert", storage_dir=tmp_path / "beliefs")
    ...
    def test_add_belief(self, tmp_path):
        ...
        belief = Belief(claim="Python is great", confidence=0.9, domain="programming")
    ...
    def test_update_belief(self, tmp_path):
        ...
        change = store.update_belief(added.id, new_confidence=0.8, reason="More evidence")
    ...
    def test_revise_belief(self, tmp_path):
        ...
        change = store.revise_belief(added.id, new_claim="New claim", new_confidence=0.9, reason="Better understanding")
    ...
    def test_get_stale_beliefs(self, tmp_path):
        ...
        stale.updated_at = datetime.now(UTC) - timedelta(days=100)
...
class TestConflictResolution:
    ...
    def test_newer_wins(self, tmp_path):
        ...
        store = BeliefStore(
            expert_name="test", storage_dir=tmp_path / "beliefs", conflict_resolution=ConflictResolution.NEWER_WINS
        )

        # Add initial belief
        belief1 = Belief(claim="Python version is 3.11", confidence=0.9, domain="python")
        store.add_belief(belief1)
    ...
    def test_merge_strategy(self, tmp_path):
        ...
        store = BeliefStore(
            expert_name="test", storage_dir=tmp_path / "beliefs", conflict_resolution=ConflictResolution.MERGE
        )

        # Add initial belief
        belief1 = Belief(claim="Python is popular", confidence=0.8, domain="python", evidence_refs=["ref1"])
        store.add_belief(belief1)
...
class TestBeliefChangeExpression:
    ...
    def test_created_expression(self):
        ...
        change = BeliefChange(belief_id="test", change_type="created", new_claim="Python is great", new_confidence=0.9)
...
class TestSharedBeliefStore:
    ...
    def test_share_low_confidence_rejected(self, tmp_path):
        ...
        belief = Belief(claim="Uncertain claim", confidence=0.5, domain="test")
    ...
    def test_cleanup_stale(self, tmp_path):
        ...
        stale.updated_at = datetime.now(UTC) - timedelta(days=365)
...
class TestBeliefProperties:
    ...
    def test_confidence_always_bounded(self, claim, confidence, decay_rate):
        ...
        belief = Belief(claim=claim, confidence=confidence, decay_rate=decay_rate)
    ...
    def test_decay_is_monotonic(self, claim, confidence, days_old):
        ...
        belief = Belief(claim=claim, confidence=confidence, decay_rate=0.01)

        # Get current confidence
        current1 = belief.get_current_confidence()

        # Age the belief
        belief.updated_at = datetime.now(UTC) - timedelta(days=days_old)
    ...
    def test_serialization_roundtrip(self, claim, confidence, domain):
        ...
        original = Belief(claim=claim, confidence=confidence, domain=domain)
    ...
    @given(evidence_refs=st.lists(st.text(min_size=1, max_size=50), min_size=0, max_size=20))
    @settings(max_examples=50, suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture])
...
class TestBeliefStoreProperties:
    ...
    @given(
        claims=st.lists(st.text(min_size=1, max_size=100), min_size=1, max_size=10),
        confidences=st.lists(st.floats(min_value=0.1, max_value=1.0, allow_nan=False), min_size=1, max_size=10),
    )
    ...
    def test_store_count_matches_beliefs(self, claims, confidences):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            store = BeliefStore(expert_name="test", storage_dir=Path(tmp_dir) / "beliefs")

            added_ids = set()
            for claim, conf in zip(claims, confidences):
                belief = Belief(claim=claim, confidence=conf, domain="test")
                added, _ = store.add_belief(belief)
    ...
    def test_domain_index_consistency(self, domain, num_beliefs):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for i in range(num_beliefs):
                belief = Belief(claim=f"Unique claim number {i} for domain {domain}", confidence=0.9, domain=domain)
    ...
    @given(
        claim=st.text(min_size=10, max_size=100), confidence=st.floats(min_value=0.1, max_value=1.0, allow_nan=False)
    )
    @settings(max_examples=30, suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture])
...
class TestConflictResolutionProperties:
    ...
    def test_higher_confidence_always_wins(self, conf1, conf2):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            belief1 = Belief(claim="Python is a programming language", confidence=conf1, domain="python")
            store.add_belief(belief1)

            # Add higher confidence similar belief
            belief2 = Belief(claim="Python is a programming language used widely", confidence=conf2, domain="python")
    ...
    def test_newer_always_wins_regardless_of_confidence(self, conf1, conf2):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            belief1 = Belief(claim="Python version is 3.11", confidence=conf1, domain="python")
            store.add_belief(belief1)

            # Add lower confidence similar belief (newer)
            belief2 = Belief(claim="Python version is 3.12", confidence=conf2, domain="python")
            added, change = store.add_belief(belief2)
...
class TestSharedBeliefStoreProperties:
    ...
    def test_domain_isolation(self, domains):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for domain in domains:
                belief = Belief(claim=f"Fact about {domain}", confidence=0.9, domain=domain)
                store.share_belief(belief, "expert1")
    ...
    @given(confidence=st.floats(min_value=0.0, max_value=0.6, allow_nan=False))
    @settings(max_examples=30, suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture])
    def test_low_confidence_rejected(self, confidence):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            belief = Belief(claim="Low confidence claim", confidence=confidence, domain="test")
    ...
    @given(num_experts=st.integers(min_value=2, max_value=5))
    ...
    def test_corroboration_tracks_all_contributors(self, num_experts):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for expert in expert_names:
                belief = Belief(claim="Python is popular for data science", confidence=0.9, domain="python")
...
class TestPersistenceProperties:
    ...
    @given(
        claims=st.lists(st.text(min_size=5, max_size=100), min_size=1, max_size=5, unique=True),
        confidences=st.lists(st.floats(min_value=0.1, max_value=1.0, allow_nan=False), min_size=1, max_size=5),
    )
    @settings(max_examples=20, suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture])
    ...
    def test_shared_persistence_preserves_contributors(self, domain, num_beliefs):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for i in range(num_beliefs):
                belief = Belief(claim=f"Unique fact number {i} about {domain}", confidence=0.9, domain=domain)
...
class TestDecayMathProperties:
    ...
    def test_half_life_property(self, confidence, decay_rate):
        ...
        belief.updated_at = datetime.now(UTC) - timedelta(days=half_life_days)
