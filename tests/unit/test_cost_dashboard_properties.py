...
operations = st.sampled_from(
    ["research", "chat", "synthesis", "fact_check", "analysis", "summarize", "translate", "embed", "search", "index"]
)
...
class TestAlertManagerProperties:
    ...
    @given(threshold_lists(), st.floats(min_value=0.0, max_value=1000.0, allow_nan=False, allow_infinity=False), limits)
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture])
    ...
    @given(threshold_lists(), st.floats(min_value=0.0, max_value=1000.0, allow_nan=False, allow_infinity=False), limits)
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture])
    def test_no_duplicate_alerts_same_day(self, thresholds: list[float], daily_total: float, daily_limit: float):
    ...
    @given(threshold_lists(), st.floats(min_value=0.0, max_value=1000.0, allow_nan=False, allow_infinity=False), limits)
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture])
    def test_alert_current_value_matches_input(self, thresholds: list[float], daily_total: float, daily_limit: float):
...
class TestCostDashboardProperties:
    ...
    @given(st.lists(st.tuples(operations, providers, costs, models), min_size=1, max_size=50))
    ...
    @given(st.lists(st.tuples(operations, providers, costs), min_size=1, max_size=50))
    @settings(max_examples=50, suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture])
    ...
    @given(st.lists(st.tuples(operations, providers, costs), min_size=1, max_size=30))
    ...
    @given(limits, st.floats(min_value=0.0, max_value=2.0, allow_nan=False, allow_infinity=False))
    @settings(max_examples=50, suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture])
    def test_alerts_trigger_at_correct_thresholds(self, daily_limit: float, utilization: float):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            dashboard = CostDashboard(
                storage_path=storage_path, daily_limit=daily_limit, alert_thresholds=[0.5, 0.8, 0.95]
            )
            ...
            for t in expected_thresholds:
                assert t in actual_thresholds, (
                    f"Threshold {t} should have triggered at {utilization * 100}% utilization"
                )

    @given(st.lists(st.tuples(operations, providers, costs, tokens, tokens), min_size=1, max_size=30))
    @settings(max_examples=30, suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture])
    def test_entry_count_matches_record_count(self, records):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for operation, provider, cost, tokens_in, tokens_out in records:
                dashboard.record(operation, provider, cost, tokens_input=tokens_in, tokens_output=tokens_out)
...
class TestCostAlertProperties:
    ...
    @given(st.sampled_from(["warning", "critical"]), thresholds, costs, limits, st.sampled_from(["daily", "monthly"]))
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture])
    def test_alert_serialization_roundtrip(
        ...
        alert = CostAlert(level=level, threshold=threshold, current_value=current_value, limit=limit, period=period)
