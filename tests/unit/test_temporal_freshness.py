...
class TestTemporalStateProperties:
    ...
    @given(
        activity_types=st.lists(
            st.sampled_from(["chat", "learn", "refresh", "upload", "other"]),
            min_size=1,
            max_size=50
        )
    )
    ...
    @given(
        activity_types=st.lists(
            st.sampled_from(["chat", "learn", "refresh", "upload"]),
            min_size=1,
            max_size=20
        )
    )
    ...
    @given(
        ...
        days_old=st.integers(min_value=0, max_value=500)
    )
    @settings(
        max_examples=50,
        suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture]
...
class TestFreshnessCheckerProperties:
    ...
    @given(
        velocity_days=st.integers(min_value=1, max_value=365),
        days_since=st.integers(min_value=0, max_value=1000)
    )
    @settings(
        max_examples=50,
        suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture]
    ...
    @given(
        velocity_days=st.integers(min_value=20, max_value=365)
    )
    @settings(
        max_examples=30,
        suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture]
    ...
    @given(
        velocity_days=st.integers(min_value=10, max_value=365),
        days_since=st.integers(min_value=0, max_value=500)
    )
    @settings(
        max_examples=50,
        suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture]
...
class TestBackwardCompatibility:
    ...
    @given(
        domain=st.sampled_from(list(FreshnessChecker.DEFAULT_VELOCITIES.keys()))
    )
    @settings(
        max_examples=20,
        suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture]
