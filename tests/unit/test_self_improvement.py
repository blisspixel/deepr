...
from deepr.experts.knowledge_consolidation import ConsolidationResult, KnowledgeConsolidator, KnowledgeEntry
...
class TestStalenessDetection:
    ...
    def test_no_cutoff_is_stale(self):
        ...
        profile = ExpertProfile(name="Test", vector_store_id="vs_test", knowledge_cutoff_date=None)
    ...
    def test_staleness_urgency_levels(self):
        ...
        incomplete_recent = ExpertProfile(
            name="IncompleteRecent", vector_store_id="vs_incomplete_recent", knowledge_cutoff_date=None
        )
...
class TestBudgetEnforcement:
    ...
    def test_can_spend_within_budget(self):
        ...
        profile = ExpertProfile(
            name="Test", vector_store_id="vs_test", monthly_learning_budget=10.0, monthly_spending=0.0
        )
    ...
    def test_cannot_spend_exceeding_budget(self):
        ...
        profile = ExpertProfile(
            name="Test", vector_store_id="vs_test", monthly_learning_budget=10.0, monthly_spending=8.0
        )
    ...
    def test_cannot_spend_exhausted_budget(self):
        ...
        profile = ExpertProfile(
            name="Test", vector_store_id="vs_test", monthly_learning_budget=10.0, monthly_spending=10.0
        )
    ...
    def test_get_monthly_budget_status(self):
        ...
        profile = ExpertProfile(
            name="Test", vector_store_id="vs_test", monthly_learning_budget=10.0, monthly_spending=3.0
        )
...
class TestKnowledgeEntry:
    ...
    def test_entry_serialization(self):
        ...
        original = KnowledgeEntry(content="Test content", source="test.md", confidence=0.8, tags={"test", "unit"})
...
class TestKnowledgeConsolidator:
    ...
    def test_consolidator_initialization(self, tmp_path):
        ...
        consolidator = KnowledgeConsolidator(expert_name="test_expert", storage_dir=tmp_path / "knowledge")
    ...
    def test_add_entry(self, tmp_path):
        ...
        consolidator = KnowledgeConsolidator(expert_name="test", storage_dir=tmp_path / "knowledge")
...
class TestDeduplication:
    ...
    def test_deduplicate_exact_duplicates(self, tmp_path):
        ...
        consolidator.add_entry(
            KnowledgeEntry(content="Python is a great programming language for data science work", confidence=0.8)
        )
        consolidator.add_entry(
            KnowledgeEntry(content="Python is a great programming language for data science tasks", confidence=0.9)
        )
    ...
    def test_deduplicate_near_duplicates(self, tmp_path):
        ...
        consolidator.add_entry(
            KnowledgeEntry(content="Python is excellent for machine learning and data analysis", confidence=0.8)
        )
        consolidator.add_entry(
            KnowledgeEntry(content="Python is excellent for machine learning and data science", confidence=0.7)
        )
    ...
    def test_deduplicate_keeps_distinct(self, tmp_path):
        ...
        consolidator = KnowledgeConsolidator(
            expert_name="test", storage_dir=tmp_path / "knowledge", similarity_threshold=0.85
        )

        # Add distinct entries
        consolidator.add_entry(KnowledgeEntry(content="Python is a programming language"))
        consolidator.add_entry(KnowledgeEntry(content="JavaScript runs in web browsers"))
...
class TestMergeRelated:
    ...
    def test_merge_related_by_tags(self, tmp_path):
        ...
        consolidator.add_entry(
            KnowledgeEntry(
                content="Python has excellent libraries for data science", tags={"python", "data"}, confidence=0.9
            )
        )
        consolidator.add_entry(
            KnowledgeEntry(content="Python provides tools for data analysis", tags={"python", "data"}, confidence=0.7)
        )
    ...
    def test_merge_keeps_unrelated(self, tmp_path):
        ...
        consolidator.add_entry(KnowledgeEntry(content="Python is great", tags={"python"}))
        consolidator.add_entry(KnowledgeEntry(content="JavaScript is popular", tags={"javascript"}))
...
class TestArchiveOutdated:
    ...
    def test_archive_old_entries(self, tmp_path):
        ...
        consolidator = KnowledgeConsolidator(
            expert_name="test", storage_dir=tmp_path / "knowledge", archive_age_days=30
        )
    ...
    def test_archive_keeps_recent(self, tmp_path):
        ...
        consolidator = KnowledgeConsolidator(
            expert_name="test", storage_dir=tmp_path / "knowledge", archive_age_days=180
        )
...
class TestConsolidateAsync:
    ...
    async def test_consolidate_full_pipeline(self, tmp_path):
        ...
        consolidator = KnowledgeConsolidator(
            expert_name="test", storage_dir=tmp_path / "knowledge", similarity_threshold=0.85, archive_age_days=30
        )

        # Add various entries
        consolidator.add_entry(KnowledgeEntry(content="Python is great for data science", confidence=0.9))
        consolidator.add_entry(
            KnowledgeEntry(
    ...
    async def test_consolidate_saves_to_disk(self, tmp_path):
        ...
        storage_dir = tmp_path / "knowledge"
        consolidator = KnowledgeConsolidator(expert_name="test", storage_dir=storage_dir)
...
class TestStalenessPropertyTests:
    ...
    @given(age_days=st.integers(min_value=0, max_value=365), velocity=st.sampled_from(["slow", "medium", "fast"]))
    ...
    def test_staleness_threshold_consistency(self, age_days: int, velocity: str):
        ...
        if age_days > stale_threshold:
            ...
            assert is_stale is False, (
                f"Should be fresh/aging: {age_days} <= {stale_threshold} (0.8x of {velocity_days})"
            )
    ...
    def test_budget_enforcement_consistency(self, budget: float, spent: float, amount: float):
        ...
        profile = ExpertProfile(
            name="Test", vector_store_id="vs_test", monthly_learning_budget=budget, monthly_spending=spent
        )
...
class TestConsolidationPropertyTests:
    ...
    def test_similarity_threshold_respected(self, contents: list[str], threshold: float):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            consolidator = KnowledgeConsolidator(
                expert_name="test", storage_dir=Path(tmp_dir) / "knowledge", similarity_threshold=threshold
            )
    ...
    @given(
        age_days_list=st.lists(st.integers(min_value=0, max_value=365), min_size=0, max_size=10),
    ...
    def test_archive_threshold_respected(self, age_days_list: list[int], archive_threshold: int):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            consolidator = KnowledgeConsolidator(
                expert_name="test", storage_dir=Path(tmp_dir) / "knowledge", archive_age_days=archive_threshold
            )
    ...
    @given(text=st.text(min_size=5, max_size=100, alphabet=st.characters(whitelist_categories=("L", "N", "P", "Z"))))
    @settings(max_examples=100, suppress_health_check=[HealthCheck.too_slow, HealthCheck.function_scoped_fixture])
...
class TestConsolidationResultPropertyTests:
    ...
    @given(num_entries=st.integers(min_value=0, max_value=50))
