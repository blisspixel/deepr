...
def citation_strategy(draw):
    ...
    url = draw(st.one_of(
        st.none(),
        st.text(min_size=5, max_size=200).map(lambda x: f"https://example.com/{x}")
    ))
...
def raw_result_strategy(draw):
    ...
    citations = [
        Citation(index=i+1, source=f"Source {i+1}", url=f"https://example.com/{i}")
...
class TestCitationPreservation:
    ...
    def test_all_citations_preserved_in_output(self, result: RawResult):
        ...
        for citation in result.citations:
            assert f"[{citation.index}]" in formatted, \
                f"Citation [{citation.index}] missing from output"
