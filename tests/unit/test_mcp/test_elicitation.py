...
class TestElicitationRequest:
    ...
    def test_to_dict(self):
        ...
        request = ElicitationRequest(
            id="test_123", message="Test message", schema={"type": "object"}, timeout_seconds=120
        )
    ...
    def test_default_status_is_pending(self):
        ...
        request = ElicitationRequest(id="test", message="Test", schema={})
...
class TestElicitationHandler:
    ...
    def test_create_budget_elicitation(self, handler):
        ...
        request = handler.create_budget_elicitation(
            job_id="job_123", estimated_cost=2.50, budget_limit=1.00, current_model="o4-mini"
        )
    ...
    def test_create_budget_elicitation_schema(self, handler):
        ...
        request = handler.create_budget_elicitation(job_id="test", estimated_cost=5.00, budget_limit=2.00)

        schema = request.schema

        assert schema["type"] == "object"
        assert "decision" in schema["properties"]
        assert schema["properties"]["decision"]["enum"] == ["approve_override", "optimize_for_cost", "abort"]
    ...
    def test_create_custom_elicitation(self, handler):
        ...
        custom_schema = {"type": "object", "properties": {"choice": {"type": "string", "enum": ["a", "b", "c"]}}}

        request = handler.create_custom_elicitation(
            message="Choose an option", schema=custom_schema, request_id="custom_123"
        )
    ...
    def test_submit_response(self, handler):
        ...
        result = handler.submit_response(request.id, {"decision": "approve_override", "new_budget": 10.00})
    ...
    def test_parse_budget_decision_approve(self, handler):
        ...
        decision, new_budget = handler.parse_budget_decision({"decision": "approve_override", "new_budget": 15.00})
    ...
    def test_parse_budget_decision_optimize(self, handler):
        ...
        decision, new_budget = handler.parse_budget_decision({"decision": "optimize_for_cost"})
    ...
    def test_parse_budget_decision_abort(self, handler):
        ...
        decision, new_budget = handler.parse_budget_decision({"decision": "abort"})
    ...
    def test_parse_budget_decision_invalid_defaults_to_abort(self, handler):
        ...
        decision, _ = handler.parse_budget_decision({"decision": "invalid_value"})
...
class TestCostOptimizer:
    ...
    def test_suggest_cheaper_model_no_option(self, optimizer):
        ...
        cheaper = optimizer.suggest_cheaper_model(
            current_model="gemini-flash", target_budget=0.001, estimated_tokens=1_000_000
        )
    ...
    def test_calculate_optimized_config_model_switch(self, optimizer):
        ...
        config = optimizer.calculate_optimized_config(
            current_model="o4-mini", current_iterations=5, target_budget=1.00, estimated_tokens_per_iteration=200_000
        )
...
class TestPropertyBased:
    ...
    def test_budget_elicitation_contains_costs(self, estimated_cost: float, budget_limit: float):
        """
        ...
        request = handler.create_budget_elicitation(
            job_id="test", estimated_cost=estimated_cost, budget_limit=budget_limit
        )
    ...
    def test_optimizer_respects_budget(self, current_model: str, target_budget: float, tokens: int):
        """
    ...
    def test_custom_elicitation_preserves_message(self, message: str):
        ...
        request = handler.create_custom_elicitation(message=message, schema={"type": "object"})
    ...
    def test_timeout_preserved(self, timeout: int):
        ...
        request = handler.create_budget_elicitation(
            job_id="test", estimated_cost=5.0, budget_limit=2.0, timeout_seconds=timeout
        )
