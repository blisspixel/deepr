...
def test_payload_is_versioned_and_shaped():
    ...
    assert caps["zero_cost_synthesis"]["prepaid_plans"] == []
    assert caps["zero_cost_synthesis"]["executable"] == ["local"]
    assert caps["zero_cost_synthesis"]["blocked_plan_synthesis"]["status"] == "execution_blocked"
    assert "deepr_consult_experts" in caps["zero_cost_synthesis"]["how"]
    assert "plan='claude'" not in caps["zero_cost_synthesis"]["how"]
    assert "Plan and API synthesis are not executable" in caps["zero_cost_synthesis"]["how"]
