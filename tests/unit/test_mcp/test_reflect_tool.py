...
class TestSchemaAndAllowlist:
    def test_registered(self):
        tools = {tool.name: tool for tool in create_default_registry().all_tools()}
        assert "deepr_reflect" in tools
        assert "execution-blocked" in tools["deepr_reflect"].description
        assert "aggregate USD ceiling" in tools["deepr_reflect"].description
...
class TestReflectTool:
    ...
    async def test_missing_report(self, mock_server):
        ...
        assert out.get("error_code") == "metered_expert_mutation_accounting_unavailable"
        mock_idx.assert_not_called()

    @pytest.mark.asyncio
    async def test_legacy_feature_toggle_cannot_bypass_explicit_block(self, mock_server, monkeypatch):
