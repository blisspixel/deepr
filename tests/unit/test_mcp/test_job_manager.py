...
class TestJobManager:
    ...
    async def test_update_phase_nonexistent_returns_none(self, manager):
        ...
        result = await manager.update_phase(
            job_id="nonexistent",
            phase=JobPhase.EXECUTING
        )
    ...
    async def test_add_belief(self, manager):
        ...
        result = await manager.add_belief(
            job_id="test_123",
            belief="AI is transforming industries",
            confidence=0.85,
            source="research.com"
        )
...
class TestJobManagerSubscriptions:
    ...
    async def test_create_job_emits_status(self):
        ...
        await sub_manager.subscribe(
            "deepr://campaigns/test_123/status",
            callback
        )
    ...
    async def test_add_belief_emits_beliefs_update(self):
        ...
        await sub_manager.subscribe(
            "deepr://campaigns/test_123/beliefs",
            callback
        )
