...
class TestToolsCall:
    ...
    @pytest.mark.asyncio
    async def test_mcp_tool_call_cannot_use_local_spend_wallet(self, mock_server):
        ...
        assert resolve_spend_caps()["monthly"] == 0.5
        ...
        assert result["isError"] is False
        assert json.loads(result["content"][0]["text"])["monthly_cap"] == 5.0
