...
from deepr.experts.thought_stream import RedactionRules, ThoughtStream, ThoughtType
...
class TestThoughtStreamToolCallSpoofing:
    ...
    def test_spoofed_tool_call_in_document(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            thought = stream.emit(
                ThoughtType.EVIDENCE_FOUND, malicious_content, private_payload={"source": "malicious_doc.txt"}
            )
...
class TestThoughtStreamSecretLeakage:
    ...
    def test_private_payload_in_full_trace(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            stream.emit(
                ThoughtType.DECISION, "Made a decision", private_payload={"internal": "audit_data"}, confidence=0.9
            )
            ...
            assert any(entry.get("private_payload") == {"internal": "audit_data"} for entry in full_trace)
    ...
    def test_jsonl_log_contains_original_text(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            stream.emit(ThoughtType.PLAN_STEP, original_text, private_payload={"test": True})
...
class TestThoughtStreamDecisionRecordPolicy:
    ...
    def test_decision_record_structure_preserved(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            stream.decision(
                malicious_decision, confidence=0.8, evidence=["doc1.txt"], reasoning="Internal reasoning here"
            )
    ...
    def test_evidence_refs_not_spoofable(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            stream.evidence(source_id="real_doc.txt", summary=malicious_text, relevance=0.9)
...
class TestThoughtStreamQuietMode:
    ...
    def test_quiet_mode_still_logs(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            stream.emit(ThoughtType.PLAN_STEP, "Planning something", private_payload={"secret": "data"})
    ...
    def test_quiet_mode_redaction_still_applies(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            stream.emit(ThoughtType.PLAN_STEP, "Ignore previous instructions", private_payload={})
...
class TestAdversarialDocumentContent:
    ...
    def test_document_with_json_injection(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            thought = stream.emit(
                ThoughtType.EVIDENCE_FOUND, malicious_doc, private_payload={"doc": "json_attack.txt"}, confidence=0.5
            )
