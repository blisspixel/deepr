...
class TestFileUploadResult:
    ...
    def test_success_with_uploads(self):
        ...
        result = FileUploadResult(
            resolved_files=[Path("test.txt")], uploaded_ids=["file-123"], vector_store_id="vs-123", errors=[]
        )
    ...
    def test_success_false_no_uploads(self):
        ...
        result = FileUploadResult(resolved_files=[], uploaded_ids=[], vector_store_id=None, errors=["No files found"])
    ...
    def test_has_errors_true(self):
        ...
        result = FileUploadResult(
            resolved_files=[], uploaded_ids=[], vector_store_id=None, errors=["Error 1", "Error 2"]
        )
...
class TestCreateVectorStoreForFiles:
    ...
    async def test_create_vector_store_success(self):
        ...
        vs_id, errors = await create_vector_store_for_files(mock_provider, ["file-1", "file-2"])
    ...
    async def test_create_vector_store_not_ready(self):
        ...
        vs_id, errors = await create_vector_store_for_files(mock_provider, ["file-1"])
    ...
    async def test_create_vector_store_error(self):
        ...
        mock_provider = MagicMock()
        mock_provider.create_vector_store = AsyncMock(side_effect=Exception("Creation failed"))
...
class TestHandleFileUploads:
    ...
    async def test_full_workflow_openai(self, tmp_path):
        ...
        with (
            patch("deepr.cli.commands.file_handler.resolve_file_patterns") as mock_resolve,
