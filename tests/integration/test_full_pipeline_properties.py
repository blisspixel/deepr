...
from deepr.experts.memory import HierarchicalMemory, Episode, ReasoningStep, MemoryTier
from deepr.experts.lazy_graph_rag import LazyGraphRAG
from deepr.experts.reasoning_graph import ReasoningGraph, ReasoningState, ReasoningPhase, Hypothesis, Claim
...
expert_names = st.text(min_size=3, max_size=50, alphabet="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 -_")
queries = st.text(min_size=10, max_size=200, alphabet="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 ?.,!-")
responses = st.text(min_size=50, max_size=500, alphabet="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 .,!-")
doc_contents = st.text(min_size=100, max_size=1000, alphabet="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 .,!-\n")
...
class TestToTReasoningClaimVerification:
    ...
    def test_reasoning_phases_are_complete(self):
        ...
        expected_phases = [
            "UNDERSTAND", "DECOMPOSE", "RETRIEVE", "GENERATE_HYPOTHESES",
            "VERIFY_CLAIMS", "SYNTHESIZE", "SELF_CORRECT", "COMPLETE", "ERROR"
        ]
