...
[![Version](https://img.shields.io/badge/version-2.26.0-blue)](https://github.com/blisspixel/deepr/releases/tag/v2.26.0)
...
runs that same review path with an explicit local Ollama judge. The judge sees
the local trace answer at command time, but Deepr stores only validated scores,
labels, notes, and `$0` judge metadata in the review artifact.
