...
- Bug discoveries during research runs (8 critical bugs found through actual usage)
...
# o3-deep-research creates phased plan, executes with context chaining
```

**4. Dynamic Research Teams** - Think tank with diverse perspectives
```bash
deepr run team "Should we pivot to enterprise?"
# o3-deep-research assembles dream team, each researches independently, then synthesizes
```

The dream team approach:
- o3-deep-research designs optimal team for your specific question
...
- `gpt-5-mini` - Prompt refinement, reasoning tasks ($0.001 avg)

Note: These models use the Responses API, not the legacy Chat Completions API. GPT-5 (o3/o4-mini series) are deep research models with native tool calling, web search, and reasoning capabilities.
