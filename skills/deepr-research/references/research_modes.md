...
| Explicit eligible plan CLI | Selected expert sync, sync-all, gap-fill, absorb, learn, query/consult, and probes after safety checks | `$0` marginal cost; consumes proven subscription quota |
| API preview | Write-free CLI request envelope and OpenRouter metadata checks | `$0`; production paid dispatch remains blocked |
...
Does stored expert state already answer the question?
  ...
  NO  -> Is admitted local or eligible plan capacity available?
          YES -> Use the documented bounded expert workflow.
          NO  -> Report the missing capacity and offer a write-free preview.
                 A budget or approval cannot unblock metered execution.
```
...
## Existing-job async inspection

```text
1. Use an existing accepted job id; do not submit a blocked paid request.
...
- paid `deepr_research` execution and API council synthesis;
...
larger budget. Provider account-control and current credential-identity proofs
are required in addition to local reservation and settlement contracts. The
release ships no production verifier that can authorize this execution.
