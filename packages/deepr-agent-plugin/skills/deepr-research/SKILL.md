...
license: Apache-2.0
compatibility: Requires deepr-research 2.50.10 and the packaged local stdio MCP server.
metadata:
  deepr-version: "2.50.10"
...
The host may expose each tool as its raw MCP name or prefix it with the server
name, such as `deepr__deepr_status`. Use the exact advertised name. The
packaged server advertises only Deepr's policy-filtered read-only catalog;
unlisted write and paid tools remain unavailable even if their names are
guessed.
