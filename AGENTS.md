...
- Types (blocking CI gate): `mypy --strict --no-warn-unused-ignores --ignore-missing-imports src/deepr/core src/deepr/providers src/deepr/mcp src/deepr/security src/deepr/queue src/deepr/storage src/deepr/tools src/deepr/routing`. The rest of the tree is a non-blocking baseline - don't add new errors.
...
- Capacity sources must be described honestly:
  - Works now: API-backed research with user keys and budget gates; local Ollama expert setup and maintenance via `expert make --local`, `expert sync --local`, `expert sync --local --fresh-context`, `expert sync --local --deep-context`, `expert absorb --local`, `eval local`, `eval local-context`, and scored `capacity admit`. Plan-quota CLI execution via explicit `deepr expert sync --plan <id>` and `deepr capacity probe-plan <id>` (codex/claude/opencode/kiro/grok/antigravity/copilot), behind a deterministic auth-mode + no-surprise-bills gate, with quota + `$0` cost-ledger writes.
  - Visible/read-only or gated today: `deepr capacity` shows plan CLIs and quota sources. *Auto-routing* to a plan-quota CLI stays gated off until a live remaining-quota probe exists (vendors do not expose trustworthy remaining quota); explicit `--plan` is the works-now path. A CLI authenticated by an API key is refused as plan capacity. ToS-gray CLIs (kiro/grok/antigravity) and metered copilot are explicit-only and never auto-routed.
