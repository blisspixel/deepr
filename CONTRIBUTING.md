...
- **Types**: `core/`, `providers/`, `mcp/`, `security/`, `queue/`, `storage/`, `tools/`, `routing/`, `worker/`, `webhooks/`, `a2a/`, the importable `deepr.skills` package, `backends/`, `agents/`, `evals/`, and `observability/` are `mypy --strict`-clean and
