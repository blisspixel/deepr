...
- [ ] `mypy --strict --no-warn-unused-ignores --ignore-missing-imports src/deepr/core src/deepr/providers src/deepr/mcp src/deepr/security src/deepr/queue src/deepr/storage src/deepr/tools src/deepr/routing src/deepr/worker`
...
- **Types**: `core/`, `providers/`, `mcp/`, `security/`, `queue/`, `storage/`, `tools/`, `routing/`, and `worker/` are `mypy --strict`-clean and
