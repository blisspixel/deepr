...
MODEL_CAPABILITIES: dict[str, ModelCapability] = {
    ...
    "openai/gpt-5.4": ModelCapability(
        ...
        strengths=["Long-horizon execution", "1M+ context", "Configurable reasoning effort"],
    ...
    "openai/gpt-5.4-pro": ModelCapability(
        ...
        strengths=["High-precision GPT-5.4 variant", "Long-horizon reasoning", "1M+ context"],
        weaknesses=["Expensive compared with non-pro variants", "May take minutes on hard tasks"],
    ...
    "openai/gpt-5.4-mini": ModelCapability(
        ...
        strengths=["Budget GPT-5.4 reasoning", "Fast responses", "400K context"],
        weaknesses=["Pricier than gpt-5-mini", "Less capable than full gpt-5.4"],
    ...
    "openai/gpt-5.4-nano": ModelCapability(
        ...
        strengths=["Fast GPT-5.4 tier", "400K context", "Summarization and classification"],
        weaknesses=["Lowest GPT-5.4 reasoning tier", "Pricier than gpt-5-nano"],
        input_cost_per_1m=0.20,
    ...
    "openai/gpt-5.5": ModelCapability(
        ...
        strengths=["Strong coding and tool use", "1M+ context", "Configurable reasoning effort"],
        weaknesses=["Superseded by GPT-5.6 Sol at the same rates", "More expensive than GPT-5.4"],
        input_cost_per_1m=5.00,
        output_cost_per_1m=30.00,
        cached_input_cost_per_1m=0.50,
    ),
    "openai/gpt-5.5-pro": ModelCapability(
        ...
        context_window=1_050_000,
        specializations=["reasoning", "planning", "agentic", "synthesis", "hard_problems"],
        strengths=["High-precision GPT-5.5 variant", "Maximum reasoning depth", "1M+ context"],
        weaknesses=["Very expensive", "High latency", "Overkill for most workloads"],
        input_cost_per_1m=30.00,
        output_cost_per_1m=180.00,
        cached_input_cost_per_1m=3.00,
    ),
    ...
    # Grok 4.6 - current xAI frontier model (released August 12, 2026)
    "xai/grok-4-6": ModelCapability(
        provider="xai",
        model="grok-4-6",
        cost_per_query=0.10,
        latency_ms=2500,
        context_window=500_000,
        specializations=["reasoning", "coding", "agentic", "tool_calling", "knowledge_work"],
        strengths=["Current xAI frontier model", "Built for long-running agents and knowledge work"],
        weaknesses=["Smaller context than Grok 4.3 or 4.20", "Long-context rates apply at 200K prompt tokens"],
        input_cost_per_1m=2.00,
        output_cost_per_1m=6.00,
        cached_input_cost_per_1m=0.50,
    ),
    # Grok 4.5 - previous xAI flagship (released July 16, 2026)
    "xai/grok-4-5": ModelCapability(
        ...
        strengths=["Previous xAI flagship", "Coding, agentic, and engineering model"],
        weaknesses=[
            "Smaller context than Grok 4.20 and Grok 4.3 (500K vs 1M)",
            "Superseded by Grok 4.6 at the same standard token rates",
    ...
    "xai/grok-build-0-1": ModelCapability(
        provider="xai",
        model="grok-build-0.1",
        cost_per_query=0.05,
        latency_ms=1300,
        context_window=256_000,
        specializations=["coding", "reasoning", "agentic", "developer_workflows"],
        strengths=["Current xAI coding model", "Function calling and structured outputs"],
        weaknesses=["Coding-specific rather than general-purpose", "Long-context rates apply at 200K prompt tokens"],
        input_cost_per_1m=1.00,
        output_cost_per_1m=2.00,
        cached_input_cost_per_1m=0.20,
    ),
    "xai/grok-code-fast-1": ModelCapability(
        ...
        strengths=["Documented API alias for Grok Build 0.1"],
        weaknesses=["Prefer the canonical grok-build-0.1 ID for new requests"],
        input_cost_per_1m=1.00,
        output_cost_per_1m=2.00,
        cached_input_cost_per_1m=0.20,
        deprecated=True,
        successor="xai/grok-build-0-1",
    ),
    ...
    # For research, we recommend Opus 5 - current flagship with adaptive thinking.
    ...
    "anthropic/claude-fable-5": ModelCapability(
        ...
        strengths=["Long-horizon agentic reasoning", "Always-on adaptive thinking", "1M context and 128K output"],
        weaknesses=["2x Opus token price", "Long latency", "No native deep research API"],
        input_cost_per_1m=10.00,
        output_cost_per_1m=50.00,
        cached_input_cost_per_1m=1.00,
        max_output_tokens=128_000,
    ),
    # Claude Sonnet 5 - current balanced Claude. Its launch price became permanent.
    "anthropic/claude-sonnet-5": ModelCapability(
        ...
        strengths=["Balanced Anthropic default", "Adaptive thinking", "1M context", "Permanent $2/$10 rates"],
        weaknesses=["Below Opus or Fable on hardest tasks", "Rejects non-default sampling parameters"],
        input_cost_per_1m=2.00,
        output_cost_per_1m=10.00,
        cached_input_cost_per_1m=0.20,
        max_output_tokens=128_000,
    ...
    "anthropic/claude-opus-4-7": ModelCapability(
        ...
        weaknesses=[
            "Use claude-opus-4-8 instead (same price)",
            "No native deep research API (requires orchestration)",
    ...
    "anthropic/claude-opus-4-6": ModelCapability(
        ...
        weaknesses=[
            ...
            "Use claude-opus-4-7 instead (same price)",
        ],
        input_cost_per_1m=5.00,
        output_cost_per_1m=25.00,
        cached_input_cost_per_1m=0.50,
    ...
    "anthropic/claude-sonnet-4-5": ModelCapability(
        ...
        weaknesses=[
            ...
            "No native deep research API",
            "Use claude-sonnet-4-6 instead (same price)",
        ],
        input_cost_per_1m=3.00,
        output_cost_per_1m=15.00,
    ),
