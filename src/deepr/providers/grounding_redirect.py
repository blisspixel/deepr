...
async def resolve_grounding_redirect_url(url: str, timeout: float = 10.0) -> str:
    """Resolve one Google grounding redirect chain with per-hop IP pinning checks."""
    try:
    ...
    try:
        ...
        async with httpx.AsyncClient(follow_redirects=False, timeout=timeout, trust_env=False) as client:
            for redirect_count in range(MAX_GROUNDING_REDIRECT_HOPS + 1):
                try:
                    resolve_safe_url_ips(current_url)
                except SSRFError:
        ...
        return url
    except (httpx.TimeoutException, httpx.NetworkError, httpx.HTTPError, httpx.InvalidURL) as exc:
