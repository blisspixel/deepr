...
test('new and invalid profile routes open retained perspective', () => {
  for (const value of [null, '', 'unknown', 'Claims', '__proto__']) {
    assert.equal(resolveExpertProfileTab(value), 'perspective')
  }
})

test('explicit profile destinations preserve saved chat and evidence navigation', () => {
  for (const value of ['perspective', 'claims', 'gaps', 'decisions', 'history', 'skills', 'chat']) {
