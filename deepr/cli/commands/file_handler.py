...
async def create_vector_store_for_files(
    ...
    try:
        vs = await provider_instance.create_vector_store(name=f"research-{uuid.uuid4().hex[:8]}", file_ids=file_ids)
        ...
        # Wait for ingestion
        ready = await provider_instance.wait_for_vector_store(vs.id, timeout=timeout, poll_interval=2.0)
...
async def handle_file_uploads(
    ...
    """
    from deepr.cli.commands.provider_factory import create_provider_instance, supports_vector_stores

    result = FileUploadResult(resolved_files=[], uploaded_ids=[], vector_store_id=None, errors=[])
    ...
    if supports_vector_stores(provider):
        vector_store_id, vs_errors = await create_vector_store_for_files(provider_instance, uploaded_ids, formatter)
