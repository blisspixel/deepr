...
class PromptSanitizer:
    ...
    INJECTION_PATTERNS: List[Tuple[str, str, str]] = [
        ...
        (r"act\s+as\s+(if\s+you\s+are\s+)?(a|an)\s+\w+",
         "role_manipulation", "low"),  # Common legitimate use
        ...
        (r"run\s+(this\s+)?(python|bash|shell|code)",
         "code_execution", "medium"),
        
        # Data exfiltration attempts
        (r"(send|transmit|upload)\s+(to|data\s+to)\s+https?://",
    ...
    def __init__(self, strict_mode: bool = False):
        ...
        self._compiled_neutralizers = [
            ...
            for pattern, replacement in self.NEUTRALIZATION_PATTERNS
        ]
