...
class AutonomousLearner:
    ...
    def __init__(self, config):
        ...
        self.research = ResearchOrchestrator(
            provider=provider, storage=storage, document_manager=document_manager, report_generator=report_generator
        )
    ...
    async def execute_curriculum(
        ...
        try:
            ...
            self._log_progress("", "Step 1: Submitting Deep Research Jobs", "", callback=progress_callback)
            ...
            for phase_num, phase_topics in enumerate(phases, 1):
                ...
                if not can_proceed:
                    self._log_progress(f"Budget check failed: {reason}", callback=progress_callback)
                    break
                ...
                job_ids = await self._submit_research_jobs(expert, phase_topics, progress, session, progress_callback)
                if job_ids:
                    all_job_ids.extend(job_ids)

            self._log_progress(
                f"Submitted {len(all_job_ids)} research jobs (running in background)", callback=progress_callback
            )

            # STEP 2: While research runs, acquire sources (scraping)
            self._log_progress("", "Step 2: Acquiring Sources", "", callback=progress_callback)
            ...
            if summary and summary.get("alerts"):
                for alert in summary["alerts"]:
                    self._log_progress(f"Cost alert: {alert['message']}", callback=progress_callback)
    ...
    async def _submit_research_jobs(
        ...
        for topic in topics:
            ...
            if not can_proceed:
                self._log_progress(f"  Skipping {topic.title} - {reason}", callback=callback)
            ...
            if not allowed:
                ...
                if is_pausable_limit(block_reason):
                    ...
                    self._log_progress(f"  Blocked {topic.title} - {block_reason}", callback=callback)
                    progress.failed_topics.append(topic.title)
                    continue

            try:
                self._log_progress(f"  Submitting: {topic.title} (~${topic.estimated_cost:.2f})", callback=callback)
                ...
                self._log_progress(f"    Failed to submit: {e}", callback=callback)
    ...
    async def _acquire_sources(
        ...
        if not all_sources:
            self._log_progress(
                "\n[SKIP] No sources to acquire (curriculum has no source references)", callback=callback
            )
            return
        ...
        for source_type, sources in limited_by_type.items():
            ...
            if original_count > MAX_SOURCES_PER_TYPE:
                self._log_progress(
                    f"  {source_type}: {len(sources)} sources (limited from {original_count})", callback=callback
                )
            else:
                self._log_progress(f"  {source_type}: {len(sources)} sources", callback=callback)
        ...
        for i, source in enumerate(limited_sources, 1):
            try:
                ...
                if source.source_type in ["documentation", "guide", "blog"]:
                    ...
                    self._log_progress("  [SKIP] Video sources not yet supported", callback=callback)
                    continue
                else:
                ...
                if not content:
                    self._log_progress("  [SKIP] No content extracted", callback=callback)
                    failed += 1
                    continue
                ...
                try:
                    ...
                    await self.research.provider.client.vector_stores.files.create(
                        vector_store_id=expert.vector_store_id, file_id=file_id
                    )
    ...
    async def _scrape_source(self, source, callback: Optional[Callable] = None) -> Optional[str]:
        ...
        try:
            ...
            if results["success"]:
                self._log_progress(f"  [OK] Scraped {results['pages_scraped']} pages", callback=callback)
                ...
                self._log_progress(
                    f"  [WARN] Scraping failed: {results.get('error', 'Unknown error')}", callback=callback
                )
                return None
    ...
    async def _submit_single_job(
        ...
        try:
            ...
            if topic.research_mode == "campaign":
                ...
                # Focus mode: Quick research with GPT-4.1 (1-5 min, $0.04/query)
                # GPT-4.1 has 1M+ context, widely available, much cheaper than GPT-5
                model = "gpt-4.1"
            ...
            self._log_progress(f"    [ERROR] Submit failed: {e}", callback=callback)
            return None
    ...
    async def _execute_phase(
        ...
        for topic in topics:
            ...
            if progress.total_cost + topic.estimated_cost > budget_limit:
                self._log_progress(f"[SKIP] {topic.title} - would exceed budget", callback=callback)
                progress.failed_topics.append(topic.title)
                continue
            ...
            try:
                ...
                self._log_progress(f"  [DONE] Research ID: {response_id}", callback=callback)

            except Exception as e:
                self._log_progress(f"[ERROR] Failed to submit {topic.title}: {e}", callback=callback)
                progress.failed_topics.append(topic.title)
        ...
        self._log_progress(f"\nPhase complete: {len(job_mapping)} research topics processed", callback=callback)
    ...
    async def _poll_and_integrate_reports(
        ...
        while pending:
            ...
            if session.is_circuit_open:
                self._log_progress(
                    "Circuit breaker open - stopping polling due to repeated failures", callback=callback
                )
                break
    ...
    async def _integrate_reports(self, expert: ExpertProfile, job_ids: list[str], callback: Optional[Callable] = None):
        ...
        self._log_progress("=" * 70, "  Integrating Knowledge", "=" * 70, callback=callback)
        ...
        try:
            ...
            if config.expert.auto_synthesis and uploaded > 0:
                ...
                if result["success"]:
                    ...
                    worldview_doc = await synthesizer.generate_worldview_document(worldview, result["reflection"])
                    ...
                    self._log_progress(
                        "",
                        "  [OK] Expert consciousness formed",
    ...
    async def _simulate_phase_execution(
        ...
        for topic in topics:
            ...
            self._log_progress(
                f"[DRY RUN] {topic.title}", f"  Est. cost: ${topic.estimated_cost:.2f}", callback=callback
            )
    ...
    def _update_expert_profile(self, expert: ExpertProfile, progress: LearningProgress, job_mapping: dict):
