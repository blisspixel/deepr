...
class HypothesisSchema:
    ...
    def validate(cls, data: Dict[str, Any]) -> tuple[bool, Optional[str]]:
        ...
        for i, h in enumerate(hypotheses):
            ...
            for field in cls.HYPOTHESIS_FIELDS:
                if field not in h:
                    return False, f"hypothesis[{i}] missing field: {field}"
...
class ClaimSchema:
    ...
    def validate(cls, data: Dict[str, Any]) -> tuple[bool, Optional[str]]:
        ...
        for i, c in enumerate(claims):
            ...
            for field in cls.CLAIM_FIELDS:
                if field not in c:
                    return False, f"claim[{i}] missing field: {field}"
...
class ReasoningGraph:
    ...
    async def _decompose_query(self, state: ReasoningState) -> ReasoningState:
        ...
        self._emit_thought(
            ...
            f"Decomposed into {len(state.sub_questions)} sub-questions"
        )
    ...
    async def _retrieve_context(self, state: ReasoningState) -> ReasoningState:
        ...
        if state.context:
            self._emit_thought(
                ...
                f"Using {len(state.context)} pre-loaded context documents"
            )
    ...
    async def _generate_hypotheses(self, state: ReasoningState) -> ReasoningState:
        ...
        if is_ambiguous:
            ...
            self._emit_thought(
                ThoughtType.PLAN_STEP, 
                f"Ambiguous query detected, generating {num_hypotheses} hypotheses"
            )
        else:
            num_hypotheses = 1
        ...
        if self.llm_client:
            hypotheses_data = await self._generate_hypotheses_with_llm(
                state.query, 
                state.context, 
                num_hypotheses
            )
            ...
            if hypotheses_data:
                ...
                self._emit_thought(
                    ThoughtType.ERROR, 
                    "Hypothesis generation failed, using fallback"
                )
        ...
        state.add_trace(
            "generate_hypotheses", 
            "hypotheses_created", 
            {"count": len(state.hypotheses), "degraded": state.is_degraded}
        )
        
        self._emit_thought(
            ThoughtType.PLAN_STEP, 
            f"Generated {len(state.hypotheses)} hypotheses"
        )
...
Generate exactly {num_hypotheses} hypotheses with confidence scores between 0 and 1."""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not is_valid:
                    self._emit_thought(
                        ThoughtType.ERROR, 
                        f"Schema validation failed: {error} (attempt {attempt+1}/{max_retries})"
                    )
                    continue
                ...
                self._emit_thought(
                    ThoughtType.ERROR, 
                    f"LLM call failed: {str(e)} (attempt {attempt+1}/{max_retries})"
                )
    ...
    async def _verify_claims(self, state: ReasoningState) -> ReasoningState:
        ...
        self._emit_thought(
            ThoughtType.PLAN_STEP, 
            f"Extracted {len(state.claims)} atomic claims"
        )
        ...
        if contradictions:
            self._emit_thought(
                ThoughtType.ERROR, 
                f"Detected {len(contradictions)} contradictions, triggering self-correction"
            )
            state.phase = ReasoningPhase.SELF_CORRECT
        else:
            self._emit_thought(
                ThoughtType.EVIDENCE_FOUND, 
                f"Verified {len(state.verified_claims)}/{len(state.claims)} claims"
            )
    ...
    def _verify_claim_against_context(
        self, 
        claim: Claim, 
    ...
    def _detect_contradictions(
        ...
        antonym_pairs = [
            ...
            ('better', 'worse'), ('good', 'bad'), ('high', 'low'),
            ('more', 'less'), ('always', 'never'), ('all', 'none'),
            ('true', 'false'), ('yes', 'no'), ('positive', 'negative'),
            ('success', 'failure'), ('win', 'lose'), ('fast', 'slow'),
        ]
        ...
        for i, claim1 in enumerate(claims):
            for j, claim2 in enumerate(claims[i+1:], i+1):
                ...
                if has_negation1 != has_negation2:
                    ...
                    if overlap > 0.4:  # 40% word overlap
                        contradictions.append({
                            ...
                            "description": f"Negation contradiction between claims",
                            "confidence": overlap
                        })
                        continue
                ...
                for ant1, ant2 in antonym_pairs:
                    if (ant1 in text1_lower and ant2 in text2_lower) or \
                       (ant2 in text1_lower and ant1 in text2_lower):
