    ...
    def __post_init__(self):
        if not self.id:
            content_hash = hashlib.sha256(
                f"{self.document_id}:{self.start_pos}:{self.content[:100]}".encode()
            ).hexdigest()[:12]
...
class ConceptExtractor:
    ...
    def __init__(self, min_phrase_length: int = 2, max_phrase_length: int = 5, min_frequency: int = 1):
    ...
    def extract_sections(self, text: str, document_id: str) -> List[DocumentSection]:
        ...
        if not headings:
            ...
            sections.append(
                DocumentSection(
                    document_id=document_id, content=text, heading="", level=0, start_pos=0, end_pos=len(text)
                )
            )
            return sections
...
class EdgeBuilder:
        ...
        for section in sections:
            ...
            for i, c1 in enumerate(concepts_in_section):
                for c2 in concepts_in_section[i + 1 :]:
                    edge = self._create_cooccurrence_edge(c1, c2, section, document_id)
                    if edge:
                        key = edge.id
                        if key in edges:
            ...
            if section.heading:
                heading_concept = self._find_concept_by_text(concepts, section.heading.lower())
                if heading_concept:
    ...
    def calculate_pmi(self, c1_count: int, c2_count: int, cooccur_count: int, total_docs: int) -> float:
...
class SufficiencyScorer:
    ...
    def _calculate_coverage(self, query: str, chunks: List[Dict[str, Any]]) -> float:
        ...
        query_words = set(
            w.lower() for w in re.findall(r"\b[a-zA-Z]+\b", query) if w.lower() not in self.stopwords and len(w) > 2
        )
    ...
    def _calculate_redundancy(self, chunks: List[Dict[str, Any]]) -> float:
        ...
        for i, c1 in enumerate(chunks):
            for c2 in chunks[i + 1 :]:
                similarity = self._jaccard_similarity(c1.get("content", ""), c2.get("content", ""))
...
class SubgraphCache:
    ...
    def _evict_lru(self):
        ...
        # Find LRU entry
        lru_hash = min(self.cache.keys(), key=lambda h: self.cache[h].last_accessed)
...
class KnowledgeGraph:
    ...
    def search(self, query: str, top_k: int = 10) -> List[Concept]:
        ...
        """
        query_words = set(w.lower() for w in re.findall(r"\b[a-zA-Z]+\b", query) if len(w) > 2)
...
class LazyGraphRAG:
    ...
    def __init__(
        ...
        self.graph = KnowledgeGraph(expert_name=expert_name, storage_dir=storage_dir / "graph")

        self.cache = SubgraphCache(max_size=cache_size, storage_path=storage_dir / "cache" / "subgraph_cache.json")
    ...
    async def retrieve(
        ...
        if not matching_concepts:
            return {"chunks": [], "from_cache": False, "sufficiency": RetrievalSufficiency(), "concepts": []}
        ...
        if use_graph:
            visited = self.graph.traverse(start_ids=concept_ids, max_depth=2, max_nodes=top_k * 3)
        ...
        if expand_if_insufficient and not sufficiency.is_sufficient(self.sufficiency_threshold):
            ...
            visited = self.graph.traverse(start_ids=concept_ids, max_depth=3, max_nodes=top_k * 5)
            concept_ids = list(visited)
        ...
        self.cache.put(query=query, node_ids=set(concept_ids), prompt_blocks=prompt_blocks)
