...
class BeliefStore:
    ...
    ) -> Tuple[Belief, Optional[BeliefChange]]:
    ...
    def get_contradictions(self, belief_id: str) -> List[Belief]:
        ...
        return [
            self.beliefs[cid]
            for cid in belief.contradictions_with
    ...
    def _load(self):
        ...
        for cdata in data.get("changes", []):
            self.changes.append(BeliefChange(
...
class SharedBeliefStore:
    ...
    def share_belief(
        ...
        if existing:
            ...
            new_confidence = (
                existing.confidence * num_contributors + belief.confidence
            ) / (num_contributors + 1)
    ...
    def import_to_expert(
        self,
        expert_store: BeliefStore,
        domain: str,
        max_beliefs: int = 10
    ) -> int:
    ...
    def cleanup_stale(self) -> int:
        ...
        for domain, beliefs in list(self.domain_stores.items()):
            ...
            stale_ids = [
                bid for bid, belief in beliefs.items()
                if belief.is_stale(threshold)
            ]
    ...
    def _save(self):
        ...
        data = {
            ...
            "contributors": {
                bid: list(experts)
                for bid, experts in self.contributors.items()
            }
        }
    ...
    def _load(self):
        ...
        self.contributors = {
            bid: set(experts)
            for bid, experts in data.get("contributors", {}).items()
        }
