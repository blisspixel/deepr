...
class MetaCognitionTracker:
            ...
            try:
                ...
                for topic, gap_data in data.get("knowledge_gaps", {}).items():
                    self.knowledge_gaps[topic] = KnowledgeGap(
                        ...
                        research_date=datetime.fromisoformat(gap_data["research_date"]) if gap_data.get("research_date") else None,
    ...
    def get_knowledge_gaps(self, min_times_asked: int = 1) -> List[KnowledgeGap]:
        ...
        return [
            ...
            if gap.times_asked >= min_times_asked
        ]
    ...
    def get_high_confidence_domains(self, min_confidence: float = 0.7) -> List[DomainConfidence]:
        ...
        """
        return [
            conf for conf in self.domain_confidence.values()
    ...
    def get_low_confidence_domains(self, max_confidence: float = 0.3) -> List[DomainConfidence]:
        ...
        return [
            conf for conf in self.domain_confidence.values()
            if conf.confidence <= max_confidence
        ]
    ...
    def suggest_proactive_research(self, threshold_times_asked: int = 3) -> List[str]:
        ...
        """
        return [
            gap.topic for gap in self.knowledge_gaps.values()
    ...
    def get_learning_stats(self) -> Dict:
        ...
        avg_confidence = sum(conf.confidence for conf in self.domain_confidence.values()) / len(self.domain_confidence) if self.domain_confidence else 0.0
