...
class MCPResourceHandler:
    ...
    def read_resource(self, uri: str) -> ResourceResponse:
        ...
        return ResourceResponse(uri=uri, data=None, error=f"Unknown resource type: {parsed.resource_type}")

    def _read_campaign_resource(self, job_id: str, subresource: str, uri: str) -> ResourceResponse:
        ...
        return ResourceResponse(uri=uri, data=None, error=f"Unknown campaign subresource: {subresource}")
    ...
    def _read_report_resource(
        ...
        if subresource == "final.md":
            ...
            return ResourceResponse(uri=uri, data=None, error=f"Report not found for job: {job_id}")
            ...
            if state:
                return ResourceResponse(uri=uri, data=state.to_dict())
            return ResourceResponse(uri=uri, data=None, error=f"Summary not found for job: {job_id}")

        return ResourceResponse(uri=uri, data=None, error=f"Unknown report subresource: {subresource}")
    ...
    def _read_log_resource(
        ...
        if not candidates:
            return ResourceResponse(uri=uri, data=None, error=f"Unknown log subresource: {subresource}")
        ...
        for name in candidates:
            ...
            if path.exists():
                try:
                    ...
                    return ResourceResponse(uri=uri, data={"content": raw, "format": "markdown"})
                except Exception as e:
                    return ResourceResponse(uri=uri, data=None, error=str(e))
