...
class ElicitationRequest:
    ...
    def to_jsonrpc(self) -> dict:
        ...
        return {
            ...
            "params": {"id": self.id, "message": self.message, "requestedSchema": self.schema},
            "id": self.id,
        }
...
class ElicitationHandler:
    ...
    BUDGET_DECISION_SCHEMA = {
        ...
        "properties": {
            ...
            "new_budget": {"type": "number", "minimum": 0, "description": "New budget limit if approving override"},
    ...
    def create_budget_elicitation(
        ...
        request = ElicitationRequest(
            id=request_id, message=message, schema=self.BUDGET_DECISION_SCHEMA, timeout_seconds=timeout_seconds
        )
    ...
    def create_custom_elicitation(
        ...
        request = ElicitationRequest(id=request_id, message=message, schema=schema, timeout_seconds=timeout_seconds)
...
class CostOptimizer:
    ...
    def calculate_optimized_config(
        ...
        cheaper_model = self.suggest_cheaper_model(current_model, target_budget, total_tokens)
        ...
        if cheaper_model:
            return {
                ...
                "estimated_cost": self._estimate_cost(cheaper_model, total_tokens),
            }

        # If no cheaper model works, reduce iterations
        current_cost_per_million = self.MODEL_COSTS.get(current_model, 1.0)
        cost_per_iteration = (estimated_tokens_per_iteration / 1_000_000) * current_cost_per_million
        ...
        return {"model": current_model, "max_iterations": 1, "strategy": "minimum", "estimated_cost": 0.0}
