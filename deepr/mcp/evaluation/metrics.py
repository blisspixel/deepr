...
class TrajectoryMetrics:
    ...
    def passes_targets(
        ...
        if self.efficiency < efficiency_target:
            failures.append(
                f"Efficiency {self.efficiency:.2%} < target {efficiency_target:.2%}"
            )
        
        if self.citation_accuracy < citation_target:
            failures.append(
                f"Citation accuracy {self.citation_accuracy:.2%} < target {citation_target:.2%}"
            )
        
        if self.hallucination_rate > hallucination_target:
            failures.append(
                f"Hallucination rate {self.hallucination_rate:.2%} > target {hallucination_target:.2%}"
            )
        
        if self.tokens_per_task > tokens_target:
            failures.append(
                f"Tokens per task {self.tokens_per_task:.0f} > target {tokens_target}"
            )
...
class MetricsTracker:
    ...
    def calculate_metrics(self) -> TrajectoryMetrics:
        ...
        metrics.efficiency = calculate_efficiency(
            self.steps, self.golden_path
        )
        ...
        metrics.tasks_completed = sum(
            ...
            if s.step_type == StepType.TOOL_CALL and s.success
        )
        metrics.tokens_per_task = calculate_context_economy(
