...
| Consult-quality calibrated judge | **Agent meaning, workflow envelope** | An explicit local Ollama or explicit plan-quota judge scores consult answer quality against the published rubric using the local trace answer at command time; deterministic code validates every score, allowed failure label, review decision, `$0` Deepr cost posture, quota-consuming plan metadata, no metered fallback, promotion gates, and storage boundaries so raw trace answers and raw judge responses do not become durable review artifacts |
