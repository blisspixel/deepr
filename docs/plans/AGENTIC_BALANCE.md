...
| Consult-quality local judge | **Agent meaning, workflow envelope** | An explicit local Ollama judge scores consult answer quality against the published rubric using the local trace answer at command time; deterministic code validates every score, allowed failure label, review decision, `$0` cost posture, no metered fallback, promotion gates, and storage boundaries so raw trace answers and raw judge responses do not become durable review artifacts |
| Hallucination-pattern risk signals | **Workflow router, agent verdict** | Deterministic code may preserve provenance, date fields, context-position metadata, schema-valid risk labels, and review-routing status, but it must not decide that a claim is false, unsupported, sycophantic, or hallucinated from lexical rules. Pattern labels inform prompt variants, retrieval strategy, regression selection, and human or calibrated-model review; they do not block answers or write beliefs by themselves |
