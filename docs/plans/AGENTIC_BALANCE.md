...
| Surface | Setting | Why |
|---|---|---|
| Initial local expert formation and knowledge views | **Workflow envelope, model interpretation** | CLI and web share bounded search, guarded source retention, independent study lenses, briefing and derived Markdown/graph publication. Code enforces owned local capacity, attempted-call accounting, per-expert overlap refusal, input identities, unchanged-state checks, authored-file protection and finite work. Retry offers completed lenses only for an unchanged model/corpus. Model judgment proposes research queries and forms explanations and positions; hashes, matched quotations and graph connectivity do not certify their truth or practical usefulness. Formation ends at research completion with qualification unreviewed. Current-source preparation and full temporal revision remain S1/S2. |
| Consultation evidence delivery | **Workflow envelope, model advice** | Complete question and contributor identities reserve bounded prompt space; whole reasoning/evidence blocks are selected with explicit omissions. Literal passage lookup normalizes whitespace/case to locate original bytes, not to judge entailment. The actual messages and prompt hash are saved. Local models retain normal reasoning behavior under finite output/time limits. The model determines the answer, tradeoffs and relevance; transport success is not a quality verdict. |
| Planned consultation preparation (S1) | **Workflow envelope, agent research judgment** | A shared CLI/MCP service enforces operation identity, capacity, source/context/call/time limits, retained-source provenance, frozen snapshots, cancellation, and proposal-only learning. Model judgment chooses what needs checking, relevance, implications, and when further inquiry is useful within those bounds. Freshness-policy metadata routes reuse; timestamps, hashes, or token overlap cannot certify semantic currentness. Read-only inspection does not research. Canonical revision stays behind the existing reviewed admission boundary. This is planned behavior under [S1](living-expertise.md#s1-prepare-before-consultation-with-bounded-temporary-evidence), not the shipped one-shot call shape. |
...
| OpenRouter model catalog and execution gate | **Workflow form and spend authority, agent model judgment** | Exact gateway model slugs, dated price-class caps, cache-write sources, preview-only status, disabled tools, routing and eval exclusion, and pre-reservation execution refusal are deterministic controls. The bounded public proof requires one standard endpoint metadata record under actual base-tag matching semantics; bounds prompt, completion, cache-read, cache-write, reasoning, fixed-request, discount, parameter, context, and status evidence; digests a no-fallback routing proposal; disables response caching; requests router metadata; and forbids paid request features. Preview estimates reserve full ordinary input plus a full-input cache write. The current-key check uses a hidden prompt by default; explicit `--from-env` uses a quarantined process copy or parses only the bounded checkout-local key without exporting it. `deepr keys check --provider openrouter` uses the stored or quarantined key. It validates numeric limit, remaining headroom, BYOK posture, usage arithmetic, expiry, response digest, and credential fingerprint. `deepr budget authorize openrouter` may bind that observation as OpenRouter-only hard-stop evidence. Attended `deepr research --provider openrouter` may run one pinned no-tool completion after wallet credits and that hard stop. MCP, schedules, and automatic fallback stay denied. None of these checks claims that one model is semantically best or that a response is good. A future unattended adapter must still prove clean account defaults and no applicable BYOK, bind composite route and response identity, adopt one parent reservation, freeze ambiguous outcomes, settle mandatory provider total cost, and reconcile final billing. Model capability, answer quality, and task fit remain calibrated model or human judgment. Design: [openrouter-metered-gateway.md](../design/openrouter-metered-gateway.md) |
...
| Plan-quota CLI execution backends | **Workflow gate over agent work** | All production adapters are currently execution-blocked. Claude safe mode preserves managed-policy hooks that can execute independently of empty model tools; the adapter refuses before quota/account probes and runner construction. Authentication, disabled paid overage, trusted remaining quota, and an empty tool/MCP catalog are necessary but insufficient without complete process confinement. Dormant transport tests retain environment sanitization, pinned Sonnet, bounded subprocess lifetime/output, typed quota outcomes, and paired `$0` cost records through synthetic adapters. No semantic verdict changes. Restoration requires exact effective-policy or independent process-confinement evidence. Design: [claude-managed-policy-containment.md](../design/claude-managed-policy-containment.md). |
