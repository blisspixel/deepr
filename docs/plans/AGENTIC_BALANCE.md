...
| Scoped remote MCP keys and audit review | **Workflow** | Authentication, mode/expert scope, confirmation gates, per-key budget ceilings, per-key rate limits, argument hashing, response-cost attribution, append-only remote-call audit events, and local audit-log filters guard tool access, spend, and observability; they never judge the semantic quality of an expert answer |
