...
| Plan-quota CLI execution backends (codex/claude/opencode/...) | **Workflow gate over agent work** | Child auth-mode control (known metered API-key env vars are removed before plan launch), fail-closed rejection of metered-at-margin adapters without complete estimate/reserve/settle/ledger support, error-channel exhaustion handling, OS-backed relative/absolute reset form parsing, typed attempt outcomes with unknown quota usage preserved, redacted tail diagnostics, quota+`$0` cost-ledger writes for every non-metered dispatch, argv/tool lockdown, stdin for long Codex prompts, and the observed-quota auto-routing gate are deterministic no-surprise-bills guards; the CLI's answer and extracted beliefs stay model judgment, passing the same confidence/trust-floor/contradiction/dedup absorb gates as any source. Design: [plan-quota-cli-backends.md](../design/plan-quota-cli-backends.md) |
| Expert consult and council synthesis | **Workflow envelope, agent synthesis** | Expert selection, stored-belief context selection, provider/model routing, Anthropic Messages API request shape, provider completion-reason parsing, output-limit status, and the `deepr-expert-collaboration-v1` roster/role/trace/budget/evidence/dissent contract are bounded context assembly and protocol metadata, not truth verdicts. Local and explicit plan-quota synthesis are owned/prepaid capacity modes and disable live metered expert fallback when stored context is missing. Local council requests disable separate Ollama thinking so the bounded output allowance remains available for a visible answer; this request-shape rule never reaches plan or metered providers. An empty visible answer remains a typed failure or truncation, and private reasoning is never promoted as the answer. The model still owns synthesis meaning and whether perspectives agree; deterministic code owns expert limits, response-shape instructions, budget reservation, cache-bucket cost settlement, ledger writes, context metadata, collaboration metadata, and no-surprise-bills behavior. |
| Expert chat backend seam | **Workflow envelope, agent response** | Backend request shape, feature flags, provider routing, unsupported-parameter omission, usage extraction, cost settlement, disabled hidden SDK retries, and no-fallback behavior are deterministic workflow controls. Browser Socket.IO chat additionally requires an explicit API backend, finite positive bounded session budget, metered acknowledgement, valid mode, one in-flight turn, durable turn holds attributed to the validated routed provider/model, bounded and reserved auxiliary calls, event-loop cancellation, typed terminal failure state, conservative ambiguous-cost settlement labeled separately from reported actual usage, and deterministic session cleanup; transport connection state never authorizes spend. Long-running modes remain attached without an artificial short timeout and expose explicit cancellation. The expert answer, tool-use intent, synthesis, uncertainty, and semantic usefulness remain model judgment bounded by budget and tool-policy gates. Design: [expert-chat-capacity-backends.md](../design/expert-chat-capacity-backends.md) |
...
| Consult-quality calibrated judge | **Agent meaning, workflow envelope** | An explicit local Ollama, explicit plan-quota, or explicit budgeted API judge scores consult answer quality against the published rubric using the local trace answer at command time; deterministic code validates every score, allowed failure label, review decision, cost posture, plan quota metadata, API estimate, budget, confirmation, ledger settlement, promotion gates, and storage boundaries so raw trace answers and raw judge responses do not become durable review artifacts |
