...
| Scoped remote MCP keys and audit | **Workflow** | Authentication, mode/expert scope, confirmation gates, per-key budget ceilings, argument hashing, response-cost attribution, and append-only remote-call audit events guard tool access, spend, and observability; they never judge the semantic quality of an expert answer |
