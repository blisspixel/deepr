...
| Level 5/6 expert self-improvement | **Workflow control plane, agent self-model and proposals** | Experts may mine traces, emit metacognitive monitor proposals, update bounded self-model fields after review, propose gaps, propose research, and propose prompt/tool/skill changes, but workflow code owns rollout stage, budget, sandbox, schema, provenance, regression checks, permission boundaries, and human-review gates. Self-model changes describe capabilities, limits, goals, calibration, and learning strategy; they do not grant new authority. A proposal can improve the system only after measured evidence shows it helped. Design: [level-5-6-expert-maturity.md](../design/level-5-6-expert-maturity.md) |
