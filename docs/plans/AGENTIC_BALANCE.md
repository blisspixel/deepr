...
| Untrusted ingested/tool content boundaries | **Workflow envelope, agent meaning** | Web, scraped, report, and tool-result text is delimited and sanitized before prompt use so embedded directives stay source data; extraction, grounding, contradiction, and dedup still rely on calibrated model judgment |
