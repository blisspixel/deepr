...
- Published JSON Schemas under `docs/schemas/` for handoff, expert self-models,
  loop status, OKF profile mapping, A2A task envelopes, remote audit events, MCP
  registration manifests, capacity guidance, sync capacity gates, scheduled
  maintenance payloads, and the shared CLI operation-result envelope.
