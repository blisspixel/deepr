...
| ToT reasoning grounding + contradiction (`reasoning_graph._verify_claim_against_context`, `_detect_contradictions`) | lexical: 30% keyword-overlap "verified" verdict; negation-word + hardcoded antonym-pair contradiction with `confidence = word_overlap` | **Audit finding (2026-06-24).** Semantic verdicts done lexically - the exact HANS/antonym-list anti-pattern, and self-admitted in a code comment ("Would use semantic similarity in production"). Mitigating factor: it runs in chat Tree-of-Thoughts and feeds `state.synthesis` (the **ephemeral answer**), not the belief store - persisted beliefs only ever come through the verified absorb pipeline, so this cannot poison the trust store directly. Fix: the ToT graph already runs a model per node, so route the verify/contradiction step to that model (or stop asserting "verified"/contradiction from keyword overlap); never let these reach beliefs except via verified absorb. Tracked in ROADMAP. |
