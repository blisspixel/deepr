...
- [~] **Expert maintenance on plan quota** (the compounding payoff): `expert sync --plan <id>` runs the whole sync (research + verified extraction) on prepaid capacity today via the shared chat-client. Remaining: scheduled/`health-check`/gap-fill wiring and *automatic* default routing (gated on the live remaining-quota probe below), so a roster stays current in the background with metered APIs reserved for interactive work.
- [~] **Auto-mode integration**: the waterfall selector ranks `local -> plan_quota -> api_metered` and the plan rung is wired, but auto-selection requires an observed remaining-quota window the CLIs do not expose, so it stays gated off by default (explicit `--plan` is the works-now path). Remaining: a live remaining-quota probe to light up auto-routing, and `--dry-run`/`--preview` showing "plan quota (resets HH:MM)".
