...
5. [~] Capacity QOL completion: `deepr capacity next` ranked actions, latest-artifact hints, and first concrete job previews are in place (`--expert`, `--report-id`, `--context-mode`, `--scheduled`), including wait guidance for fresh/deep scheduled sync jobs that should not fall through to metered API. `deepr expert sync --scheduled` now consumes that guidance for due subscription syncs and returns a wait payload instead of spending when owned/prepaid capacity is blocked. Remaining work is reusing the same scheduler contract across the other recurring expert maintenance loops.
