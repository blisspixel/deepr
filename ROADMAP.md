...
**Current main (v2.16 capacity track):** the evidence layer is available (`deepr eval continuity` + the calibration harness `eval calibrate`, design: [calibration-and-trust.md](docs/design/calibration-and-trust.md)); **$0 local-model execution** (Ollama backend; `expert sync`/`absorb --local`, plus `expert sync --local --fresh-context` for free-only retrieval context), **local comparison** (`deepr eval local` with a local Ollama judge or explicit CLI judge), **eval-artifact admission** (`deepr capacity admit --from-eval latest`), and **capacity visibility** (`deepr capacity`) are wired toward routing on owned/prepaid capacity before metered API, including the normalized `ResearchBackend` profile, append-only `quota_ledger.jsonl` substrate, pure backend eligibility for observed plan-quota state, and pure backend selection with measured quality floors (design: [capacity-waterfall.md](docs/design/capacity-waterfall.md); fresh local context: [local-fresh-context.md](docs/design/local-fresh-context.md)); **portable experts** - one data dir (`DEEPR_DATA_DIR`) relocates experts + research to a synced folder so they follow you across machines ([ADR 0004](docs/decisions/0004-one-experts-root-and-portable-data-dir.md)); routing **quality priors** keep auto mode useful without paid evals; and guided setup (`deepr init`) is stable. Remaining capacity work is feeding admitted local scores into runtime quality-floor selection, plan-quota adapters, live window/credit probes, adapter writes, scheduler integration, and auto-mode runtime integration. Next after capacity is a verified expert-loop and OKF interchange release: make the existing loops observable, resumable, and portable without making OKF the source of truth. The cross-cutting principle for what deepr hardcodes vs lets the model decide (workflow vs agent, determinism on side-effects not meaning) is set in [AGENTIC_BALANCE.md](docs/plans/AGENTIC_BALANCE.md); the boundary for which checks specifically are deterministic vs model-based is its instance in [checks-deterministic-vs-agentic.md](docs/design/checks-deterministic-vs-agentic.md).
...
- **Local-model execution + capacity** (v2.16, in progress): `deepr capacity` (+ `--probe`) shows owned/prepaid capacity (local Ollama, plan CLIs, metered APIs) and summarizes locally observed plan-quota state from `quota_ledger.jsonl`; a local Ollama backend runs research at $0 via the injectable seams (`expert sync`/`absorb --local`), with `expert sync --local --fresh-context` adding a free-only retrieval pack before the local model call. `deepr eval local` compares local Ollama models with a local judge or an explicitly approved CLI judge (`--judge-cli grok` / `--judge-command`) so admission decisions have review evidence before automation. The first waterfall rung is wired: eval-gated local **admission** (`deepr capacity admit`/`admissions`/`revoke`, including `--from-eval latest`) makes `expert sync`/`absorb` drain an admitted local model at $0 before any metered API call, with `--local`/`--api` overrides. The normalized `ResearchBackend` profile, backend eligibility gate, and backend selector are in place for routing, logging, quota decisions, reserve floors, overage blocking, measured quality floors, and no-observation stops. Still to come: feeding admitted local scores into runtime quality-floor selection, plan-quota CLI adapters, live quota probes, adapter writes, scheduler integration, and auto-mode runtime integration. Design: [capacity-waterfall.md](docs/design/capacity-waterfall.md); [local-fresh-context.md](docs/design/local-fresh-context.md)
...
- [~] **Eval-gated local admission** (free does not outrank quality): a local model is *not routable* until the user has reviewed local benchmark evidence. `deepr eval local` now compares Ollama models with either a local judge or an explicitly approved CLI judge on the agentic-loop prompt set, and saved artifacts can feed admission through `deepr capacity admit --from-eval latest`. Remaining work is feeding admitted scores directly into runtime quality-floor selection. Admission is per model+version - swapping the local model invalidates its eval and drops it from routing until re-benchmarked. Cheap local judges cost $0 at the margin; CLI judges may consume external quota and are opt-in only.
- [~] **Capacity waterfall in auto mode**: the pure selector now orders `local -> plan_quota/credit_pool -> api_metered`, applies eligibility, and enforces measured per-task quality floors; remaining work is feeding it live benchmark scores, scheduler context, and adapter results. Metered is reached only when no free-at-margin source can meet the floor, and is still budget-checked as today.
...
- [ ] **Capacity quality-of-life path**: make the cheapest safe path obvious without hiding gates. Next increments: `deepr capacity next` with a ranked action list, admission hints that point at the latest usable eval artifact, dry-run previews that explain why each rung is blocked, and guided scheduler suggestions for jobs that should wait for local or plan capacity instead of paying now.
...
`deepr eval local` with local or explicit CLI judges, saved-artifact local
admission, routing quality priors, portable
experts/research via one data dir
...
1. [~] Backend abstraction + quota ledger + eligibility/selection gates + `deepr capacity` visibility (visibility, `ResearchBackend`, quota ledger, eligibility decisions, and backend selection are in place; live vendor probes and adapter writes remain)
...
3. [~] `local-ollama` + `--local` wiring are in place; local sync now supports `--fresh-context` with free-only retrieval, and saved local eval artifacts now feed admission; plan-quota CLI adapter work remains
4. [~] Capacity-waterfall routing with quality gates; local rung, local/CLI comparison, fresh-context local sync, eval-artifact admission, eligibility gate, and pure selector are in place, remaining runtime score ingestion, adapters, and scheduler integration still to connect
5. [ ] Capacity QOL completion: ranked next actions, latest-artifact hints, block-reason previews, and scheduler suggestions that make the safe cheap path feel obvious
