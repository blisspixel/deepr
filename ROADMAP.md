...
The fleet-autopilot track (Phase 4d) is largely closed; the active edge is verification quality and proof that the expert roster stays current through Deepr's own cheapest-capacity path. This order is dependency-based, not a time estimate:

1. **Release and repo hygiene** - keep `main` as the single source of truth, publish the matching GitHub release after CI passes, and close stale dependency branches only after their intended updates are present on `main`. Why: users and downstream agents need the package version, README badge, changelog, tag, and default branch to agree before they trust any handoff contract.
2. **Dogfood expert refresh and consultation** - run `deepr capacity`, refresh the project-relevant experts with `$0` local/fresh-context or explicit plan-quota capacity, then consult them on README and ROADMAP. Why: the roadmap should be informed by Deepr's own maintained expert state, and the refresh path is one of the core product promises.
3. **Trusted plan-quota availability probes** - Codex session-log `rate_limits` and Claude Code OAuth usage now write through the `QuotaSnapshot` contract; next wire Grok and Antigravity metadata probes for explicit-only visibility. Why: automatic plan routing must be unlocked by observed remaining capacity, not by CLI presence or wishful free-capacity assumptions.
4. **Provider prompt-cache cost model** - research Anthropic, OpenAI/Azure, Gemini, and xAI prompt/context caching as metered-API cost controls, then add estimator and actual-usage accounting before enabling any provider-specific cache controls. Do not add automatic pre-warming, keep-warm loops, or 1-hour cache TTLs unless the user opts in under an explicit budget ceiling. Why: Deepr repeats long stable expert/system/source context, so caching can reduce spend, but cache writes, longer TTLs, and pre-warm calls can increase spend if enabled blindly.
5. **Replayable evidence before wider escalation** - make source-pack evidence content-addressed and re-verifiable: raw snapshot reference, URL, timestamp, content hash, extractor model id/version, prompt version, and memoized claim+source+window verification results. Why: bounded maker-checker escalation needs stable evidence roots or it can re-check stale synthesis and create cost storms.
6. **Maker-checker bounded escalation** - finish metered provider-adapter checker construction behind spend-policy gates, then add a second different-vendor checker only for refuted, unsupported, or high-risk claims before holding them. Why: `grounding_assurance` is now visible in handoffs; the next value is using it to prevent weak claims from becoming trusted knowledge.
7. **Local-vs-frontier A/B** - build or refresh the same expert from the same sources through local and frontier capacity, then compare grounding, calibration, coverage, and cost. Why: automatic routing should promote `$0` local models only when they meet a measured quality floor, not because they are cheap.
8. **Fleet cost and concurrency finishers** - ship conditional GET for known sources, then wrap any remaining scheduled mutating verbs in the per-(expert, verb) overlap guard. Why: this keeps refresh loops cheap and idempotent before broader scheduling or daemon work.
9. **Remaining semantic correctness gaps** - route the remaining lexical verdict surfaces to calibrated model judgment and add local-first semantic belief recall. Why: the hardest remaining quality failures are missed paraphrases and brittle meaning checks, not more UI or orchestration surface.
...
- **MCP server**: Functional with 29 tools, but MCP spec itself is still maturing
...
- MCP server with 29 tools, persistence, security, multi-runtime configs
...
- [ ] **Provider prompt-cache economics**: research Anthropic, OpenAI/Azure, Gemini, and xAI caching surfaces as API-metered cost controls, not prepaid capacity. Ship only after the cost estimator and actual usage ingestion account for cache creation tokens, cache-read tokens, TTL, provider cache keys, and any pre-warm behavior. Anthropic is the sharpest gate: 5-minute cache writes cost more than base input, 1-hour writes cost more again, cache reads are cheaper, and pre-warm requests can still incur a cache-write charge. OpenAI and Azure automatic cached-token reporting, Gemini implicit/explicit context caching, and xAI automatic caching plus conversation/cache keys need provider-specific handling. No automatic pre-warming, keep-warm jobs, or longer TTL default until the expected-cost model proves savings and the user budget explicitly allows it.
...
- [x] **Auth-mode detection** (`safety.detect_auth_mode` / `evaluate_plan_quota_safety`): if a backend's metered-env var is set (`OPENAI_API_KEY`/`CODEX_API_KEY`, `ANTHROPIC_API_KEY`, `XAI_API_KEY`, ...), the next call would bill that key, so the gate refuses to classify it as `plan_quota` and tells the operator to unset it or use `--api`. Deterministic and tested.
- [~] **No-surprise-bills invariants** (kernel-enforced, tested):
  ...
  - [~] Overage handling: copilot is `metered_at_margin` (off by default, ack-gated); kiro's note records overage-off-by-default; per-vendor overage *probing* on a cadence remains. A backend whose mode reads metered (env key present) is refused as plan capacity today.
...
- [x] **Expert maintenance on plan quota** (the compounding payoff): `--plan <id>` runs the whole job (research + verified extraction) on prepaid capacity across all three research-bearing maintenance loop-closers - `expert sync`, `expert absorb`, and `route-gaps --execute` - via the shared chat-client. With `deepr capacity admit-plan`, scheduled/auto maintenance routes to the admitted plan backend by default; metered APIs stay reserved for interactive/high-priority work. (`health-check`/`reflect` are read/eval surfaces, not research execution.)
...
- [x] Web-grounded local research (`research_web_local` + `deepr expert learn-web`): a local model searches the live web (free DuckDuckGo) + scrapes pages + synthesizes a cited report, absorbed at $0. Fixes the "local research answers from stale parametric knowledge" gap so experts reflect the latest, not the training cutoff.
...
- [ ] Local fleet daemon / tray app with quiet-hours: run background enrichment (learn-web, sync, gap-fill on local models) only in configurable windows (e.g. 18:00-05:00) or when the GPU is idle, so unattended $0 maintenance never contends with the user's foreground GPU work. Pairs with capacity admission + loop runs already in place.
