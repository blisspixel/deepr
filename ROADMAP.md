...
2. [x] **Original-idea use surfaces** (shipped 2026-06-27) - persisted original ideas now feed derived expert memory cards, consult context, expert-council planning, and expert handoff payloads only as labeled `deepr-expert-perspective-state-v1` perspective state. Ideas that are not online yet stay usable when they have origin, rationale, uncertainty, expected observations, and disconfirming signals, but they are not forced through a false "web support or reject" rule or promoted as verified external facts. Why: an expert is a developing perspective, not a fact book; Deepr needs clean state for theories, taste, tradeoffs, stance, original synthesis, and exploration.
...
5. **Level 5 consult trace and semantic quality flywheel** - stored-belief perspectives, ledgered synthesis, `$0` consult evals, explicit local/plan synthesis, MCP-owned-capacity consult arguments, replayable `deepr-consult-trace-v1` records, sanitized `deepr-consult-trace-candidates-v1` review, `deepr-consult-quality-eval-case-v1` semantic review packets, and `deepr-consult-quality-review-v1` scored review artifacts are in place. `deepr expert review-consult-quality` records human or calibrated-model scores, blocks lexical verdicts, enforces reviewer and acceptance gates, and promotes only accepted cases into gap or eval artifacts without writing beliefs. `deepr expert consult-quality-trends` now emits a read-only `deepr-consult-quality-trend-v1` report over reviewed artifacts, including score trends and deterministic consult prompt regression candidates selected from reviewer scores and review status only. `deepr expert judge-consult-quality --local-judge-model` now runs the same review path with an explicit local Ollama judge, validates rubric scores and labels, and stores only the review artifact plus `$0` local judge metadata. `deepr expert judge-consult-quality --plan <id>` now runs the same calibrated review path on explicit plan-quota capacity, records quota-consuming `$0` Deepr metadata, and keeps live metered fallback disabled. `deepr expert judge-consult-quality --api-provider` now supports explicit premium metered API judges with required model, positive budget, estimate acknowledgement, preflight reservation, usage settlement, and no raw response storage. `deepr eval judge-calibration` now provides the first `$0` measurement of that flywheel's trust gate: it pairs a human review and a calibrated-model review of the same consult trace and reports per-dimension agreement (mean absolute error, signed bias, exact/within-tolerance rates) plus decision agreement as `deepr-judge-calibration-report-v1`, flagging insufficient data below five independently paired traces so a judge is measured before its scores are trusted. The report now also breaks agreement down per model-judge reviewer and marks each `trusted` only when it clears both the paired-trace floor and a within-tolerance agreement rate against the human anchor, with `trusted_model_reviewers` exposing that set. `deepr expert consult-quality-trends --gate-untrusted-judges` now consumes that set: it excludes calibrated-model reviews from judges that are not measured-trusted from prompt-regression candidate selection only (human reviews stay eligible, descriptive stats still cover every review), recorded in a `regression_gate` block, with the gate off by default. So an unproven model judge can no longer silently steer which consult prompts are selected for regression. Next broaden human-anchored review-pair coverage so more judges have enough evidence to be measured, before trusting any semantic label as a measured product metric. Keep the trace contract disciplined: include a small always-present context packet, keep larger belief/source/gap packets context-selected, validate generated trace and host-handoff artifacts against schema before they ship, and report which checks ran so a failed consult can become a durable regression case. Why: consult is Deepr's primary team-of-experts surface, and self-improvement only works when failures become durable test cases instead of one-off anecdotes. See [level-5-6-expert-maturity.md](docs/design/level-5-6-expert-maturity.md).
...
13. **Fleet cost, concurrency, and release hygiene** - automatic metered `route-gaps --execute` now records append-only value decisions per research gap and defers low-value paid fills before provider dispatch, while explicit `--api`, local, plan, scheduled wait, and dry-run paths stay unchanged. Legacy `expert fill-gaps` and the web fill-gaps endpoint now require explicit metered API acknowledgement before provider construction, and health-check/doc recommendations point to `route-gaps --execute --scheduled` as the local/plan-first path. Local and plan fresh-context sync now persists HTTP validators and sends conditional `If-None-Match` / `If-Modified-Since` requests for known sources, reusing `304 Not Modified` cached hashes for the existing no-change proof. Reflection follow-up execution now holds the per-expert `reflect` overlap guard before constructing gap-fill execution, so a colliding run records `overlap_locked` and exits without starting duplicate follow-up research. Next wrap any remaining scheduled mutating library wrappers in the per-(expert, verb) overlap guard, keep `main` as the single source of truth, publish the matching GitHub release after CI passes, and close stale branches only after their intended updates are present on `main`. Why: refresh loops must stay cheap and idempotent, and users and downstream agents need package version, README badge, changelog, tag, and default branch to agree before they trust any handoff contract.
14. [x] **Expert next-action navigator** (shipped 2026-07-10) - `deepr expert next NAME` emits `deepr-expert-next-v1`, a `$0` structural plan over claims, freshness, gaps, contradictions, and durable loop evidence. It returns argument-safe argv plans, checks capacity before suggesting scheduled compiled sync, and cannot execute work, change policy, or claim semantic maturity. Why: the expert surface already has the right controls; the missing quality-of-life layer was knowing which control matters now. Design: [expert-next-actions.md](docs/design/expert-next-actions.md).
...
- **Provider support**: OpenAI (GPT-5.5, GPT-5.5-pro, GPT-5.4 family, GPT-5-mini, GPT-4.1, o3/o4-mini-deep-research), Gemini (3.1 Pro Preview, 3.5 Flash, 3 Flash, 3.1 Flash-Lite, 2.5 Flash, Deep Research Agent), xAI Grok (4.20 flagship: Reasoning/Non-Reasoning/Multi-Agent; plus 4.3), Anthropic (Claude Fable 5, Sonnet 5, Opus 4.8/4.7/4.6, Sonnet 4.6/4.5, Haiku 4.5), Azure AI Foundry (o3-deep-research + Bing, GPT-5/5-mini, GPT-4.1/4.1-mini, GPT-4o)
...
- [ ] Live-validation findings (2026-07-11, local expert dogfood):
  ...
  - [ ] Interactive local consult, absorb, and sync need phase-level progress with elapsed time, current bounded phase, cancellation state, and a durable trace reference. Busy-before-dispatch waits are now defensive, but a healthy multi-minute local generation still appears silent between dispatch and completion.
  - [ ] Open a durable `running` consult trace before local or plan generation, append heartbeats and terminal state under the same id, and make cancellation ownership explicit. A post-fix one-expert qwen3.6 probe exceeded a five-minute host guard; the host timeout left its Windows child process tree alive until the exact verified PIDs were stopped, and Deepr had no trace because it records only after `run_consult` returns. The calling harness must own process-tree cancellation; Deepr must still leave interrupted-work evidence and a resumable typed stop.
...
- [x] Live-validation finding (2026-07-11, cross-platform release packaging): the deterministic frontend archive normalized timestamps and permissions but preserved Windows CRLF in generated text, inherited Python's platform-specific ZIP creator field, and relied on zlib output, so the Windows-built payload differed from the Linux CI rebuild. Fixed 2026-07-11 by normalizing generated text assets to LF, pinning Unix creator metadata, storing intermediate entries without compressor variance, and regression-testing equivalent CRLF and LF inputs plus the full metadata contract. Wheel and sdist transport remain compressed.
