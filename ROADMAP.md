...
Current baseline: `deepr capacity` visibility, `deepr capacity next`, local-only expert creation, the local Ollama execution path,
`deepr eval local` with local or explicit CLI judges, saved-artifact local
admission, runtime admitted-score quality gating, routing quality priors, portable
...
3. [~] `expert make --local`, `local-ollama`, and `--local` wiring are in place; local sync now supports `--fresh-context` with free-only retrieval, saved local eval artifacts now feed admission, and automatic local routing requires a measured admitted score; plan-quota CLI adapter work remains
4. [~] Capacity-waterfall routing with quality gates; local rung, local/CLI comparison, fresh-context local sync, eval-artifact admission, runtime admitted-score gating, eligibility gate, and pure selector are in place. Remaining work is adapters, live probes, auto-mode runtime integration, and scheduler integration.
5. [~] Capacity QOL completion: `deepr capacity next` ranked actions and latest-artifact hints are in place. Remaining work is concrete job dry-run block previews and scheduler suggestions that make the safe cheap path feel obvious.
