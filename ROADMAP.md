...
3. **Temporal knowledge graph completion and memory quality** - make temporal graph writes first-class in the compiler and replace default legacy absorb writes with explicit graph-commit apply once verified envelope coverage is sufficient. Compiler claim verification can now carry caller-supplied or store-backed recall context, and sync can run the budget-gated verifier, persist graph-commit sidecars, and apply verified envelopes through the explicit opt-in `--apply-compiled-claims` path, but recall stays subordinate to the graph: recall finds candidates; the belief graph and verifier decide. Why: the expert should answer "what changed," "why do you believe it," "what is your current take," "what is contested," "what are you watching," and "what would change your mind," which no chunk store can answer.
