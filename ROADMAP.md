...
3. **Research processing compiler and compiled wiki** - turn expert refresh from "files were added" into a bounded processing transaction: raw source pack -> source notes -> atomic beliefs -> typed temporal graph edges -> contradiction and gap agenda -> compiled wiki/digest view. Source-pack sync artifacts now also emit `deepr-source-pack-manifest-v1`, the first compiler stage: a deterministic, `$0`, no-model manifest of source provenance, content hashes, excerpt hashes, and readiness for later semantic compile. Next add source-note artifacts and a single commit envelope for claim extraction, keeping extraction, grounding, contradiction, semantic edge selection, and narrative synthesis in calibrated model judgment. Why: an expert becomes excellent by thinking over new material and updating its belief graph, not by accumulating unread documents. This is the Level 5 learning transaction under the agentic-balance boundary.
