...
| [Remote Expert Conversations](docs/design/remote-expert-conversations.md) | Protocol-neutral durable follow-up over MCP first, A2A after 1.0 conformance, with structured consulting and evaluation gates |
...
**Capability-adaptive principle:** the target is first-class operation on any OS with local Ollama (`$0`), verified non-metered plan capacity, or bounded cloud APIs. In v2.40, explicit local work and the safety-eligible Claude Code plan adapter are executable only after their zero-dollar or paid-overage-off proofs pass. Opaque CLI judges, arbitrary CLI tools, generic outbound MCP clients, metered APIs, and cloud provisioning are quarantined before dispatch. Metered request preview and accounting exist, but production paid dispatch remains blocked until a provider-specific authenticated account-control verifier and current credential-identity resolver are installed. Selected scheduled expert-maintenance paths can choose admitted local or trusted-quota plan capacity; global cheapest-first runtime routing and automatic metered fallback are not shipped. `deepr init` detects all three tiers and `deepr capacity` distinguishes executable, visible/read-only, blocked, and waiting capacity. Cost-efficiency is the default policy, but execution claims follow proven capability rather than CLI presence.
...
under spend, security, provenance, and verification gates. Research is the
input, source-verified factual state plus explicitly admitted non-factual
perspective state is the reusable product, and better
repeated decisions are the outcome. Graph size, report count, and apparent
self-confidence are not substitutes for held-out and downstream evidence.
...
The shipped one-shot consult is a smaller baseline: it independently selects
stored-state packets, makes no expert-generation calls, permits no peer turns,
and runs at most one synthesis. The experimental local investigation is the
question-specific path: independent research, bounded blinded discussion,
optional private revision, checking, synthesis, and separately staged factual
and perspective learning. Plan-quota, metered API, remote execution, and
automatic learning apply remain gated.
...
The Digital Consciousness expert must keep a theory-conditioned indicator and
credence matrix. It should show which observation matters under which theory,
separate capability evidence from phenomenal-consciousness claims, preserve
ethical uncertainty, and avoid a binary consciousness verdict. A model's
self-report or benchmark performance is evidence to interpret, not proof.
...
authority. Each expert can also expose a generated `EXPERT.md` memory card:
a compact orientation surface for humans and host agents, never the
authoritative store.

## Performance and Polyglot Evolution

The accepted design is [evidence-gated-polyglot-evolution.md](docs/design/evidence-gated-polyglot-evolution.md).
Deepr stays Python-first until a production-shaped benchmark proves that one
...
- [ ] **P0: forensically reconcile the historical orphaned spend before any
  ...
  expected non-report work, a lost artifact, or unresolved provider evidence;
  attach any available provider receipt and job identity; and make the doctor
  distinguish explicitly disposed non-report spend from unexplained spend.
...
- [ ] **P1: re-enable live metered expert chat only after one complete,
  ...
  an environment variable is not spend authority. The contract must cover
  serialized input and history, output, reasoning tokens, tools, cache writes,
  hosted storage, background jobs, retries, redirects, transport, and every
  fallback under one parent reservation. It must also bind one Deepr-owned SDK
  client, official endpoint, authenticated account and billing scope, exact
  model identity, disabled overage, and one-use request digest before dispatch.
  The provider's hard no-overage ceiling must be no higher than the operator
  ceiling, and its control state must be observed live for that one dispatch,
  not trusted from a cached authorization window. Provider callbacks, retained
  server context, and separately billed tool sessions stay disabled until they
  have their own complete envelopes.
  Missing or mismatched provider identity consumes the full hold and freezes
  paid work for reconciliation. Cancellation, concurrency, replay, and ledger
  failure require the same conservative settlement.
  - [x] **2026-07-29 correction:** removed the runtime and environment override.
    Metered completion, streaming, Grok tool research, paid fallback,
    background deep research, and deep-research retrieval now fail before
    provider construction or dispatch. The former output-only half-budget cap
    and average-cost holds were not hard total-cost bounds.
  - [x] **2026-07-29 correction:** generic or injected paid SDK clients,
    provider webhooks, retained response context, and Code Interpreter now fail
    before dispatch. Owned adapters bind an official endpoint and an exact
    canonical outbound model. Completion settlement uses canonical model,
    token-tier, cache, and tool evidence across web, worker, core, and MCP
    paths; ambiguous evidence consumes the full hold.
  - [x] Local and proven zero-dollar plan chat remain available and never enter
    the metered wrapper.
  - [ ] Build and independently verify the complete maximum-charge and client
...
- [ ] **P1: migrate every gated metered expert lifecycle surface to one shared
  ...
  `expert refresh` and `--synthesize`, `expert resume`, normal metered
  `expert reflect` and MCP `deepr_reflect`, API `fill-gaps` including consensus
  and deep modes, explicit API sync and sync-all, paid portraits, API
  consult-quality judging, live provider benchmarks, and paid
  `deepr eval calibrate --corpus`. Price storage and tool side effects before admission,
  bind every nested call to the parent run ceiling, durably mark dispatch,
  require canonical settlement or conservative full-bound fallback, and test
...
- [ ] **P1: re-enable metered multi-call research and hosted context only after
  ...
  durable parent reservation with exact nested-call settlement. Hosted file
  upload, file search, and vector stores need upload, indexing, retention,
  retrieval, and cleanup pricing in the same reservation. Automatic
  cross-provider fallback needs a separate reservation for every attempt plus
  approval of the full retry envelope. Until then these paths fail closed;
  bounded `$0` previews remain available, while production metered dispatch is
  frozen pending authenticated account authority and credential identity.
  - [x] Close exported-provider bypasses (2026-07-29). The provider base now
    owns a non-overridable public submission boundary. A one-use task-local
    authority, bound to the exact provider object and canonical deep copy of
    the request, is minted only after the durable reservation dispatch mark.
    Direct calls, reused authority, provider mismatch, or request drift fail
    before an adapter SDK call. Hosted upload and vector-store creation also
    fail at the provider boundary until their full lifecycle is priced and
    reserved. Cleanup, status, and cancellation remain available.
  - [x] Persist provider request authority and conservative completion
    settlement (2026-07-29). Each durable hold now owns an internal random
    binding plus exact provider, model, job, request digest, and ceiling before
    a grant can be minted; public queue metadata strips that capability.
    Canonical queued-model pricing and provable web-search and code-interpreter
    charges settle exact evidence. Missing or inconsistent model, token, tool,
    or settlement evidence consumes the held ceiling or freezes paid work.
    Missing, refunded, and non-settled rows cannot become terminal success.
  - [x] Require private zero-dollar capacity proof at model seams
    (2026-07-29). Local and plan-quota client factories mint process-local proof
    only after their deterministic admission succeeds. Caller labels, injected
    SDK clients, and numeric zero estimates no longer authorize claim,
    verification, judge, report-absorb, local-eval, local-research, or expert
    chat dispatch.
- [ ] **P1: migrate the remaining legacy direct metered interfaces.** `deepr
  check`, `deepr make docs`, `deepr make strategy`, `deepr agentic research`,
  and MCP provider sampling fallback must use the shared durable call
  transaction, bounded output, canonical settlement, and a parent ceiling when
  they can issue multiple calls. They fail before provider construction in
  v2.40.
  - [x] Disable the legacy multi-provider consensus engine (2026-07-29). Its
    one-to-three provider fan-out and separate agreement judge used rough
    planning estimates but had no durable holds, dispatch marks, output
    ceilings, or complete settlement. Public execution and every direct
    provider helper now fail before client construction until each call and the
    judge share one enforceable parent transaction.
- [x] Measure the current local scale and dominant paths (2026-07-11). The
  largest live belief store loads in about 1 ms, all live stores total about
  2 MB, and local council work takes hundreds of seconds. Current storage does
  not justify a rewrite. CLI cold start, blocking async retrieval, unbounded
  response buffering, semantic-vector loops, write amplification, and quota
  ledger serialization are the concrete Python-first targets.
- [ ] Add a versioned provider-free performance harness for CLI cold start,
  expert load and recall, graph-commit batches, ledger scale, mocked fresh
  retrieval, poll cycles, and concurrent MCP reads. Record p50, p95, p99, CPU,
  peak RSS, event-loop lag, workload size, Python build, OS, and hardware.
- [~] Close the measured Python gaps before extracting a language. Shipped
  2026-07-11: lazy CLI/provider/expert loading reduced version and root-help p95
  to about 223 ms and 217 ms; synchronous search/scraper calls moved off the
  event loop; source fetch and worker polling gained ordered bounded fanout,
  same-host serialization, and cancellation cleanup; HTTP/Wayback bodies gained
  decompressed-byte ceilings and redirect SSRF validation; and quota-ledger
...
- [ ] Consider Rust first for the deterministic ExpertEventV2 validation and
  replay engine after the Python reference and shadow replay exist. Meaning
  stays model or human owned. Require differential, fuzz, crash, Windows/Linux,
  packaging, fallback, and end-to-end performance evidence.
- [ ] Consider a separate Go hosted runner only after measured control-plane
  saturation remains following Python async fixes. Do not move expert meaning,
  budget authority, or verified memory mutation into it.
- [ ] Keep Mojo out of the production dependency graph until Deepr owns a
  measured tensor, GPU, or accelerator kernel that remains slow after NumPy and
  mature native-library comparison.
...
independent question-specific positions, exact require-all completion, one
owned-local synthesis, fixed resource envelopes, proven loopback Ollama cloud
disablement and GGUF materialization, and fsynced content-free `$0` attempt and
terminal markers. Paid research now persists an internal provider, model, job,
request, ceiling, and random binding before minting a one-use provider-object
grant at the non-overridable provider boundary. Canonical cost state now has a
durable identity, registry-prefix anchor, artifact high-water evidence, strict
JSON, and monotonic checkout caps. Ambiguous response or settlement evidence
consumes the reservation ceiling or freezes paid work, and local or plan-quota
model work requires private Deepr-minted zero-dollar proof. OpenAI 2.50.0,
Axios 1.19.0, and PostCSS 8.5.25 are current; TypeScript remains at the latest
release compatible with TypeScript ESLint.
...
one bounded blinded discussion round, independent checking, dissent-preserving
synthesis, durable lifecycle controls, and separately staged factual and
perspective learning. Bulk learning is provenance-bound, atomic across every
...
(``DEEPR_ALLOW_METERED_EXPERT_CHAT``), no double-ledger after durable settle,
deep-research final usage as overrun-only ledger rows, durable fixed-cost
skill-tool admission with fail-closed defaults, embedding dual-gate, soft cost
admission that fails closed on bookkeeping failure across citation/gap/
map-reduce/multi-pass/curriculum/synthesis/task-planner surfaces, and the
metered-chat re-enable review checklist. Live metered chat remains gated off.
The v2.40 audit removed that historical environment override entirely because
it could not prove a maximum total provider charge.
...
Those historical controls are disabled substrate in v2.40 and cannot authorize
provider work.

**v2.36.0 additions:** one-shot CLI, MCP, and A2A consults now allocate a shared
trace id and open an append-only lifecycle journal before cancellable backend
setup or dispatch. Cumulative elapsed, logical-work, spend, capacity,
cancellation, and failure state are durable under bounded cross-process locks;
storage and settlement work runs off the event loop and remains owned through
cancellation. Metered council synthesis requires exact pricing, conservative
affordability bounds, full reservation, canonical settlement, and zero hidden
SDK retries. Live metered perspective fallback remains gated when stored expert
context is unavailable. Every standalone metered `ExpertChatSession` dispatch
also fails closed in v2.36; local and explicit plan read-only query remains
available, and no metered chat live validation is claimed.
`deepr eval deliberation` adds eleven frozen `$0`
structural cases and two published schemas without enabling a live multi-round
surface or making semantic verdicts. Plan-quota quota and `$0` cost events are
idempotent and durable across cancellation, runner failure, torn history, and
fsync replay; subprocess launch and cleanup preserve path-safe partial status.
The v2.36 validation pass made no paid provider calls. Live deliberation remains
gated on provider-call token and context enforcement, replay, resume, and
capacity-wait acceptance.

The same v2.36 safety audit gates unsafe metered expert lifecycle entry points:
...
calibration. Each stays disabled until it uses the shared durable per-call and
parent-run budget transaction with storage and tool pricing. Local, scheduled,
dry-run, history-only, and explicit plan-quota paths where available, plus `$0`
`eval calibrate --from`, remain available.

Direct provider research now builds one finite request envelope for preview,
reservation, queue metadata, final provider payload, and settlement. Unknown
pricing, unsupported context or output bounds, unbounded provider tool loops,
hosted storage, automatic paid fallback, and metered multi-call research fail
before provider work. Direct bounded single-job research was available in that
v2.36 baseline; v2.40 freezes production paid dispatch pending authenticated
account, endpoint, credential, billing-scope, and provider hard-limit proof.
Legacy metered fact checking, artifact generation, agentic artifact research,
and MCP sampling fallback also fail before provider construction. The published
Recon handoff now carries evidence observations only and grants no belief-write
authority.

**v2.35.0 additions:** scheduled local maintenance now observes GPU contention
without a model call and records bounded 30-minute, 2-hour, then 6-hour wait
guidance without falling through to paid capacity. Expert learning and compiled
memory now retain replayable evidence, apply verifier-ready graph operations,
advance freshness only from accepted knowledge changes, and enforce aggregate
run budgets and overlap guards. Browser expert chat uses explicit metered
approval, exact provider/model reservations, cancellation, and honest cost
settlement. Queue dispatch restores persisted reservation authority before
provider work. Python distributions now carry the built dashboard and CI
verifies wheel contents. Consult traces preserve exact runtime capacity and
roster metadata; semantic council acceptance remains explicitly gated.
Measured Python-first hardening now cuts root CLI cold start from seconds to
about 223 ms p95, bounds and host-serializes retrieval fanout, streams scraper
bodies under typed size ceilings, serializes quota-ledger writers across
processes, and records explicit evidence gates before any Rust, Go, Mojo, or
free-threaded Python production dependency is considered.

**v2.34.4 additions:** `deepr expert next NAME` now emits the read-only
`deepr-expert-next-v1` navigator over claims, freshness, gaps,
contradictions, and durable learning-loop evidence, returning a bounded set of
argument-safe command argv without judging semantic maturity or changing policy.
Portable expert documentation and `deepr doctor` now state the actual safety
contract: generic synced folders support sequential device use only, with one
writer at a time and a completed sync before switching devices. The staged
multi-device design moves toward device-partitioned immutable event journals,
deterministic replay, and explicit semantic conflict review instead of unsafe
last-write-wins files.

**v2.34.3 additions:** public API, single-job web, and batch web callers can no longer supply or read server-owned provider cleanup identifiers, closing two same-credential cleanup authorization findings validated on 2026-07-10. Cancellation now resolves the provider recorded on the job, reports success only when job state, cost-reservation closure, and provider-resource cleanup are confirmed, repairs interrupted closure on retry, and uses an atomic active-state transition so concurrent terminal history is preserved. Provider terminal states converge across worker and web paths with explicit handling for `incomplete` and `cancelled`; unknown future status values remain active and observable instead of being assumed terminal. CLI completion refreshes now require canonical cost settlement, idempotent provider cleanup with durable cleanup evidence, result persistence, and terminal queue state in safety order. Fixed diagnostics keep provider-controlled content out of public responses and logs. The Research Live surface stops polling on cancellation and keeps unconfirmed cancellation visible for recovery.

**v2.34.2 additions:** provider validation no longer echoes unexpected response content into console or dashboard-captured output, treats an unexpected response as a failed validation with a nonzero command exit, and benchmark-start validation returns only a fixed safe error instead of serializing an exception. Focused regressions cover both CodeQL source-to-sink paths and the validation failure signal.

**v2.34.1 additions:** dashboard benchmark approval now binds the exact run options to an exact machine-readable cost estimate and enforces it as both the preflight and runtime ceiling. Every benchmark evaluation, judge, and provider-validation call reserves before submission, settles a conservative ceiling to the append-only ledger, and consumes the same finite run budget. Reservations cover adapter output maxima and bounded search allowances; unknown pricing or context metadata fails closed. Native managed research agents, Gemini 3 grounding, and xAI search tools are excluded from paid benchmark execution until they expose deterministic request-level spend bounds, while the research tier uses bounded web-search orchestration. Grok 4.3 chat evaluation remains available through a no-tool Responses request. Benchmark subprocesses resolve from any working directory, malformed estimates fail closed, unbounded paid runs are refused, and destructive demo mutations serialize. Expert, trace, model, and settings surfaces now distinguish initial, empty, stale, partial-failure, retrying, and recovery states without discarding valid cached evidence.

**v2.34.0 additions:** the experimental dashboard now fails closed on provider readiness and cost estimation, reconciles queue and ledger semantics, exposes reliable live-update state, restores validated session-scoped research drafts without persisting uploads, preserves saved drafts until linked prompts are explicitly accepted, and improves narrow-screen and assistive-technology navigation. Frontend draft-state tests are now a blocking CI gate.

**v2.33.0 additions:** semantic recall preference evidence now uses standard IR metrics, a 30-case operating floor, deterministic paired bootstrap confidence intervals, exact retrieval-contract binding, recomputation, and live model-specific index-state validation while preserving lexical-first defaults. Provider-backed research across REST, web, CLI, batch, MCP, and internal orchestration now shares durable cross-process maximum-cost admission, atomic dispatch/cancellation claims, conservative ambiguous-outcome settlement, and provider-aware no-replay behavior. Direct CLI admission precedes provider file preparation, compensating and completion cleanup closes created resources, and team/campaign execution preserves configured queue paths, selected models, provider polling, and durable tracking without synthesizing pending phases. Paid document review, research planning, team architecture, campaign context-summary/review, semantic context-index, and expert-validation calls reserve the configured ceiling before execution and settle provider usage; automatic pre-confirmation context discovery is keyword-only.

Multi-provider research automation with expert system, domain-specific skills, MCP integration, native first-party instruments (Recon + Distillr + Primr; Phase 2b complete), and observability. 8000+ unit tests, 80% branch coverage enforced on Python 3.12/3.13/3.14 (all blocking). Toolchain managed by `uv` (`uv.lock` committed); pre-commit hooks with ruff; strict mypy islands, dependency audit (`pip-audit`), SBOM generation, and a blocking Gitleaks history scan are wired into CI (see [Phase E](#phase-e-engineering-standards-and-code-quality-elevation-foundational-continuous)).


**Historical v2.36.2 baseline:** the evidence layer is available (`deepr eval continuity` + the calibration harness `eval calibrate`, design: [calibration-and-trust.md](docs/design/calibration-and-trust.md)); **agentic red-team metrics** now include `deepr eval red-team`, a local `$0` attack-success-rate verifier for prompt-boundary, MCP handoff and loop-status read-path, tool-spoofing, and trust-floor probes, plus `--save` trend artifacts under the configured benchmarks directory; **$0 local-model execution** (local-only `expert make --local`, Ollama backend, `expert sync`/`absorb --local`, plus `expert sync --local --fresh-context` and `--deep-context` for free-only retrieval context with optional SearXNG), **local comparison** (`deepr eval local` with a local Ollama judge; the then-exposed explicit CLI judge path is now quarantined), **local context evaluation** (`deepr eval local-context` comparing no/fresh/deep context), **source-pack sync artifacts** for context-bearing runs, **eval-artifact admission** (`deepr capacity admit --from-eval latest`), **runtime admitted-score quality gating**, **capacity next actions** (`deepr capacity next`, with `--probe` for a live `$0` local readiness check), and **capacity visibility** (`deepr capacity`, `capacity fleet`, `capacity probe-fleet`) are wired toward routing on owned/prepaid capacity before metered API, including the normalized `ResearchBackend` profile, append-only `quota_ledger.jsonl` substrate, pure backend eligibility for observed plan-quota state, pure backend selection with measured quality floors, and scheduled wait/action-plan guidance for recurring expert maintenance (design: [capacity-waterfall.md](docs/design/capacity-waterfall.md); fresh local context: [local-fresh-context.md](docs/design/local-fresh-context.md)); **durable expert loops** now record schema-versioned `ExpertLoopRun` state across scheduled waits, sync, gap-fill execution, reflection, and health-check runs, with CLI, MCP, and web rollups; **OKF interchange** exports derived bundles from structured state and re-imports through verified absorb; **expert memory cards** now expose `deepr-expert-memory-card-v1` and `deepr expert memory-card NAME --write`, a `$0` generated `EXPERT.md` orientation view over profile, manifest, belief events, and self-model state, including explicitly tagged theories and insights, self-research agenda, what would change the expert's mind, and agency scope; **hosted MCP foundation** includes HTTP/SSE serving, scoped keys, per-key budgets and rate limits, remote audit logs, smoke checks, registration manifests, deployment recipes, published schemas, and scheduler schemas for sync, gap-fill, reflection, and health-check maintenance payloads; **sequential portability** - `deepr init --data-dir` coordinates expert, report, and runtime roots for one-device-at-a-time use; stop Deepr services and complete file sync before switching devices ([ADR 0004](docs/decisions/0004-one-experts-root-and-portable-data-dir.md), [design](docs/design/multi-device-expert-continuity.md)); routing **quality priors** keep auto mode useful without paid evals; guided setup (`deepr init`) is stable; and expert handoff payloads now preserve grounding assurance per claim with verified and cross-vendor verified summary counts. The graph commit boundary now applies factual belief writes, verifier-supplied typed edges, verified gap promotions, verified exploration-agenda promotions, verified hypothesis promotions, verified concept promotions, verified stance promotions, and verified original-idea promotions through schema-versioned explicit apply operations while preserving `$0` no-model apply semantics. Claim-verification decisions can now carry read-only `candidate_only` recall context for memory-quality routing without changing readiness or writing the graph. Plan-quota CLI adapters now execute via the `research_fn`/chat-client seam (`deepr expert sync --plan <id>`, `deepr expert sync-all --plan <id>`, `deepr expert route-gaps --execute --plan <id>`, `deepr expert absorb --plan <id>`, topic `deepr expert learn --plan <id>` with `learn-web` retained as an explicit alias, `deepr capacity probe-plan <id>`, bounded concurrent `deepr capacity probe-fleet`, and bounded concurrent `deepr mcp validate-consult-fleet`) behind a deterministic auth-mode + no-surprise-bills gate; `deepr expert sync-all --scheduled` and scheduled `deepr expert route-gaps --execute` can consume admitted, quota-observed plan selections from the existing waterfall for roster-level sync and gap-fill maintenance, and non-dry `route-gaps --execute` now holds the per-expert `route-gaps` overlap lock plus deterministic startup jitter so overlapping scheduled fills skip cleanly before any engine or client construction. Legacy `deepr expert fill-gaps` and the web fill-gaps endpoint require explicit metered API acknowledgement before provider construction, while health checks and docs point ordinary gap filling to the local/plan-first scheduled route path. Codex and Claude are the clean auto-routable plan candidates once trusted headroom observations exist, while OpenCode is explicit-only until Deepr can verify the routed provider is OAuth/subscription or local. Kiro/grok/antigravity are explicit-only. Copilot remains visible/read-only and execution-blocked pending deterministic estimation, reservation, usage settlement, and canonical cost-ledger support. `deepr capacity refresh-quota codex`, `deepr capacity refresh-quota claude`, and `deepr capacity refresh-quota grok` record trusted metadata quota windows without model calls (design: [plan-quota-cli-backends.md](docs/design/plan-quota-cli-backends.md)). Live validation on 2026-06-30 confirmed Codex, Claude, Grok, and Antigravity plan probes work on this workstation at `$0` tracked daily spend; Grok and Antigravity remain explicit-only because their policy and automation risk did not change. Remaining capacity work is Grok heavy-consult reliability diagnostics, Antigravity live window/credit probes, remaining scheduler dispatch beyond sync-all and gap-fill, and auto-mode runtime integration. Remaining reach work is live third-party host registration and broader hosted-operational validation. Remaining red-team work is broader adaptive MCP extraction probing, expert-chat harness coverage, and ingestion-path corpora. The cross-cutting principle for what deepr hardcodes vs lets the model decide (workflow vs agent, determinism on side-effects not meaning) is set in [AGENTIC_BALANCE.md](docs/plans/AGENTIC_BALANCE.md); the boundary for which checks specifically are deterministic vs model-based is its instance in [checks-deterministic-vs-agentic.md](docs/design/checks-deterministic-vs-agentic.md).
**Current v2.40 quarantine correction:** the historical baseline above records
surfaces that once executed, not current authority. All external CLI judge
requests now fail before process creation. Deepr's outbound `smoke-http`,
URL-based `validate-consult`, and both managed and URL-based
`validate-conversation` modes fail before network, server, socket, or model
executor construction. The inbound loopback HTTP server and local container
remain executable. AWS, Azure, GCP, and Cloudflare deployment files are
mechanically inert reference templates, not supported hosted surfaces.

**Perspective-label correction (2026-07-18):** the legacy phrase "verified
hypothesis/concept/stance/original-idea promotion" in the current-main history
means that a model verifier admitted the typed operation through the commit
contract. It does not mean externally verified truth, verified novelty, or
human review. New surfaces use `model_assessed_well_formed`,
`operator_confirmed_apply`, and `human_reviewed: false` so these different
claims cannot be collapsed.

**Plan-quota safety correction (2026-07-18):** transport compatibility and a
zero-dollar Deepr ledger event do not prove that a CLI is safe for untrusted
research prompts or that the vendor will not bill. Claude Code is the only
current safety-eligible adapter. Every dispatch requires a fresh provider
observation that paid extra usage is explicitly disabled, written durably before
the vendor process starts. Claude runs pinned Sonnet in safe mode with explicit
empty tool and MCP surfaces and disabled session persistence and slash commands. Missing,
malformed, enabled, or unwritable overage state fails closed. Antigravity is now
execution-blocked because its native tools and
transcript side effects cannot be confined for an untrusted prompt. Codex,
OpenCode, Kiro, Grok, Antigravity, and Copilot remain fleet-visible but fail
before vendor dispatch because native-tool confinement, stored provider/auth
provenance, prepaid overage posture, or full metered accounting is unproven.
...
capacity. Those paths now fail closed pending the P1 shared durable transaction.
Local and safety-eligible explicit plan-quota expert paths remain available. Core metered
research and separately bounded API council synthesis are unaffected.

**Health-check continuity correction (2026-07-11):** ordinary manual and scheduled health audits remain read-only and do not create `ExpertLoopRun` state. Durable health-check loop records apply only to explicit `--archive-stale` mutation, confirmation-wait, overlap, and completion paths. Scheduled audit plans use `deepr-health-check-action-plan-v2` without `loop_run`.

**v2.32.0 additions:** public README screenshots now use isolated safe demo cost data and future screenshot QA refuses over-limit cost captures unless explicitly overridden. The blocking strict mypy gate now covers security, queue, storage, tools, routing, worker, webhooks, A2A, and the importable `deepr.skills` package in addition to the original strict islands, while the roadmap explicitly tracks the remaining bundled skill tool-script typing boundary. The A2A Agent Card property tests now assert the current built-in `deepr_consult_experts` skill contract, and the official MCP `2026-07-28` release candidate is tracked as a future protocol migration watch item.

**Cycle 24 capacity validation update (2026-06-30, historical transport evidence):** `deepr capacity validate-fleet` bundles selected plan CLI transport probes with no-metered consult-contract validation and emits `deepr-plan-fleet-validation-v1`. The workstation run showed Codex, Claude, Grok, and Antigravity transports returning at `$0` Deepr spend; this did not prove safe native-tool posture or vendor billing treatment. The 2026-07-18 plan-quota safety correction above controls current execution.

**Copilot cost-safety correction (2026-07-11):** Copilot is visible/read-only
capacity metadata, not an explicit execution backend. Its per-token billing
cannot be truthfully represented by the non-metered plan adapter's `$0` ledger
event. Probe, sync, and absorb paths now reject it before probe or client
construction, including JSON and `-y`, until deterministic estimation, durable
reservation, usage settlement, and canonical cost-ledger support exist.

**Cycle 25 sync-all capacity update (2026-06-30):** `deepr expert sync-all` now supports `--plan <id>` for one explicit non-metered plan backend across a whole roster pass, and scheduled automatic roster sync consumes the existing waterfall's admitted, trusted-quota-observed plan selection instead of waiting or falling through to metered API. Metered-at-margin CLI backends remain rejected for roster plan dispatch. The earlier `--api` roster path is execution-gated in v2.36 until the shared parent reservation and exact child settlement contract exists. The follow-on scheduler work moved to gap-fill dispatch, broader scheduled surfaces, and auto-mode runtime integration.

**Cycle 26 scheduled gap-fill capacity update (2026-06-30):** `deepr capacity admit-plan <backend> --task-class gap_fill` now admits non-metered plan capacity for gap-fill maintenance, and `deepr expert route-gaps NAME --execute --scheduled` consumes the existing waterfall's admitted, trusted-quota-observed `gap_fill` plan selection before it runs. If that selector does not return local or non-metered plan capacity, scheduled gap-fill still waits instead of spending on metered research. Remaining capacity work is Grok heavy-consult reliability diagnostics, Antigravity live window/credit probes, scheduler dispatch beyond sync-all and gap-fill, and auto-mode runtime integration.
...
**Fleet autopilot (Phase 4d, current main):** the roster now self-maintains end to end. `deepr fleet status` is a read-only `$0` roster-health watchdog (non-zero exit when any latest run failed or durable fleet state is unreadable); `deepr expert sync-all` syncs every due expert in one capacity-aware pass (owned/prepaid capacity first, admitted quota-observed plan dispatch when available, per-expert budgets within a ceiling, skip-not-fail, overlap-locked); and `deepr fleet install-schedule` emits the correct host scheduler recipe for Windows Task Scheduler, cron, or systemd. The configured off-box heartbeat is now configuration-visible but delivery-blocked because Deepr cannot prove the external service's billing source or marginal cost. Each pass is made safe and cheap by a content-hash pre-sync change-detection gate, a per-(expert, verb) overlap guard plus deterministic startup jitter, budget degradation tiers plus a value-of-spend gate, and a TTL sweep for leaked budget reservations. Knowledge quality gains a **cross-vendor maker-checker** only through an admitted local or safety-eligible plan path.
...
**v2.30.0 additions:** the verification spine's promise is now falsifiable. `deepr eval grounding-correctness` is a `$0` (local Ollama default) eval that runs the grounding checker over a curated golden set of human-labeled `(claim, evidence, label)` entailment triples and reports whether a SUPPORTED verdict is actually correct - `support_precision`, `false_support_rate` (the dangerous over-claim), recall, abstention, per-label accuracy - as `deepr-grounding-correctness-v1`, with a `--set baseline|hard|all` selector (the `hard` set adds lexical traps, unit/number contradictions, shared-entity distractors, and `partial` conjunctive-claim cases). Live baseline+hard result on `qwen2.5-coder:32b`: 100% support precision, 0% false-support across 50 cases - the verification is measurably real, not theater (a bounded curated set, not proof at scale). Separately, `deepr route explain "<query>"` is a `$0`, no-model inspect-before-spend command that shows which experts a consult would fan out to (deterministic keyword-overlap selection router, never a quality verdict) and the non-probing capacity outlook, as `deepr-route-explanation-v1`.

**v2.31.0 additions:** the grounding stamp finally reaches the step that writes an answer. Council consults now *disclose* each selected belief's grounding assurance into the synthesis packet - an inline `cross-vendor verified` / weaker `same-vendor verified` label next to the existing confidence and contested markers, a `beliefs_verified` count in the perspective `context` for programmatic hosts (carried verbatim into `deepr-consult-v1`, so MCP/A2A hosts see it), and a synthesis prompt that now defines those labels so the model can read them. Per AGENTIC_BALANCE this is disclosure, not a gate: deterministic code surfaces the label; the synthesis model owns how much to weigh it. Lexical-overlap selection order is unchanged and an unverified belief is never dropped or penalized - the naive alternative (moving grounding into the read-time trust ceiling) was rejected as brittle, since it would let one model verdict retroactively and permanently gate a belief. Grounding assurance was already recorded on beliefs and surfaced in handoff/recall/memory-card summaries; this closes the last gap by carrying it into synthesis. The "which levels count as verified" semantics now live once on `CheckAssurance` (`VERIFIED_ASSURANCES` / `ASSURANCE_LEVELS`), shared by the council packet and the handoff summary.

**Purpose, outcome, and value-evaluation foundation (2026-07-16):** `deepr expert blueprint`
now creates explicit `deepr-expert-blueprint-draft-v1` files and zero-call
`deepr-expert-blueprint-preflight-v1` artifacts before any authority exists.
Preflight validates form, normalizes and hashes the draft, summarizes structural
coverage, and supplies review questions while explicitly denying semantic
assessment, human-review claims, canonical writes, and scope authority. Apply
records an operator attestation in append-only `deepr-expert-blueprint-v1`
history; reviewer identity remains unverified and human authorship is not
claimed. `deepr expert record-outcome` and `deepr expert outcomes` add the same
honest operator-attestation boundary for later decision observations, with
optional consult, belief, source, and outcome-evidence links.
`deepr eval expert-value NAME --template` now emits an intentionally incomplete,
blueprint-bound four-arm workbook, and `--from-file` validates completed
operator semantic and protocol attestations over hashed source worlds and run
artifacts before reporting separate quality, stale-memory, false-support,
transfer, cost, effort, and
cost-only break-even measures. The evaluator itself costs `$0`, makes no model,
provider, or network calls, writes only to an explicit output path, adds
reproducible 95 percent paired-bootstrap intervals without superiority flags,
selects no winner, and changes no default. Default operator-attested mode does not
open referenced files. Explicit `--artifact-root` mode independently
root-confines and recomputes every declared local SHA-256 binding before a
report can be written. External arm execution can still consume the capacity
...
memory surface. This order is dependency-based, not calendar-based:
...
0. **No Surprise Bills: spend caps that cannot be overshot (governing money
   ...
   tighter bound winning, the monthly gate wired into `deepr research` and
   MCP dispatch, ambiguous-POST retry double-billing eliminated,
   artifact-before-settlement ordering everywhere, strict ledger tracking by
   ...
   - [~] **Settle-time divergence and provider-complete pricing**: the
      divergence event, cross-process freeze, canonical queued-model pricing,
      and exact provable OpenAI-compatible web-search and code-interpreter
      charges are shipped. Missing or inconsistent model, usage, tool, or
      settlement evidence consumes the reservation ceiling or freezes paid
      work. Finish complete envelopes for autonomous server-side tool loops and
      hosted storage before enabling them. Grok search and legacy deep-research
      chat remain gated because a token cap does not bound their autonomous
      tool bill.
    ...
    - [~] **Canonical cost-state consolidation before paid recovery**: a stable
      canonical cost-state identity, monotonic source-registry prefix, ledger
      and reservation high-water digests, strict duplicate-key rejection, and
      monotonic checkout `.env` caps now bind editable and installed processes.
      Missing, truncated, replaced, or widened registered state keeps paid
      authority at zero. Still migrate every legacy artifact under locks,
      preserve recoverable source evidence, and add an independent monotonic
      anchor so a whole canonical-root rollback cannot look internally valid.
   ...
   - [x] **Opaque external execution quarantine** (2026-07-29): arbitrary CLI
     judges, skill tools, generic outbound MCP and A2A calls, SearXNG upstream
     search, remote heartbeat delivery, ngrok, cloud deployment helpers, hosted
     blob storage, portrait generation, and unaccounted server-side tool loops
     now block before process, network, client, reservation, or durable-write
     side effects. Direct local Ollama and plan quota probes disable ambient
     proxies and redirects so an external intermediary cannot forge `$0` or
...
2. **Longitudinal expert value proof** - prove that a compiled, maintained
   ...
   `deepr-expert-value-report-v1`, and `deepr eval expert-value`. The evaluator
   binds the exact current blueprint, requires at least two linearly chained
   frozen source worlds and the same acceptance cases across all four arms, and
   keeps correctness, source relevance, factual support, false support,
   invalidated-belief reuse, abstention, update behavior, transfer, reviewer
   effort, cost, and outcome links separate. Pairwise deltas use reproducible 95
   percent paired-bootstrap intervals, and a failed update stays in the
   completion denominator without a fabricated latency. Default
   operator-attested mode does not open artifact references. Semantic and
   protocol attestations record unverified identity and no human-authorship
   claim. Explicit
   `--artifact-root` mode root-confines local references and independently
   recomputes every bound SHA-256 digest before reporting. Neither mode runs
   arms, inspects answer text, infers semantic labels, ranks arms, assesses
   statistical sufficiency, attributes outcomes causally, or changes a default.
   Next run one flagship expert with blinded, randomized semantic review and
   explicit operator attestation, then
   publish the completed workbook, artifacts, and report before using any result
   to propose a default, prompt, retrieval, or learning-policy change. No
   aggregate score may hide false support, stale-memory reuse, or negative
   transfer. Design:
...
4. **Protocol-native expert collaboration over MCP and A2A** - design:
   ...
   MCP already exposes expert reads, consults, loop status, handoff, belief
   explanation, scoped keys, budgets, rate limits, audit logs,
   `structuredContent` JSON-object results, and the
   `deepr-expert-collaboration-v1` council contract. The current operating model
   is one or many experts and one bounded proposal artifact. Each member
   contributes an independently selected stored-state packet, not a live
   question-specific generation. There are zero peer turns and at most one
   synthesis call. The synthesizer produces agreement and disagreement lists,
   and consult never writes expert state, beliefs, or graph state. The generated
   contract now exposes those facts, the total spend ceiling, the 10 percent API
   synthesis slice, and zero metered perspective calls. CLI `--output FILE`
   explicitly saves the full artifact. These fields are proposal metadata, not
   proof that experts independently judged, deliberated, or reached consensus.
   The copyable
   [Three Expert Council guide](docs/THREE_EXPERT_COUNCIL.md) applies operator-attested
   Temporal Knowledge Graphs, Digital Consciousness, and MCP blueprints, sets
   hard per-job, daily, and monthly `$5` caps, and routes discussion into
   source-seeking research rather than absorbing it as evidence.

   July 2026 evidence adds requirements to any live runtime: deliberately
   diverse evidence, consensus-free early stopping, blinded peer-content
   evaluation with private canonical lineage, targeted message retention, and
   selective escalation rather than mandatory full-panel execution. Measure
   identity and order bias, minority-position preservation, quality per call,
   and false support before widening a default.

   `deepr mcp validate-consult` proves the no-metered one-shot path through an
   offline fixture or in-process local or safety-eligible plan capacity without
   judging answer meaning. Supplying a URL returns
   `MCP_HTTP_CONSULT_VALIDATION_BLOCKED` before network access.
   `deepr mcp validate-consult-fleet` performs
   bounded plan-backend validation. The A2A library advertises
   `deepr_consult_experts` and maps completed consults onto Deepr task artifacts,
   but no shipped command serves that contract long-running and the data model
   still needs A2A 1.0 conformance work. The `$0` structural evaluator and
   protocol-neutral durable single-expert local conversation core are complete.
   Experimental `deepr expert investigate` now runs locally at exact `$0` with
   no fallback. It freezes each expert, gives the shared question a distinct
   hash-bound frozen-domain evidence lens, performs independent free-web
   retrieval, supports `independent`, `discuss`, and `deep` protocols, checks
   and synthesizes with preserved dissent, and stages separate factual and
   non-factual perspective envelopes. Model-generated charter queries cannot
   authorize network access. `apply-learning` hash-verifies and preflights every
   selected envelope before explicit idempotent apply across locked experts.
   Factual writes require sources plus an independent verifier. Perspectives
   require model-assessed form, internal coherence, and testability but retain
   `truth_verified=false`, `novelty_verified=false`, and
   `human_reviewed=false`. Applying perspectives does not advance factual
   freshness.

   Plan-quota, metered API, MCP/A2A remote investigation, and automatic learning
   apply remain gated. A future metered `$5` run must reserve and settle against
   one `$5` total, never one allowance per expert or phase, and must enforce
   aggregate calls, searches, tokens, context, elapsed time, retries, replay,
   cancellation, and proposal-only authority before shipping.

   Promotion evidence must compare the strongest single expert, one-shot
   consult, independent investigation, discussion, and deep revision under
   matched envelopes. Measure decision utility, factual support, retrieval
   precision and diversity, temporal ordering, contradiction behavior,
   calibrated abstention, minority-hypothesis retention, later hypothesis
   predictions, negative transfer, forgetting, source independence, and later
   held-out benefit. Graph size, candidate count, consensus, and self-reported
   confidence are not improvement metrics.

   Research cutoff 2026-07-16: the current design is informed by
   [Diverse Evidence, Better Forecasts](https://arxiv.org/abs/2607.01661),
   ...
   These are emerging results, including preprints, so Deepr treats their
   reported gains as design hypotheses to test. They motivate evidence
   diversity, explicit null hypotheses, bounded selective communication, and
   the prohibition on automated novelty certification.
   - [x] Stage 2 MCP surface (2026-07-15): four free local-only tools
     ...
     frozen expert snapshots; local Ollama executor with no metered fallback and
     no expert-memory writes; request identity binding for stdio and scoped-key
     HTTP; ownership and expert-allowlist gates; published
     `deepr-expert-conversation-operation-v1` and
     `deepr-mcp-conversation-validation-v1`. The inbound conversation tools
     remain available on the local server, but both managed-loopback and
     authenticated-URL `deepr mcp validate-conversation` modes now return a
     structured block before network, server, socket, or model-executor
     construction. Unit coverage enforces that quarantine. Next: held-out
     quality comparison against repeated one-shot
     consults, then A2A 1.0 migration and durable serving, OAuth and live
     third-party registration, and independently evaluated structured panel
     deliberation. Keep LAN-agent smoke tests copyable through `docs/MCP_AGENT_TEST_GUIDE.md`. Deepr remains a role on the team, not the global orchestrator; host agents decide and enact. Why: external agents should be able to ask a durable expert council to collaborate, inspect agreement and dissent, then continue with structured state instead of opaque prose.
   ...
   cost estimation. Live metered chat is hard-gated in v2.40 with no runtime or
   environment override; `backend=api`, browser, and
   CLI chat return `metered_expert_chat_accounting_unavailable` before provider
   work. The OpenAI and Anthropic request-shape adapters remain covered as
   disabled implementation substrate, not usable metered capacity.
   ...
   submission substrate still uses the OpenAI Responses API shape but remains
   production-frozen until there is a separate research-job backend contract.
   Local Ollama and plan-quota
...
5. **Level 5 consult trace and semantic quality flywheel** - stored-belief
   ...
   local Ollama judging, trend reports, and judge-calibration report processing
   work without metered fallback. Plan-quota CLI, arbitrary CLI, and premium
   API judge requests are execution-gated before process or provider
   construction until Deepr can independently prove billing source, overage
   posture, and total cost under the shared durable transaction. Untrusted
   model judges cannot steer
...
- **Bounded research preview**: `research --preview` computes one finite provider request envelope without provider work. Production metered research, legacy `check`, and metered `learn` entry points are blocked in v2.40 pending authenticated provider account authority and current credential identity.
- **Guided setup**: `deepr init` (detect sources without claiming workflow eligibility, write `.env`, set budget + a portable data dir), `deepr doctor --skip-connectivity` (offline diagnostics incl. storage roots, severity-aware so optional/keyless state is not flagged as an error), base `deepr capacity` for source-specific next inspections, and optional plain `deepr doctor` for supported OpenAI, Gemini, and xAI connectivity checks; Anthropic and Azure remain configuration-only there
- **Cost controls**: Budget limits, canonical cost ledger, cost tracking, `costs show/timeline/breakdown/doctor`
- **Expert state and local creation**: `expert make --local`, structured expert reads, and derived export/import views. Standalone metered creation and chat are gated in v2.40; explicit local and safety-eligible non-metered plan expert workflows remain available where documented.
- **Sequential portability**: `deepr init --data-dir` coordinates expert, report, and runtime roots for sequential cross-machine use; `DEEPR_DATA_DIR` alone does not override the separate report root. Generic synced folders require stopped Deepr services, one writer at a time, and a completed sync before switching devices. Concurrent device mutation remains planned ([ADR 0004](docs/decisions/0004-one-experts-root-and-portable-data-dir.md), [design](docs/design/multi-device-expert-continuity.md))
- **CLI output modes**: `--verbose`, `--json`, `--quiet`, `--explain`
- **Context discovery**: `deepr search`, `--context <id>` for reusing prior research
- **Provider registry and bounded previews**: model metadata spans OpenAI, Gemini, xAI, Anthropic, Azure OpenAI, and Azure AI Foundry. Registry presence is not an execution claim. Finite request envelopes are preview-only in v2.40; all production metered provider dispatch, including managed Gemini Deep Research, xAI multi-agent research, and Azure Foundry Agent/Thread/Run execution, is blocked.
...
- **Web dashboard**: Local research management UI - 12 polished pages with Socket.IO push and polling fallback, write-free OpenAI request preview and production-frozen submission, session-scoped draft recovery that excludes uploads, estimate-bound benchmark approval, scoped partial-failure recovery, explicit queue and ledger semantics, skeleton loading, shadcn/ui components, compact mobile state, and accessible result navigation
...
- **Native Recon instrument** (v2.11.0): auto-discovery and the structured `deepr-recon-evidence-handoff-v1` contract are available when `recon-tool` is installed. Metered agentic-chat invocation is gated in v2.40; evidence ingestion remains subject to the explicit local or owned-capacity workflow and belief-write verification gates.
- **Native Distillr instrument** (v2.12): discovery and free `find_insights` corpus search remain available when `distill-mcp` is installed. Paid ingestion through metered expert chat is gated in v2.40.
- **Native Primr instrument** (v2.12): discovery and free `quick_lookup` context remain available when `primr-mcp` is installed. Paid deep-dives through metered expert chat are gated in v2.40.
- **MCP server**: Functional with 36 tools on the current compatibility surface. Migration to the final MCP `2026-07-28` stateless core and per-request negotiation remains open; Deepr does not claim background Tasks support.
- **Agentic expert chat**: metered dispatch is gated in v2.40 pending the P1
...
- **Provider failure handling**: durable reservation and settlement substrate is
  ...
  and automatic cross-provider metered fallback are blocked in v2.40.
- **Cloud deployment templates**: AWS/Azure/GCP templates provided but not battle-tested at scale
- **Grok provider**: single bounded responses are represented in the registry where pricing is complete; xAI multi-agent deep research is gated in v2.40.
- **Anthropic provider**: Extended Thinking and orchestration request shapes remain disabled implementation substrate; production metered dispatch is blocked.
- **Azure AI Foundry provider**: model metadata remains visible, but Agent/Thread/Run execution with Bing grounding is gated in v2.40 until its multi-call and tool costs share one durable envelope.

### Additional Works-Now Surface

- One bounded API research preview when the selected provider, model, tools, token ceilings, storage behavior, and price are all known. Production metered dispatch is blocked in v2.40 for every provider.
...
- Read-only and derived expert surfaces: stored beliefs, gaps, contradictions, loop status, memory cards, handoffs, health audits, validation, exports, and local/plan consultation. Metered learning, chat, lifecycle mutation, and generated portraits remain gated.
- Expert skills system: 7 built-in skills, Python + MCP tool types, auto-activation triggers, three-tier storage
- Conversations API for browsing stored sessions. Resuming with a new metered turn remains gated in v2.40.
...
- Durable accounting substrate for a future bounded provider attempt, with
  conservative ambiguous-outcome settlement. Production metered dispatch and
  automatic metered fallback are blocked in v2.40.
- `$0` route explanations and exact per-job previews. Automatic cross-provider metered fallback is gated until every attempt owns a reservation.
- Batch and campaign previews remain available; metered execution is gated until a durable parent reservation can cover all child requests.
- Cost dashboard (`costs timeline`, `costs breakdown --period`, `costs expert`, `costs doctor`)
- Multi-layer budget inspection and fail-closed recovery; positive limits do not authorize production metered dispatch
...
Completed implementation details now live in [docs/CHANGELOG.md](docs/CHANGELOG.md) by release and in the current Unreleased section.
...
The AWS, Azure, GCP, and hosted MCP templates are mechanically inert,
reference-only, and unsupported in v2.40. See [deploy/README.md](deploy/README.md).
Their managed compute, networking, storage, queues, logs, and gateways can incur
charges outside Deepr's ledger. Provider budget alerts are not hard caps, so
the templates cannot honor the repository-wide `$5` ceiling.
...
A shared library (`deploy/shared/deepr_api_common/`) provides reusable
validation, security, and response utilities across the cloud handlers. The
legacy deploy scripts and Azure setup helper fail closed before authentication,
file writes, or provisioning. Hosted research submission independently blocks
before payload parsing, durable writes, queueing, or provider construction.
...
- **Types**: mypy is a blocking `--strict` gate; target is 100% of `deepr/` strict-clean. Wired non-blocking first to record the baseline, then strict-blocking on `core/` + `providers/` + `mcp/` + `security/` + `queue/` + `storage/` + `tools/` + `routing/` + `worker/` + `webhooks/` + `a2a/` + importable `deepr.skills` and every new module, ratcheting package-by-package until the whole tree is clean. (Astral's `ty` is a candidate to replace mypy once it stabilizes.)
...
- **Security**: `pip-audit` blocking on every push; weekly dependency update automation (pip + github-actions + npm); SBOM via `uv export` per release; OpenSSF secure-coding practices (boundary validation with Pydantic v2, no secret logging, exception safety) as review criteria.
...
- [ ] Extend the strict-blocking gate to the rest of the tree, package-by-package (whole-tree `mypy` stays a non-blocking baseline meanwhile)
  ...
  - 2026-07-08: `tools/` is now the seventh blocking strict island after minimal contract-aligned typing cleanup in tool execution, registry, web search, and search-backend score parsing. Follow-up probes still found `routing/` strict errors.
  - 2026-07-08: `routing/` is now the eighth blocking strict island after minimal data-shape typing in auto-mode routing and a constructor annotation in `experts.router.ModelRouter`. The local no-incremental strict probe passed for `src/deepr/routing`.
  - 2026-07-08: `worker/` is now the ninth blocking strict island after annotation-only cleanup in the local job poller. The local no-incremental strict probe passed for `src/deepr/worker`.
  - 2026-07-08: `webhooks/` is now the tenth blocking strict island after annotation-only cleanup at the Flask callback and ngrok tunnel lifecycle boundaries. The local no-incremental strict probe passed for `src/deepr/webhooks`.
  - 2026-07-08: `a2a/` is now the eleventh blocking strict island. The package was already strict-clean; the local no-incremental strict probe passed for `src/deepr/a2a`, and focused A2A unit and property tests passed.
...
- [x] Grok 4.20 multi-agent adapter substrate via xAI Responses API (execution gated in v2.36 pending a complete parent fan-out reservation):
...
- A2A interoperability prototype, currently a library and validation surface:
  - [x] Agent Card generator with current and legacy discovery paths and expert
        skills, including `deepr_consult_experts`.
  - [x] In-memory submitted -> working -> completed/failed/cancelled task
        lifecycle, progress seam, and bounded terminal-task eviction.
  - [x] Budget propagation and trace ID stitching via Deepr task metadata.
  - [x] Versioned `deepr-a2a-task-v1` task/result envelope with runtime
        fail-closed output validation before host dispatch.
  - [x] Multi-expert consult mapping that attaches the full
        `deepr-consult-v1` artifact with capacity, cost, roster, and dissent
        metadata.
  - [ ] Migrate or version the prototype against official A2A 1.0 operations,
        states, Agent Card fields, `contextId`, media types, and caller-scoped
        authorization. The current Deepr envelope is not an A2A 1.0 conformance
        claim.
  - [ ] Ship `deepr a2a serve` only after tasks and contexts are durable across
        restart, per-request authorization is caller-scoped, polling passes,
        and a separate A2A 1.0 client validates the live listener. Streaming
        follows durable replay; push notifications remain deferred.
...
- List / 1:1 / group consult over MCP - `deepr_list_experts`,
  ...
  `deepr_explain_belief`, `deepr_temporal_edges`. One core (`experts/consult.py` + `ExpertCouncil`) backs
...
- [x] Configurable auto-fan-out breadth: `MAX_CONSULT_EXPERTS` and
  ...
  Parallelism stays bounded by `MAX_COUNCIL_CONCURRENCY`. Explicit rosters now
  share the same ten-expert ceiling and reject exact, case, or slug aliases for
  one canonical expert before coroutine creation.
...
- [ ] Durable expert conversations over MCP first, then A2A: the local Ollama
  ...
  relies on an MCP transport session and never writes conversation text into
  expert memory. Design:
  [remote-expert-conversations.md](docs/design/remote-expert-conversations.md).
  ...
  - [x] Added MCP start, continue, inspect, and close tools for local Ollama only
    on 2026-07-16. Managed-loopback and authenticated-HTTP
    `validate-conversation` are now quarantined before transport or executor
    construction, with no-network regression coverage. No metered fallback.
  ...
  - [ ] Map the shared core to A2A `contextId` only after the A2A 1.0 and durable
    serve prerequisites above pass.
  - [ ] Keep structured panel deliberation separate: independent first
    positions, targeted challenge, private revision, preserved dissent, and
    transparent aggregation must beat the one-shot council within the same
    capacity envelope. `deepr eval deliberation` already runs eleven `$0`
    frozen-fixture structural checks, but live multi-round execution remains
    gated on measured provider-call token and context enforcement.
- [x] Multi-expert consult mapping in the A2A library: the generated Agent Card
...
- [~] Eval methodology v2 (design: [docs/design/calibration-and-trust.md](docs/design/calibration-and-trust.md)):
  ...
  - [x] Continuity-property metrics, measured from stored expert state at $0 (2026-06-13, methodology v1.2 on 2026-06-29): `deepr/experts/continuity_metrics.py` + `deepr eval continuity NAME` (`--json`) score staleness honesty, abstention correctness, contradiction surfacing, what-changed exactness, read-side temporal edge qualifier visibility, and generated-digest temporal edge qualifier visibility - each against ground truth derived *independently* of the surface it scores (time-based vs confidence-based; recorded edges vs the contested reader; raw event log vs the what_changed buckets; stored temporal edges vs explanation and digest outputs), so a high score means the surfaces agree with reality, not themselves. Methodology-versioned for run comparability; not-applicable metrics are excluded from the overall rather than scored zero. The ATANT audit shows popular memory benchmarks test at most 2 of 7 continuity properties - this measures Deepr's own surface instead of chasing LoCoMo-style scores (see [docs/design/belief-lifecycle.md](docs/design/belief-lifecycle.md))
...
- [ ] Graph-structured expert memory (design: [docs/design/temporal-knowledge-graph.md](docs/design/temporal-knowledge-graph.md)) (the temporal knowledge graph - what makes an expert a *perspective*, not a corpus):
  ...
  - [~] Semantic recall over beliefs (the one genuine recall gap, confirmed by the 2026-06 memory-systems sweep): belief recall today is lexical word-overlap within a domain (`_find_similar`/`_find_related`/`get_beliefs_by_domain`), so a belief only surfaces when the query shares words and lands in the right domain - a paraphrased or cross-domain belief is invisible. Shipped: a persisted local belief-vector index stores already-computed embeddings, ignores stale vectors when a claim changes, prunes archived belief vectors, and lets `BeliefStore.recall_belief_candidates` plus claim-verification recall packets use caller-supplied query embeddings while remaining `candidate_only`, read-only, and no-provider. A construction-side refresh helper can index missing or stale vectors through an injected embedder only after an explicit budget estimate passes, and `deepr expert refresh-semantic-recall NAME --embedding-model MODEL --embeddings-json PATH` now wires that helper to an operator-facing precomputed-vector import path that reports declared upstream estimate separately from `$0` Deepr spend. The operator-facing `deepr expert semantic-recall NAME QUERY` and host-facing MCP `deepr_semantic_recall` surfaces now expose `$0` lexical candidate recall and indexed vector recall when the caller supplies an explicit query embedding and embedding model; neither generates embeddings. Remaining: wire provider or plan-capacity embedding dispatch into absorb/sync, off by default, with local or quota capacity preferred and metered API requiring an explicit estimate and budget. The existing epistemic layer (trust ceiling, decay, contradiction edges, `explain_belief`) still does the *judgment*. Recall finds; the belief graph concludes - this is route-to-the-model, not a lexical verdict, so it passes the STOP banner. Constraints: local-first index (numpy/`sqlite-vec`, no new service), no automatic query embedding without an explicit budget/capacity gate; explicitly **not** GraphRAG, Neo4j, or Letta-style memory - those re-solve problems the structured graph already solves at a cost the design rejects. Bonus: the same vector recall raises the *recall of contradiction candidates*, feeding the entailment screen paraphrased conflicts the lexical router misses.
- [~] Regenerated expert digest (a browsable view over the structured store, never the source of truth):
  ...
  - [x] Render stored temporal edge qualifiers in the digest as derived view content, including valid time, observed time, temporal scope, provenance, and honest missing-endpoint labels
...
- [~] Belief lifecycle and salience (design: [docs/design/belief-lifecycle.md](docs/design/belief-lifecycle.md)) - memory governance, grounded in the 2026-06 memory-systems corpus review (monotonic accumulation is the literature's documented root failure mode; outcome-driven forgetting converges to true usefulness; contested beliefs are irreducible signal):
  ...
  - Atomic claim decomposition at absorb: extraction prompt already enforces
   ...
   - [~] Outcome evidence foundation (2026-07-16): append-only
     operator-attested decision outcomes now provide the missing observation signal and can link
     consult traces and beliefs. They do not establish causal attribution. The
     longitudinal evaluator reports outcome links descriptively and emits
     `causal_attribution: false`; belief-level credit assignment and any
     When-to-Forget policy remain gated on prospective held-out evidence.
...
- [ ] Skill format conversion between Claude Skills, OpenClaw Skills, and agentskills.io
...
votes. The bounded-deliberation design, `$0` structural evaluator, and opt-in
local structured-position graph exist, but public runtime and live multi-round
consultation remain gated.
...
unless every model node proves a literal-loopback Ollama endpoint, stable
config-level cloud disablement, and an exact materialized local GGUF digest and
size. The native transport inherits no provider credentials or proxies, follows
no redirects, and retries zero times. Provider is `local`; tools, remote
retrieval, plan fallback, and metered fallback are absent. Node, call, aggregate
token, context-byte, artifact-byte, concurrency,
...
**Eval-only implementation, 2026-07-29.** `deepr eval consult
--structured-local QUESTION` now freezes one bounded stored packet per selected
expert, validates the fixed acyclic graph, literal-loopback endpoint,
`cloud.disabled=true` stable-config proof, and exact local GGUF provenance
before inference, runs independent question-specific positions with explicit
local concurrency, requires every branch before one synthesis, and emits the
versioned brief, position, synthesis, and run contracts. Every node has one
attempt, `$0` metered authority, no plan or API fallback, no tools or retrieval,
no expert or project-state writes, bounded tokens, bytes, artifacts, and time,
and exact terminal accounting. A credential-free native transport disables
environment proxies, redirects, and retries; each node requires an fsynced,
content-free `$0` cost-ledger dispatch marker, and local model residency is fixed
at five minutes. This is evaluation infrastructure, not a new MCP or public consult
mode. Matched-resource semantic comparison remains the promotion gate.
...
- [~] **Consult brief contract:** the eval-only
      `deepr-structured-consult-brief-v1` now freezes the exact question,
      expert snapshots, DAG, local authority, and aggregate envelope. The richer
      decision, time-horizon, known-evidence, constraint, unknown, and success
      framing below remains open before runtime promotion. Add a versioned, read-only
...
- [~] **Question-specific perspective contract:** the eval-only
      `deepr-structured-consult-position-v1` now separates answer or abstention,
      evidence claims and refs, assumptions, unknowns, uncertainty, alternative,
      disconfirming test, and decision implications. Forecast-specific estimates,
      a follow-up-question field, and matched-resource quality evidence remain.
      Prototype
...
- [x] **Owned-local dependency-graph prototype:** add versioned brief,
      ...
      [local-structured-consult-graph.md](docs/design/local-structured-consult-graph.md).
- [~] **Evidence-linked synthesis contract:**
      `deepr-structured-consult-synthesis-v1` now preserves position artifact ids,
      agreements, dissent, uncertainty, and next tests behind `require_all`.
      Claim-level lineage, unique-evidence fields, and semantic evaluation remain.
      Prototype a versioned synthesis
...
through 2026-07-16, the exact call formula, `$0` and `$5` semantics, contracts,
...
- [x] **Stage 4, staged per-expert learning:** implemented experimentally on
      2026-07-17 and extended 2026-07-18. The factual lane reuses source-note
      claim extraction, independent support and target-domain verification, and
      graph commit envelopes. The perspective lane preserves typed hypotheses,
      concepts, stances, and original ideas from each final position when the
      checker assesses their form, internal coherence, and testability as
      `well_formed`. It cannot verify truth, importance, originality, novelty,
      or human review. No lexical rule decides meaning. `apply-learning`
      hash-verifies and preflights every selected expert and lane before one
      explicit idempotent apply under roster locks. Dialogue and synthesis
      cannot enter the factual source pack; partial outcomes resume
      idempotently; no proposal is auto-applied or labeled human-reviewed.
...
- [ ] **Stage 6, metered API and automatic-apply gates:** reserve and settle
      every child call beneath one total ceiling, including a `$5` maximum.
...
- [x] **Live-validation finding, hidden reasoning exhausted structured output:**
      a 2026-07-18 deep pilot let a thinking-capable Ollama model spend the
      entire 4,096-token output allowance in its separate reasoning field. The
      public content was empty, so the run failed closed before any learning
      write. Fixed 2026-07-18 by explicitly sending `think: false` on native
      investigation calls. Deepr still never treats a hidden reasoning trace as
      the validated answer, and models that cannot disable thinking continue to
      fail closed if they produce no structured content.
- [x] **Live-validation finding, search telemetry consumed evidence slots:** a
      free search backend returned a relative click-tracking link that the SSRF
      boundary correctly refused, but it still occupied one bounded page-fetch
      position. Fixed 2026-07-18 by filtering bounded absolute HTTP(S) URL form,
      credentials, invalid ports, and unsafe host syntax before fetch-slot
      assignment, while recording only an aggregate rejection count.
- [x] **Live-validation finding, evidence URLs polluted discovery queries:**
      explicit caller URLs were correctly fetched first but were also copied
      into every generated search route. Long URL-bearing searches produced
      noisy results and weakened the distinct expert lenses. Fixed 2026-07-18
      by separating discovery text from explicit retrieval targets: search
      routes omit URLs, direct evidence URLs remain first-class fetches, and a
      URL-only deep request performs no unnecessary search.
- [x] **Live-validation finding, deep-round prompt envelope exceeded context:**
      a 2026-07-18 deep pilot completed research, three independent positions,
      one challenge round, and two private revisions before the third revision
      exceeded the correct 32K-context per-call byte ceiling. The run stopped as
      `budget_exhausted`, wrote no expert state, and spent `$0.00`. Fixed
      2026-07-18 by adding explicit component and total packet ceilings for
      snapshots, inputs, source excerpts, positions, discussions, checking, and
      synthesis. A revision may now cite only caller evidence and the expert's
      own rendered source pack; peer dialogue remains a proposal rather than
      evidence. A worst-shape regression proves every protocol prompt fits the
      pinned 114,688-byte input allowance.
- [x] **Security-review finding, plan quota could not prove zero overage:**
      Claude Code's current subscription flow can optionally use paid extra
      usage, while attempt and legacy compatibility accounting previously
      populated an unsupported `overage_enabled: false` value. Fixed 2026-07-18 by removing fabricated
      overage metadata and requiring a fresh, strictly parsed provider
      observation before every Claude dispatch. The durable observation must
      state that paid extra usage is disabled. Claude receives no tools, no
      project or session context, and pinned Sonnet. Unknown state, enabled
      extra usage, ledger failure, or an unclassified model stops before process
      launch. Explicit selection cannot override the gate.
- [x] **Live-validation finding, zero-dollar Claude flag was not executable:**
      Claude Code 2.1.206 rejects `--max-budget-usd 0` during argument parsing
      because it accepts only a positive value. A 2026-07-18 probe therefore
      stopped before model inference, recorded the conservative attempt, and
      spent `$0.00`. Fixed the same day by removing the invalid flag rather than
      substituting positive spend permission. The executable no-bill boundary
      remains fresh provider proof that paid extra usage is disabled, refusal
      of API credentials, pinned Sonnet, safe mode, empty tool and MCP surfaces,
      and no session persistence. The next minimal probe completed at `$0.00`.
- [x] **Live-validation finding, Claude confinement disabled plan auth:**
      `--bare` intentionally suppresses OAuth and keychain reads, so a stored
      Max subscription appeared logged out even though metadata authentication
      was valid. The vendor placed that nonzero error on stdout, while Deepr's
      diagnostic inspected only stderr. Fixed 2026-07-18 by using
      `--safe-mode`, which preserves plan authentication while disabling
      customizations, together with empty tools, strict empty MCP configuration,
      disabled slash commands, and disabled session persistence. Bounded
      redacted nonzero diagnostics now inspect both output channels. Unit
      regressions pass, and a fresh guarded probe returned exactly `OK` with
      paired quota and `$0` cost records.
- [x] **Security-review finding, Antigravity side effects were unconfined:**
      explicit Antigravity execution could invoke native agent tools and write
      transcript state outside Deepr's output contract. Fixed 2026-07-18 by
      blocking execution before vendor launch. Fleet visibility remains for
      diagnostics only until both native tools and transcript side effects can
      be deterministically confined.
...
- [x] **Live-validation finding, mixed no-op learning blocked bulk preview:** a
      completed 2026-07-18 deep run staged three provenance-bound empty or
      blocked envelopes alongside three ready envelopes containing six
      operations. The bulk coordinator treated the first empty envelope as a
      transaction failure, so the operator could not inspect the ready lanes.
      Fixed 2026-07-18 by classifying only a provenance-verified producer
      `blocked` or `empty` envelope with zero operations as an explicit no-op.
      All ready lanes still preflight together. An unexpectedly empty ready
      envelope or any additional schema, target, provenance, or operation
      failure remains an atomic blocker. Live dry-run now reports all six
      candidates, three no-op envelopes, zero applied writes, and `$0.00` cost.
- [x] **Live-validation finding, semantic context leaked into claim verdicts:**
      one factual candidate omitted MCP from its statement, but the verifier
      borrowed domain relevance from the surrounding evidence packet. Another
      perspective described the future MCP release candidate as a completed
      transition. None were applied. Fixed 2026-07-18 by versioning the claim
      verification prompt and requiring relevance judgment over the exact
      candidate statement, plus explicit synthesis instructions to retain
      draft, proposal, release-candidate, planned, final, and shipped maturity.
      These remain model judgments and require held-out calibration; no lexical
      verdict was added.
- [ ] **Investigation evidence-quality gate:** the corrected deep run completed
      20 local calls, 12 searches, and 24 page fetches at `$0.00`, but one
      expert source pack still included low-relevance general-web results. The
      factual verifier safely blocked that lane, yet safe refusal is not useful
      research. Compare source relevance, authority, diversity, recency, and
      downstream decision value across direct caller evidence, current bounded
      discovery, and any future model-assisted query route before changing
      network authority or adding calls. Meaning stays model-judged; URL form,
      fetch authority, and bounds stay deterministic.
- [ ] **Semantic promotion gate:** the 2026-07-17 three-expert local pilot
      ...
      fail-closed no-op behavior. A corrected 2026-07-18 deep run completed all
      20 planned local calls and exposed statement-level relevance, temporal
      maturity, and source-quality failures before any write. All final
      artifacts remained `unreviewed`,
...
search + plan quota), with a monthly reserve (default `$5`) that is a pool you
...
- [x] **Concurrency-safe monthly reservation** (2026-06-21): the monthly cap
      ...
      a low monthly reserve - the primary over-spend path for a `$5`/month fleet.
...
- [x] **Library-wide maintenance** (`expert sync-all`, see expert-library.md)
      ...
      heartbeat transport. A third 2026-07-22 pass made dry-run structurally
      write-free: it builds outcomes from the preflight subscription snapshot,
      bypasses profile reload and all write-capable maintenance construction,
      identifies preview mode in human and JSON output, and proves unchanged
      expert-tree paths and bytes across missing and migratable belief state.
      These paths have direct domain and CLI regression coverage.
...
- [~] **Hardening**:
  ...
  - [x] **Off-box heartbeat** (2026-06-24): `experts/heartbeat.py` -
        ...
        - [x] **No-surprise-spend quarantine** (2026-07-29): configuration form
              remains visible for migration and diagnostics, but delivery now
              returns `blocked_unmetered_external_service` before any network
              request. A future remote sink needs a verified zero-dollar or
              prepaid-overage-off contract before reactivation.
...
- [ ] Hosted MCP endpoint (design: [docs/design/hosted-mcp-endpoint.md](docs/design/hosted-mcp-endpoint.md)) (the autopilot on-ramp - promoted from backlog, see Phase 2 landscape note):
  ...
  - [x] Streamable HTTP/SSE serve path:
        ...
        reachable binds.
  - [x] Fail-closed outbound smoke posture:
        `deepr mcp smoke-http URL` now returns a structured block before opening
        a connection. A remote endpoint cannot prove its own request was `$0`,
        including when the URL names a loopback address.
  - [x] Deploy recipe:
        [deploy/mcp-http.md](deploy/mcp-http.md) documents the executable local
        loopback service and loopback-published container. Caddy/nginx and the
        Azure Container Apps, AWS ECS Fargate, GCP Cloud Run, and Cloudflare
        Worker files are mechanically inert reference shapes only. They are not
        supported or live-validated deployment surfaces.
...
- [ ] Server-owned provider-resource registry: persist provider file and vector
      ...
      reserved metadata; this follow-up removes metadata as the ownership source.
- [ ] Durable provider-cleanup recovery: retry failed file and vector-store
      deletion, expose cleanup debt to operators, and retain bounded diagnostics
      without provider-controlled content.
...
- [x] **Public-bind guard hardening.** A2A, MCP HTTP, the Flask API, and the web
  ...
  values are treated as all-interface binds rather than local loopback. This
...
observation proving paid extra usage is disabled before every dispatch. Codex,
OpenCode, Kiro, Grok, Antigravity, and Copilot are visible but blocked.
Automatic Claude routing additionally requires admission and a trusted
remaining-quota observation. The QOL target is a clear route planner: tell the
user whether a job should run locally, use an eligible plan, wait for capacity,
or use a metered API with an explicit budget ceiling.

Vendor reality (revalidated 2026-07-18 - revalidate before each release):

- **Claude Code**: Anthropic paused the announced June 15 Agent SDK credit-pool
  change. Current `claude -p` usage can draw from Claude subscription limits,
  but optional paid extra usage can continue at API rates. Deepr therefore
  requires a fresh provider response with `extra_usage.is_enabled: false`
  immediately before each dispatch. It does not infer billing posture from a
  `$0` local ledger row, cached quota state, or plan ownership.
...
- [~] **CLI provider adapters** - shipped via the `research_fn`/chat-client seam (not the API-shaped `DeepResearchProvider` contract, which is wrong for a subprocess CLI): a bounded subprocess runner, deterministic auth and native-tool gate, declarative adapter registry, and paired quota plus cost-ledger accounting. Explicit `--plan` selects an adapter but does not override safety. Design: [plan-quota-cli-backends.md](docs/design/plan-quota-cli-backends.md).
  ...
  - [~] `antigravity` (`agy -p`) - visible/read-only; execution is blocked
    because native tools and transcript side effects are not confined
  - [x] `copilot` (`copilot -p -s`) - visible/read-only, **execution blocked**; usage-based/metered per token since 2026-06-01 and cannot run until estimate/reserve/settle/canonical-ledger support exists
...
- [x] **Eval-gated local admission** (free does not outrank quality): a local model is *not routable* until the user has reviewed local benchmark evidence. `deepr eval local` compares Ollama models with a local Ollama judge on the agentic-loop prompt set, saved artifacts feed admission through `deepr capacity admit --from-eval latest`, and automatic runtime routing now feeds the admitted score into the measured quality-floor gate. Admission is per model+version - swapping the local model invalidates its eval and drops it from routing until re-benchmarked. Local judges cost `$0` at the margin. All external CLI-judge compatibility options fail before process creation because billing source, overage posture, and total cost cannot be proven.
- [~] **Capacity waterfall in auto mode**: the pure selector orders `local -> plan_quota/credit_pool -> api_metered`, applies eligibility, and enforces measured per-task quality floors; selected expert maintenance consumes admitted local scores at runtime. The automatic metered rung is disabled in v2.36. Remaining work is trusted quota evidence, scheduler/runtime integration, live quality evidence, and a separately approved reservation before any future metered selection.
...
- [~] **No-surprise-bills invariants** (kernel-enforced, tested):
  - [x] Auto-routing never selects a plan-quota CLI without an observed, non-exhausted quota window and the adapter's complete safety eligibility. Explicit `--plan` does not bypass the gate. Unclassified, native-tool-unsafe, and metered-at-margin CLIs fail before execution. Default is never a silent paid call.
  - [~] Overage handling: Claude requires a fresh provider observation proving
    paid extra usage is disabled before every dispatch. Missing, malformed,
    enabled, or unwritable state fails before launch. Copilot is
    `metered_at_margin` and execution-blocked; Kiro remains blocked because its
    prepaid overage posture is unproven. Known metered env keys are stripped
    from explicit plan child processes so API-key shells do not become plan
    spend paths. Equivalent trustworthy live overage proofs remain required
    before any other adapter can execute.
  - [x] Eligible non-metered quota events land in `quota_ledger.jsonl` and a `$0` `cost_ledger.jsonl` entry carries the quota units, so `costs show` and anomaly detection see volume even at $0.
...
- [x] **Simple default surface** (5 of 6 hit this wall; shipped v2.13.x): `deepr --help` now opens with a worked three-command quickstart and lists five core commands (research, expert, costs, doctor, web) before an Advanced section; deprecated commands and single-letter aliases are hidden from the listing but still execute. `.env.example` was reduced to the first-run essentials; extended environment settings are documented in `docs/INSTALL.md` and the relevant feature guides. README gained the plain-language one-liner, the budget-is-a-ceiling note, and a who-this-is-for block.
...
- [x] **Expert mutation audit log** (architect, shipped 2026-06-29): absorb/resolve-conflicts/learn belief writes now append `deepr-expert-mutation-audit-v1` records beside each belief store with timestamp, operation, expert, actor, belief id, event hash, and before/after state hashes. The audit stays separate from belief meaning and does not duplicate full claim text.
...
- [x] **README clarity passes** (buyer + non-technical user; shipped 2026-06-29): the first-screen README now states that users bring AI accounts, plan quotas, API keys, or local models; Deepr previews routes, uses the cheapest capable path, and turns useful results into experts that remember. `--budget 3` is labeled as a $3 budget ceiling in prose and examples, and an explicit "Who Deepr Is For" section separates buyers, builders, agent-host users, and casual one-off chat users.
...
- [~] CLI conformance to mid-2026 best practices (audited 2026-06-12 against clig.dev / kubectl / uv / Heroku / no-color.org; deep-research verified). Deepr already does most of it well: tiered `--verbose/--json/--quiet` with mutual-exclusion, `error_code` in JSON output, stderr discipline in quiet mode, no secrets in argv (keys via env only), kebab-case flags, hidden deprecated commands with warnings + model auto-migration, UTF-8 console handling. Shipped 2026-06-12: non-TTY no-args prints help instead of launching interactive (agent/CI safety); `deepr completion <shell>` for tab-completion. Remaining, by ROI:
  ...
  - [~] `--plain` tabular output for `grep`/`awk` (complements `--json` for `jq`), and a global `--no-color` flag. Shipped 2026-06-29: root `--no-color` now sets `NO_COLOR` and disables color on existing Rich consoles before commands or help render. Remaining: command-specific `--plain` tabular contracts.
...
- [x] Cloudflare Worker ingress reference added 2026-06-19. It is a
      mechanically inert, unsupported template, not a shipped edge deployment.
...
- [ ] New-machine validation findings (2026-06-12, keyless dev setup):
  - [x] Unit tests still relied on convention rather than tooling to avoid outbound network access. Fixed 2026-06-29: the dev pytest environment now uses a socket guard that disables outbound sockets by default and allows only loopback hosts for local fixtures, so accidental provider or web calls fail inside the unit gate instead of leaking to a developer network.
...
- [ ] Live-validation findings (2026-07-11, local expert dogfood):
  - [x] Python 3.14 CI on 2026-07-15 exposed a race in the synchronous consult
    backend cancellation regression: its 100 ms total transaction ceiling could
    expire during durable lifecycle startup before the backend thread entered,
    so the test failed without exercising the cancellation contract. Fixed
    2026-07-15 by signaling entry and completion back onto the event loop
    without consuming another executor worker, giving preflight realistic
    headroom, and keeping the blocked worker wait well beyond the tested
    deadline. Lifecycle startup still consumes the real transaction ceiling.
  - [x] Deepr-Recon integration guidance treated DNS-derived provider indicators as high-confidence facts about what an organization actually runs, including a direct "stripe.com uses AWS" belief example. Corrected 2026-07-12 by defining the versioned lossless `deepr-recon-evidence-handoff-v1` expectation with queried-domain scope, separate observations and unresolved inferences, raw provenance hashes, observation time and freshness, collection success and opportunity, alternatives, and confidence basis. Its frozen Stripe/AWS indicator fixture rejects organization product-use verdict and belief-write authority. This validates the handoff contract, not that every current Recon payload already emits it; adapter emission remains an explicit integration prerequisite in `docs/INTEGRATIONS.md`.
  - [x] `expert make --local --local-model` recorded a per-expert maintenance model, but explicit local sync ignored it and silently used `DEEPR_LOCAL_MODEL` or the first model returned by Ollama. Fixed 2026-07-11 with one shared precedence contract: an explicit command or admitted-capacity model wins, then a valid model recorded on a local expert profile, then the existing process-wide default. Single sync, forced-local roster sync, absorb, topic learn, OKF absorb, and gap-fill construction now share that contract, with regressions that distinguish the profile model from the global default.
  - [x] A fresh-context subscription sent its full synthesis prompt, including a long focus, to DuckDuckGo; five noisy snippets yielded only one content-addressed page, but a large local model still ran for about ten minutes before the unready evidence envelope became visible. Fixed 2026-07-11 by separating the full answer prompt from a concise topic plus bounded-focus retrieval route, retaining explicit URLs, and adding a provenance-only preflight before local or plan generation. Fresh and deep modes now require bounded minimum counts of replayable content-addressed evidence, do not require every search result to fetch, persist under-ready source packs, return a retryable no-metered failure, and leave the subscription due without consuming model or plan capacity.
  - [x] Topic `expert learn` and `learn-web` claimed six live sources even when three page fetches failed, then stored beliefs with only `report:web:<query>` plus prose rather than a durable source URL or pointer. Fixed 2026-07-11 by applying the same content-addressed readiness preflight before local or plan synthesis, keeping snippets and failed fetches as diagnostic candidates, and persisting every attempt as a source pack, manifest, source notes, and snapshots below the configured expert root. Successful synthesis also persists its report. The extraction model now selects exact supporting source labels per candidate; deterministic code validates membership and stores only those replay pointers, never every URL. Under-ready attempts remain retryable, do not fall back, and do not advance knowledge freshness.
  - [x] Follow-up `learn-web` dogfood retrieved five ready sources but rejected all 24 extracted candidates because CLI extraction returned exact replay-catalog values such as `source_note:...` rather than catalog keys, while other outputs used citation-form `[S1]` labels around the `S1` key. The parser accepted neither shape, then the command marked the untouched expert freshly learned and reduced failed fetches to an unactionable HTTP message. Fixed 2026-07-11 by accepting exact catalog-value membership or one deterministic citation-bracket wrapper around an otherwise exact key, keeping unknown pointers and semantic aliases invalid, and never parsing prose evidence as a fallback. Freshness advances only after an absorbed or contested write; human output groups rejection reasons and shows each failed retrieval with a bounded diagnostic label plus URL stripped of userinfo, query parameters, and fragments. Structured learn-web failures suppress the redundant low-level strategy warning while generic scrape callers retain it. No retry or fallback is implied.
  - [x] Claude plan absorption recorded semantically compatible consciousness and harness claims as contradictions, including one pair that received a model YES. The router's tokenization was noisy, but precision tuning alone would only move the brittle boundary. Fixed 2026-07-11 by making normal contradiction detection return only model-selected pairs, preventing generic belief insertion from persisting lexical router hits, and requiring a fresh-context structured disconfirmation pass after an initial absorb-time YES before a model-confirmed typed edge can be written. Ambiguous or disabled verification preserves both claims without an edge or lexical dedup collapse. Recorded candidates now expose verification provenance; health, contested, why, and digest label unverified candidates honestly; continuity methodology v1.3 scores structural surfacing separately from verification coverage and states that neither metric proves semantic accuracy. Exact live pairs are regression fixtures and no model call is required by the eval.
  - [x] Ordinary `expert health-check` claimed to be read-only but appended a health-check loop run, and its scheduled read-only payload could expose a `pending` run even though the command only proposed actions. Fixed 2026-07-11 by removing loop writes and `loop_run` fields from manual and scheduled audit-only paths. The scheduled action-plan contract is versioned as `deepr-health-check-action-plan-v2` because removing that field is a real contract change. Explicit `--archive-stale` mutation, confirmation-wait, and overlap paths retain durable loop records.
  - [x] Scheduled local maintenance could dispatch into an already-busy GPU or treat temporary contention as a failed run, with no durable retry guidance. Fixed 2026-07-11 for scheduled sync, sync-all, local route-gaps, and plan-backed compiled sync with a local recall embedder: a read-only best-effort GPU utilization probe classifies local capacity as free, busy, or unknown before engine construction; confirmed busy state records typed `capacity_unavailable` WAITING outcomes with 30m -> 2h -> 6h bounded retry guidance and never sleeps, kills work, or falls through to plan/API capacity. Busy artifacts preserve the requested operation and material flags as structured argv plus selected capacity/model metadata, so explicit-local guidance cannot contradict the deferred request. Missing or malformed platform support remains honestly unknown and does not disable local scheduling, resident VRAM is not a busy verdict, and explicit local work without `--scheduled` remains the operator override. Design: [scheduled-local-capacity.md](docs/design/scheduled-local-capacity.md).
  - [x] Generated expert memory cards suggested invalid placeholder commands such as `expert consult NAME`, and a written card still advertised the preview contract without stating that its only write was a derived view. Fixed 2026-07-11 with argument-safe quoted expert names, the real repeatable `--expert` consult syntax, and an additive `writes: derived_view_only` contract on written artifacts.
  - [x] The first four-expert local council dogfood ran for 373 seconds and ended at its output limit, but the OpenAI-compatible path looked for `finish_reason` on the response instead of the choice and labeled the partial answer `completed`. The missing tail also erased the structured agreement and disagreement fields, while a mandatory math section encouraged unsupported decorative formulas. Fixed 2026-07-11 by reading the provider completion reason from the correct shape, returning `truncated` plus `OutputLimit`, failing the trace and CLI exit status while preserving the partial artifact, putting dissent fields before prose, making quantitative analysis evidence-conditional, and allowing a larger `$0` local output bound without increasing metered or plan output limits.
  ...
  - [x] A final lifecycle adversarial pass found that elapsed errors were advertised as retryable after provider dispatch, critical progress checkpoint errors were swallowed by the council's best-effort callback wrapper, durable writes blocked the async host, settled cancellation cost was absent from lifecycle progress, raw storage I/O leaked local paths through generic MCP errors, and the published start-event reason rule was weaker than runtime validation. Fixed 2026-07-12 with dispatch-aware elapsed retryability, awaited fatal progress propagation that cancels sibling work, serialized off-loop lifecycle writes with no hidden late writer, bounded settled-cost transfer through cancellation and timeout, distinct path-safe lock and I/O errors with partial-write ambiguity, and matching schema conditions for initial and resumed starts.
  - [x] An externally terminated sync could leave partial source-pack artifacts but no loop record because sync wrote its first snapshot only after engine completion. Fixed 2026-07-11: non-dry single-expert sync and each locked sync-all attempt append a RUNNING snapshot before engine construction, then append completion or caught-failure state with the same run id. Caught failures preserve settled non-negative spend exposed by the error chain in the failed snapshot and sync-all rollup. A hard termination now leaves durable interrupted-work evidence for loop-status and recovery tooling, while dry runs remain write-free and a failed RUNNING write blocks dispatch.
  - [x] `expert sync-all --dry-run` still reloaded profiles through a migration-persisting path and constructed normal engines and absorbers, so a successful preview could rewrite profile or belief state without taking the skipped overlap lock. Fixed 2026-07-22 with one pure preview builder over the read-only preflight subscription snapshot. Preview now bypasses profile reload, provider-client, absorber, engine, belief, lock, loop, heartbeat-request, and artifact construction, identifies its mode in human and JSON output, and has whole-tree byte regressions for missing and migratable belief state.
- [x] Browser expert chat silently opened each Socket.IO turn with a hardcoded `$10` API budget, ignored the requested chat mode, and discarded the session after a successful turn so follow-ups and slash commands could not share state. Fixed 2026-07-11 with an API-only public contract that requires a positive configured ceiling plus two explicit metered-cost acknowledgements before provider construction, applies the selected mode, reuses one serialized per-socket session, prevents budget increases above the approval, and closes the session on explicit end, disconnect, or terminal failure. The REST fallback now enforces the same fail-closed contract; browser local and plan parity remains explicitly unsupported rather than silently falling through to API capacity.
- [x] Live capacity-guidance validation described an explicit `--plan` request as able to fall back to metered API capacity and told operators that revoking a local admission automatically caused metered fallback. Fixed 2026-07-12: the read-only outlook now states that explicit local and plan selections never fall through, missing automatic cheap capacity is reported without guessing whether the concrete command will wait, require explicit metered approval, or refuse, and revocation directs the operator to a fresh `capacity next` inspection before any metered-capable run.
  - [x] Browser `chat_stop` only muted new tokens while the provider coroutine kept running and could still save and emit completion, and the frontend treated every turn longer than 60 seconds as lost even though deep modes advertise 5-20 minutes. Fixed 2026-07-11 by cancelling the in-flight coroutine on its owning event loop, explicitly requesting cancellation for accepted background provider jobs, closing provider and cost-session resources before `chat_cancelled`, suppressing save and completion after cancellation, and holding a durable per-turn ceiling that refunds before dispatch or settles conservatively after ambiguous dispatch. The UI now keeps long-running work attached without an artificial short timeout and preserves partial text only after the server acknowledges Stop. Legacy returned provider-error strings are typed as terminal failures before browser save or completion, and optional follow-up suggestions now reserve a bounded 200-output-token call, settle usage or ambiguous cost, and skip when the approved session ceiling cannot cover them.
  - [x] `capacity probe-plan copilot --json` could dispatch without acknowledgement, while acknowledged Copilot probe, sync, and absorb calls wrote `$0` plan events even though Copilot bills per token. Fixed 2026-07-11 by making every metered-at-margin plan adapter fail before probe or client construction until adapter-specific estimation, durable reservation, usage settlement, and canonical cost-ledger support exist. Human, JSON, `-y`, and retained compatibility flags all enforce the same nonzero result; non-metered plan adapters are unchanged.
  - [x] `expert absorb --plan codex` surfaced only the beginning of a long CLI stderr stream, so the Codex banner displaced the terminal failure cause, and nonzero, timeout, or empty-output dispatches left no canonical `$0` attempt event. Fixed 2026-07-11 by returning a bounded credential-redacted stderr tail with prompt-overlap suppression, recording exactly one cost event for every eligible plan dispatch, and distinguishing successful usage, terminal exhaustion, unknown-usage attempts, and processes that never launched in quota state. Direct probes now own the same accounting, and fleet visibility reports an ambiguous failed dispatch as `attempt_failed` rather than active.
  - [x] The next Codex plan retry exposed its terminal cause correctly but classified `You've hit your usage limit. Try again at 9:20 AM.` as an unknown-usage attempt because the adapter knew only older limit signatures and relative reset durations. Fixed 2026-07-11 with an error-channel-only Codex signature and host-local absolute-clock parsing. The parser chooses only a unique future OS-resolved instant, handles tomorrow and DST offset changes, stores UTC, and leaves ambiguous, nonexistent, or unavailable local clocks unknown instead of guessing.
  - [x] The web background poller queried and cleaned every persisted job through the OpenAI adapter even when `ResearchJob.provider` named another owner, while the OpenAI-only dashboard submission route accepted Gemini, xAI, and Anthropic model identifiers. Fixed 2026-07-11 by making the recorded provider authoritative for polling, terminal settlement, cancellation, and cleanup; unknown or unavailable adapters remain active and observable without fallback. Single and batch web submissions now reject non-OpenAI model families before reservation or provider construction and persist `provider="openai"` explicitly. The CLI web launcher also refuses non-loopback Werkzeug instead of always forcing its unsafe-production override.
  - [x] Direct `python -m deepr.web.app` startup validated a safe loopback bind and then passed `allow_unsafe_werkzeug=False`, so current Flask-SocketIO could reject the intended development server. Fixed 2026-07-11 with one loopback-validating startup helper that enables the Werkzeug override only after that check; non-loopback direct startup still fails before server construction.
  - [x] Live browser-chat validation exposed six connected trust failures: the page initialized a `$0.50` ceiling above the server's `$0.20` maximum, a terminal provider error left only the user message plus a transient toast, conservative ceiling settlement was mislabeled as reported actual cost, the durable hold used the expert's local profile model instead of the routed API model, the OpenAI SDK silently retried the same 429 twice behind one Deepr reservation, and `costs show --daily-limit 2` displayed a persisted `$10` limit. Fixed 2026-07-11 by initializing and clamping the input to the loaded server maximum, retaining a failed assistant entry with explicit re-confirmation guidance, labeling conservative unaccounted-ceiling settlement and its known-cost math, selecting and validating one exact turn target before both reservation and dispatch, failing closed on unknown or mismatched identity, constructing OpenAI and Anthropic expert-chat clients with SDK retries disabled, and applying command-line display limits after persisted dashboard state loads.
  - [x] The same browser-chat run wrote its thought-stream audit and created memory and graph directories below a raw display-name directory while the expert profile and beliefs lived below the canonical slug, silently splitting one expert across two paths. An audit found the same defect class in feedback, prompt-optimization, knowledge-consolidation, direct knowledge-graph, LazyGraphRAG, hierarchical-memory, and document-reconstruction defaults. Fixed 2026-07-11 by routing every default through `canonical_expert_dir` while preserving explicit storage-directory overrides. `deepr expert migrate-legacy-state NAME` is a no-provider, dry-run-by-default repair for one exact profile identity and a closed set of known runtime artifact names plus projected-empty known directories; apply copies, flushes, verifies, then removes each source, refuses differing collisions before any move, and leaves unknown content and other experts untouched. The already exercised `migrate-thought-logs` name remains a hidden compatibility alias. Historical state is not migrated automatically.
  - [x] Cost Intelligence showed nonzero monthly, daily, and all-time canonical-ledger totals while its model card claimed `No spending data yet`. The breakdown endpoint already returned current canonical-ledger model events, but the frontend collapsed a loading or failed query and a genuinely empty range into the same misleading state. Fixed 2026-07-11 with distinct loading, retryable error, populated, and empty-range states plus an endpoint regression proving model-attributed and unknown-model events reconcile from the canonical ledger.
  - [x] The append-only browser-chat accounting correction preserved the conservative `$0.20` total and named the routed `gpt-5.2` model, but provider/model breakdowns still charged the original local-profile model and counted the zero-dollar correction as another call. Fixed 2026-07-11 with a reconciliation-aware derived attribution view. It accepts only a unique earlier idempotency target and schema-valid zero-adjustment metadata, applies provider changes only when both original and routed providers are explicit, rejects malformed or ambiguous corrections, and leaves raw events plus all dollar totals unchanged. The exact live ledger shape now attributes `$0.20` to `gpt-5.2`, one call, while the canonical two-event history remains append-only.
  - [x] Unit CLI runs populated the workspace `queue/research_queue.db` despite the autouse `DEEPR_DATA_DIR` isolation fixture, leaving 323 queued zero-attempt `test query` rows with stale reservation metadata. Both legacy config loaders and no-path `SQLiteQueue` callers had hardcoded the legacy path even though the runtime-root contract says queues relocate. Fixed 2026-07-11 with one precedence contract: explicit `DEEPR_QUEUE_DB_PATH`, then `<DEEPR_DATA_DIR>/queue/research_queue.db`, then the legacy path when neither is configured. The autouse fixture now pins both runtime and queue roots; a CLI regression proves a sentinel workspace queue remains byte-for-byte unchanged. `deepr doctor` inspects the actual `research_queue` schema in read-only SQLite mode and warns on queued zero-attempt rows older than 24 hours without deleting or cancelling anything. The 323 existing user queue rows remain untouched for operator review.
  - [x] The full unit suite's handoff-only hallucination-risk test also loaded a live high-stakes consult trace from `~/.deepr`, producing two signals instead of one because the default trace resolver ignored the configured runtime root. Fixed 2026-07-11 with explicit-path precedence, `<DEEPR_DATA_DIR>/consult_traces/consult_traces.jsonl` when a runtime root is configured, and the existing per-user path only when no override is configured. The regression proves both test isolation and backward-compatible operator storage.
  - [x] Four dogfood experts had verified live beliefs and fresh continuity events but retained a null `knowledge_cutoff_date`, so health-check reported critical staleness while continuity reported fresh evidence. Fixed 2026-07-11 with one completion-observation freshness contract across absorb, topic learn, sync, graph commit, corpus/vector ingestion, MCP, chat, learner, reflection follow-ups, and gap fills. Accepted or contested writes advance both freshness fields and regenerate only Deepr-derived messages; replayable source-backed current syncs may also advance them. Dry runs, all-rejected absorption, under-ready evidence, failures, empty imports, and ungrounded no-change markers do not. Partial file learning records only successful uploads. The cost-$0 `expert reconcile-freshness` command previews by default and can repair profile metadata only from accepted append-only events for currently live beliefs, without provider calls or belief changes.
- [ ] Bug-hunt findings (2026-06-30, expert-chat cost-safety sweep):
  - [x] Anthropic-style expert-chat usage settled as `$0` when registry pricing lookup failed, even though the request had consumed metered input, output, cache-write, or cache-read tokens. Fixed 2026-06-30 by moving Anthropic chat settlement to conservative default pricing on lookup failure and adding regression coverage for nonzero fallback accounting.
  - [x] Legacy `expert fill-gaps` and the web fill-gaps endpoint could construct metered OpenAI-backed gap filling from the old path instead of steering users to local/plan-first `route-gaps --execute`. Fixed 2026-07-01: CLI `fill-gaps` now requires `--api`, unattended `--yes` requires `--confirm-metered-cost`, the web endpoint requires `allow_metered_api` plus `confirm_metered_cost`, and health-check/doc recommendations point to `route-gaps --execute --scheduled`.
  - [x] Generated external Distillr telemetry under `library/.distill/telemetry.jsonl` was tracked in source and contained provider-usage records from historical xAI runs. Fixed 2026-07-01 by removing the generated file from the repository, ignoring `library/.distill/`, and adding a hygiene regression that fails if external runtime telemetry is tracked again.
- [x] GitHub CodeQL security findings (2026-06-30): open CodeQL alerts on main were triaged and fixed. Portrait generation now refunds reserved cost and returns a generic error without re-raising provider exceptions through Flask, citation validation reads markdown documents only after resolving them under the expert documents root, and `deepr mcp agent-guide --json` redacts bearer tokens and refuses unrecoverable JSON-only key creation. Regression tests cover all three fixes.
...
- [x] Live-validation finding (2026-07-12, direct benchmark entry point): the supported CLI blocked live provider benchmarks, but invoking `scripts/benchmark_models.py` directly could still reach provider validation, evaluation, and judge calls. Its documented no-call `--dry-run` also ran optional network model discovery. Fixed 2026-07-12 by applying the same fail-closed v2.36 admission before direct provider preflight, preserving `$0` prompt and stored-ranking paths, preventing discovery during dry-run, and regression-testing both boundaries.
- [x] Live-validation finding (2026-07-12, standalone utility cost audit): `scripts/analyze_doc_gaps.py` loaded `.env` and constructed an OpenAI client before any shared spend gate, while `scripts/discover_models.py --llm` auto-selected an available metered provider and issued a model request without reservation or canonical settlement. Fixed 2026-07-12 by applying the central fail-closed metered mutation gate before documentation-analysis credential loading and before LLM discovery preflight, protecting the direct LLM-discovery Python seam, preserving offline registry display and read-only provider model-list discovery, and regression-testing that no gated path reaches environment, client, registry, or model work.
- [x] Live-validation finding (2026-07-12, AWS hosted research cost audit): authenticated `POST /jobs` wrote an unreserved paid-research job to DynamoDB and SQS, and the Fargate worker independently imported and constructed a provider for any queued message without restoring durable reservation authority. Fixed 2026-07-12 with the dependency-local `aws_metered_research_accounting_unavailable` gate at both boundaries. The API returns a typed HTTP 503 before body parsing, job-id allocation, or durable writes; the worker marks and acknowledges legacy queued work before provider import or construction, with a second guard inside direct execution. Deployment validation now proves the block without enqueueing a job, and regressions assert no prompt, model, secret, or path reaches the public error or status update.
- [x] Live-validation finding (2026-07-12, expert chat command audit): interactive `deepr expert chat` could dispatch slash commands before a normal turn reached the standalone metered-chat gate. `/plan` constructed an unreserved OpenAI client, `/council` could enter paid synthesis without the supported consult approval flow, legacy `/learn` reached hosted file and vector storage, and `/synthesize` used two process-local check-then-record calls. Fixed 2026-07-12 by gating the CLI before session construction and independently gating task-planning, council, hosted-vector upload, and both knowledge-synthesis call seams. Explicit local or plan query and consult remain the supported alternatives.
- [x] Hosted-CI finding (2026-07-12, cross-platform Click wrapping): the API-consult zero-budget regression searched raw output for `positive value`, but Linux Click wrapped the two words across a newline while Windows kept one line. Fixed 2026-07-12 by normalizing whitespace only in the assertion, preserving the exact user-facing error and passing the same contract across Python 3.12, 3.13, and 3.14.
- [ ] Restore direct conflict resolution only after detection, each adjudication, and every consensus provider call share one explicit confirmation, deterministic request bound, durable run ceiling, exact aggregate settlement, and mutation audit contract. Prefer local or explicit non-metered plan capacity before any metered API route.
...
- [ ] Live-validation findings (2026-06-14, $0 CLI walkthrough with real keys):
  - [x] Runtime artifact roots still had hardcoded benchmark, trace, portrait,
        report, and MCP-state paths that could split local state when
        `DEEPR_DATA_DIR` or `DEEPR_REPORTS_PATH` was configured. Fixed
        2026-06-29 by routing the affected writers and readers through the
        configured runtime data root or reports root, adding regression tests,
        and validating the full unit coverage gate at 83.43%.
  - [x] Tests polluted the user's real `data/experts/` (found via `expert list` showing `MagicMock`, `test_expert`, and stray dirs - the smoking gun `MagicMock/Test Expert.name/`). Any test building an expert/belief store/memory with no explicit dir wrote into the real store; `experts_root()` was the one isolation the conftest lacked. Fixed: autouse `_isolate_runtime_data_root` fixture (`DEEPR_DATA_DIR` -> per-test tmp) + regression test; leaked dirs cleaned. Same class as the cost-ledger pollution.
