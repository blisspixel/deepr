...
- [~] CLI conformance to mid-2026 best practices (audited 2026-06-12 against clig.dev / kubectl / uv / Heroku / no-color.org; deep-research verified). Deepr already does most of it well: tiered `--verbose/--json/--quiet` with mutual-exclusion, `error_code` in JSON output, stderr discipline in quiet mode, no secrets in argv (keys via env only), kebab-case flags, hidden deprecated commands with warnings + model auto-migration, UTF-8 console handling. Shipped 2026-06-12: non-TTY no-args prints help instead of launching interactive (agent/CI safety); `deepr completion <shell>` for tab-completion. Remaining, by ROI:
  - [~] Structured error envelope for agent consumers (RFC 9457 / Cloudflare agent-error pattern). Shipped on the core hierarchy (2026-06-12): `DeeprError` carries `category` + `retryable`, and `to_dict()` surfaces them plus `retry_after`, so an agent can classify failures and drive backoff without scraping prose. Remaining: align the MCP `ToolError` surface (it has `retry_hint`/`fallback_suggestion` strings but no machine `category`/`retryable` booleans) and the CLI `OperationResult` JSON error to the same fields, and map provider-SDK exceptions onto the typed hierarchy at the boundary so the classification is populated end to end. Pairs with the Phase 5 handoff-schema work.
