...
2. **Expert self-model and metacognitive monitor** - first-class read-only `deepr-expert-self-model-v1` records are in place for capabilities, limits, current goals, calibration, learning strategy, continuity summary, blocked capabilities, unresolved risks, and a bounded current-focus packet. Consult perspective context now carries the self-model focus packet when an expert profile is available. Next wire the self-model into learning runs, then add the metacognitive monitor that turns measured failures into reviewed goal or strategy updates. Why: Level 5/6 in Deepr means the expert can think about its own knowledge and learning process, not merely execute a fixed loop or accumulate documents.
