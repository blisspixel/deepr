...
- [~] CLI conformance to mid-2026 best practices (audited 2026-06-12 against clig.dev / kubectl / uv / Heroku / no-color.org; deep-research verified). Deepr already does most of it well: tiered `--verbose/--json/--quiet` with mutual-exclusion, `error_code` in JSON output, stderr discipline in quiet mode, no secrets in argv (keys via env only), kebab-case flags, hidden deprecated commands with warnings + model auto-migration, UTF-8 console handling. Shipped 2026-06-12: non-TTY no-args prints help instead of launching interactive (agent/CI safety); `deepr completion <shell>` for tab-completion. Remaining, by ROI:
  - [~] Structured error envelope for agent consumers (RFC 9457 / Cloudflare agent-error pattern). Shipped across all four error surfaces (2026-06-12): `DeeprError`, the provider-layer `ProviderError` (+ `classify_provider_exception` helper and OpenAI-adapter wiring), the MCP `ToolError` (always-present `category`/`retryable`, `from_exception`), and the CLI `OperationResult` JSON error (`from_exception`) all carry `category` + `retryable` (+ `retry_after`). An agent can now classify a failure and drive backoff without scraping prose. Remaining: extend `classify_provider_exception` wiring to the other six provider adapters (anthropic, gemini, grok, azure, azure-foundry, and the deep-research paths) so classification is populated on every provider path, not just OpenAI. Pairs with the Phase 5 handoff-schema work.
