...
1. **Graph commit envelope for compiled expert memory** - finish the write boundary after the shipped compiler envelopes. Already done: `deepr-source-pack-manifest-v1`, `deepr-source-note-v1`, explicit `deepr expert sync --compile-claims`, `deepr-semantic-claim-extraction-v1`, `deepr-claim-verification-v1`, local `candidate_only` recall, and generated `deepr-expert-memory-card-v1` / `EXPERT.md` views. Next: add the one commit envelope that atomically writes accepted verifier decisions into beliefs, typed temporal edges, contradiction records, gap updates, and exploration agendas. It must preserve the agentic boundary: deterministic code owns schema, spend, provenance, idempotency, locks, and writes; calibrated model judgment owns support, contradiction, deduplication, temporal scope, edge type, hypothesis quality, and stance quality. Why: graph mutation is the point where bad meaning becomes durable, so it needs one narrow, replayable, auditable commit point.
2. **Richer concept, hypothesis, stance, and original-idea compilation** - expand the compiler prompts and envelopes beyond factual claim candidates. Ideas that are not online yet must still be representable when they have origin, rationale, uncertainty, expected observations, and disconfirming signals. They must not be forced through a false "web support or reject" rule, and they must not be promoted as verified external facts. Why: an expert is a developing perspective, not a fact book; Deepr needs a clean state model for concepts, theories, taste, tradeoffs, and exploration.
3. **Temporal knowledge graph completion and memory quality** - make temporal graph writes first-class in the compiler, wire recall into the compiler and absorb verifier bands, and add calibrated support, contradiction, dedup, temporal-scope, and edge-type decisions before any commit envelope writes the graph. Keep recall subordinate to the graph: recall finds candidates; the belief graph and verifier decide. Why: the expert should answer "what changed," "why do you believe it," "what is your current take," "what is contested," "what are you watching," and "what would change your mind," which no chunk store can answer.
4. **Protocol-native expert collaboration over MCP and A2A** - MCP already exposes expert reads, consults, loop status, handoff, belief explanation, scoped keys, budgets, rate limits, audit logs, and the `deepr-expert-collaboration-v1` council contract. A consult artifact now includes roster selection, per-expert role, shared consult trace id, budget/capacity contract, evidence packet, dissent handling, and result artifact refs for host agents. The current operating model is one or many experts, one bounded artifact: single-expert advice and expert-council guidance use the same consult contract, with preserved dissent and visible capacity posture. A2A has an authenticated task envelope and Agent Card baseline. Next map the collaboration contract onto A2A task artifacts, add hosted operational validation for external orchestrators, and keep LAN-agent smoke tests copyable through `docs/MCP_AGENT_TEST_GUIDE.md`. Deepr remains a role on the team, not the global orchestrator; host agents decide and enact. Why: external agents should be able to ask a durable expert council to collaborate, inspect agreement and dissent, then continue with structured state instead of opaque prose.
