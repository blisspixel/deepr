...
Current baseline: `deepr capacity` visibility, `deepr capacity next`, the local Ollama execution path,
...
3. [~] `local-ollama` + `--local` wiring are in place; local sync now supports `--fresh-context` with free-only retrieval, saved local eval artifacts now feed admission, and automatic local routing requires a measured admitted score; plan-quota CLI adapter work remains
