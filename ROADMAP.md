...
5. [~] Capacity QOL completion: `deepr capacity next` ranked actions, latest-artifact hints, and first concrete job previews are in place (`--expert`, `--report-id`, `--context-mode`, `--scheduled`), including wait guidance for fresh/deep scheduled sync jobs that should not fall through to metered API. `deepr expert sync --scheduled` now consumes that guidance for due subscription syncs and returns a wait payload instead of spending when owned/prepaid capacity is blocked. `deepr expert route-gaps --execute --scheduled` now returns pending routes and waits instead of starting metered gap-fill research. `deepr expert reflect --scheduled` now waits before the reflection evaluator or follow-up research can run from recurring jobs. Remaining work is reusing the same scheduler contract for health-check actioning.
