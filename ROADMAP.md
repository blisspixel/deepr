...
- [~] Belief lifecycle and salience (design: [docs/design/belief-lifecycle.md](docs/design/belief-lifecycle.md)) - memory governance, grounded in the 2026-06 memory-systems corpus review (monotonic accumulation is the literature's documented root failure mode; outcome-driven forgetting converges to true usefulness; contested beliefs are irreducible signal):
  ...
  - [ ] **Source-independence check before the trust-floor bump** (dogfood-sourced, 2026-06-21): the tertiary ceiling rises 0.60 -> 0.80 on `len(set(evidence_refs)) >= 2`, but does not verify the two sources are *independent* - syndicated copies of one origin can inflate confidence. Add an independence signal (distinct registrable domain / canonical-URL / near-duplicate-content screen, routed to a model verdict per AGENTIC_BALANCE, never a lexical conclusion) so corroboration must be genuine before the floor lifts. Every dogfood expert independently flagged this class as Deepr's #1 risk.
