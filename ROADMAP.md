...
- [~] **Agentic red-team suite.** First slice shipped: `src/deepr/security/red_team.py` plus `deepr eval red-team` run a local `$0` attack-success-rate verifier across prompt-injection, system-prompt extraction, jailbreak, data-exfiltration, structured tool-call/tool-result spoofing, and memory trust-floor bypass probes. The default suite currently reports 11/11 blocked cases and fails the command if any built-in attack succeeds. This remains a workflow verifier over prompt boundaries and confidence ceilings, not a semantic safety verdict. Remaining: automated expert-chat and ingestion-path corpora, ADAM-style adaptive extraction probing through MCP read tools, and richer release-to-release trend artifacts.
