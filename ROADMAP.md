...
- **Types**: mypy is a blocking `--strict` gate; target is 100% of `deepr/` strict-clean. Wired non-blocking first to record the baseline, then strict-blocking on `core/` + `providers/` + `mcp/` + `security/` + `queue/` + `storage/` + `tools/` + `routing/` + `worker/` and every new module, ratcheting package-by-package until the whole tree is clean. (Astral's `ty` is a candidate to replace mypy once it stabilizes.)
...
- [ ] Extend the strict-blocking gate to the rest of the tree, package-by-package (whole-tree `mypy` stays a non-blocking baseline meanwhile)
  ...
  - 2026-07-08: `worker/` is now the ninth blocking strict island after annotation-only cleanup in the local job poller. The local no-incremental strict probe passed for `src/deepr/worker`; `webhooks/` remains a small follow-up strict island.
