...
- [ ] **P1: migrate the remaining legacy direct metered interfaces.** `deepr
  ...
  they can issue multiple calls. They fail before provider construction.
...
partial saves and restarts, including automatic migration of legacy databases.
Job phases and costs are monotonic at terminal boundaries, stale provider polls
cannot resurrect finished work, and non-finite numeric state is refused before
mutation. Async task batches reject duplicate identities, malformed dependency
graphs, missing coroutines, and invalid concurrency limits before execution;
cancellation now stops every tracked coroutine even across concurrent batches.
Wildcard subscriptions accept only canonical campaign and expert base URIs.
...
event, model-proposed search plans are type-checked, deduplicated, and bounded
before execution, empty topics make no completion call, provider exception
details stay out of fallback output, and distribution checks refuse generated
frontend dependency files in both wheels and source archives.

**Shipped in v2.50.4 (see docs/CHANGELOG.md):** expert-scoped MCP keys deny
generic research and global skill discovery, scoped key-store mutations lock
the complete cross-process transaction, legacy hosted API authentication
fails closed when its secret is unavailable, dashboard expert creation rejects
Windows device names, and release-facing docs and plugin versions are current.

**Shipped in v2.50.3 (see docs/CHANGELOG.md):** empty queue-sync completions
fail closed, CLI and queue sync close incomplete provider snapshots, the
dashboard poller pages every processing job, terminal queue rows stay
immutable, REST docs require a token when one is configured, MCP expert
names reject Windows device names, and HTTPS dashboard cookies set Secure.
...
4. **Protocol-native expert collaboration over MCP and A2A (target v2.55; after v2.51-v2.54)** - design:
   ...
   **Protocol migration landed (v2.41.0, 2026-07-31):** the dual-era `2026-07-28` core shipped - per-request `_meta` version and capability negotiation, `server/discover`, modern result envelopes, `subscriptions/listen` on both transports, Streamable HTTP metadata-header and Origin validation, modern `-32602` resource errors, and honest capability advertisement (`logging` over-claim removed, elicitation self-report corrected). Legacy handshake clients are unaffected. Tasks remain unclaimed. Remaining hosted-claims work below still applies before widening remote serving.
   ...
   cost estimation. Live metered chat is hard-gated with no runtime or
...
- **Bounded research preview**: `research --preview` computes one finite provider request envelope without provider work. Production metered research, legacy `check`, and metered `learn` entry points are blocked pending authenticated provider account authority and current credential identity.
...
- **Cost controls**: Cumulative wallet credits, independent job and calendar limits, durable reservations, canonical cost ledger, cost tracking, `budget status`, and `costs show/timeline/breakdown/doctor`. Local wallet funding is not provider credit or provider hard-stop proof.
- **Expert state and local creation**: `expert make --local`, structured expert reads, flagship roster management, and derived export/import views. Standalone metered creation and chat are gated; explicit local and safety-eligible non-metered plan expert workflows remain available where documented.
...
- **Provider registry and bounded previews**: model metadata spans OpenAI, Gemini, xAI, Anthropic, Azure OpenAI, and Azure AI Foundry. Registry presence is not an execution claim. Finite request envelopes support write-free preview. No production provider account-control verifier ships, so all provider dispatch remains blocked, including managed Gemini Deep Research, xAI multi-agent research, and Azure Foundry Agent/Thread/Run execution.
...
- **Expert skills**: Inventory, metadata, installation, and scaffolding for 7 built-in skills, including native Recon, Distillr, and Primr. Python and MCP skill execution remains quarantined until runtime network and credential confinement can be proven.
...
- **Provider failure handling**: durable reservation and settlement substrate is
  ...
  and automatic cross-provider metered fallback are blocked.
- **Cloud deployment templates**: AWS/Azure/GCP templates provided but not battle-tested at scale
- **Grok provider**: single bounded responses are represented in the registry where pricing is complete; xAI multi-agent deep research is gated.
...
- **Azure AI Foundry provider**: model metadata remains visible, but Agent/Thread/Run execution with Bing grounding is gated until its multi-call and tool costs share one durable envelope.
...
- Durable accounting substrate for a future bounded provider attempt, with
  ...
  automatic metered fallback are blocked.
...
- Multi-layer wallet, job, and calendar inspection with fail-closed recovery; positive limits do not authorize production metered dispatch or prove provider controls
...
reference-only, and unsupported. See [deploy/README.md](deploy/README.md).
...
- [ ] Extend the strict-blocking gate to the rest of the tree, package-by-package (whole-tree `mypy` stays a non-blocking baseline meanwhile)
  - 2026-09-16: `backends/`, `agents/`, `evals/`, and `observability/` joined the blocking strict island. `services/` (52 remaining `--strict` errors) stays baseline-only.
...
- [x] Live-validation finding (2026-08-13, README dashboard capture): a temporary process-only `DEEPR_MAX_COST_PER_DAY` or `DEEPR_MAX_COST_PER_MONTH` override was folded into the immutable checkout `.env` provenance record. Returning to the unchanged file values then looked like a forbidden widening and blocked configuration loading. Fixed 2026-08-13 by registering only values parsed from the validated checkout file while applying process limits independently at runtime, with a regression that clears the temporary caps and proves the unchanged checkout remains valid.
