...
1. **Graph commit apply path for compiled expert memory** - finish the write boundary after the shipped compiler envelopes. Already done: `deepr-source-pack-manifest-v1`, `deepr-source-note-v1`, explicit `deepr expert sync --compile-claims`, `deepr-semantic-claim-extraction-v1`, `deepr-claim-verification-v1`, `deepr-graph-commit-envelope-v1` as an apply-gated no-write envelope for verified factual decisions, explicit `deepr expert apply-graph-commit` for idempotent factual belief and typed-edge writes, `deepr-graph-commit-apply-v1` apply results, local `candidate_only` recall, and generated `deepr-expert-memory-card-v1` / `EXPERT.md` views. Next: expand the commit envelope and apply command from factual add-belief operations into typed temporal edges from verifier output, gap updates, and exploration agendas. It must preserve the agentic boundary: deterministic code owns schema, spend, provenance, idempotency, locks, and writes; calibrated model judgment owns support, contradiction, deduplication, temporal scope, edge type, hypothesis quality, and stance quality. Why: graph mutation is the point where bad meaning becomes durable, so it needs one narrow, replayable, auditable commit point.
