...
1. **Corpus-to-expert compiler and generated wiki memory** - turn expert refresh from "files were added" into a bounded learning transaction: source pack -> source-note cards -> concept map -> claim, hypothesis, stance, and original-idea candidates -> type-specific verifier decisions -> typed temporal graph writes -> contradiction, gap, and exploration agenda -> regenerated wiki/digest view. `deepr-source-pack-manifest-v1` and `deepr-source-note-v1` are the deterministic `$0` no-model provenance stages: source windows, stable note IDs, hash readiness, and sync loop-run artifact refs. `deepr-semantic-claim-extraction-v1` is now the first model-judgment envelope: prompt/schema version capture, provider/model/capacity refs, raw response hashes, source-note/window refs, verifier-pending candidate claims, and graph writes explicitly disabled. Live invocation is wired as an explicit `deepr expert sync --compile-claims` sidecar for local, plan, and metered OpenAI-shaped clients behind budget and cost-ledger gates; it writes claim-extraction artifacts only. Next add support verification for externally factual claims, concept/hypothesis/stance/original-idea envelopes with gates appropriate to each state type, and one commit envelope that writes the expert graph only after the right checks pass. Absence from the live web is not a veto on a useful idea; it means the idea must be labeled as hypothesis, stance, proposal, or original synthesis with origin, rationale, assumptions, predicted observations, and disconfirming signals instead of being promoted as a verified external fact. The wiki remains a derived view over canonical state. Why: a corpus is what was read; an expert is the compiled, calibrated perspective that survives context resets.
