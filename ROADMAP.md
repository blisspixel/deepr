...
| **v2.51** (in progress) | Versioned host, capability, control, event, and artifact contracts plus read-only MCP investigation projection | Hosts need bounded, replayable, owner-scoped observation before they receive control authority | The reference-only `deepr-mcp-host-profile-v1` contract and OpenClaw stable reference are implemented; completion still requires stateless `run_id` and cursor reads that reproduce the canonical lifecycle, remain content-free, and deny cross-run access |
