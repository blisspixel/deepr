...
| **v2.50** (shipped, 2.50.4 patch) | Standards foundation plus fail-closed spend, storage, authentication, scoped remote access, and terminal-state handling | Format truth must precede distribution; spend, identity, and durable state must fail closed after the first release | Offline pinned fixtures pass; OKF remains derived and verification-gated; completion receipts cannot leave a hold; scoped key mutations preserve concurrent updates |
