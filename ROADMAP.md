...
1. **Graph commit apply path for compiled expert memory** - finish the write boundary after the shipped compiler envelopes. Already done: `deepr-source-pack-manifest-v1`, `deepr-source-note-v1`, explicit `deepr expert sync --compile-claims`, `deepr-semantic-claim-extraction-v1`, `deepr-claim-verification-v1`, verifier-supplied typed candidate edges in claim verification, `deepr-graph-commit-envelope-v1` as an apply-gated no-write envelope for verified factual decisions and candidate-to-candidate typed edges, `deepr-graph-commit-envelope-v2` with verifier-gated `promote_gap` operations, `deepr-graph-commit-envelope-v3` with verifier-gated `promote_exploration_agenda` operations, `deepr-graph-commit-envelope-v4` with verifier-gated `promote_hypothesis` operations, `deepr-graph-commit-envelope-v5` with verifier-gated `promote_concept` operations, explicit `deepr expert apply-graph-commit` for idempotent factual belief, typed-edge, gap-promotion, exploration-agenda, hypothesis, and concept writes, `deepr-graph-commit-apply-v1` apply results, local `candidate_only` recall, and generated `deepr-expert-memory-card-v1` / `EXPERT.md` views. Next: expand the compiler beyond factual claims, typed edges, gaps, agendas, hypotheses, and first concept writes into richer stance and original-idea state. It must preserve the agentic boundary: deterministic code owns schema, spend, provenance, idempotency, locks, and writes; calibrated model judgment owns support, contradiction, deduplication, temporal scope, edge type, gap quality, agenda quality, hypothesis quality, concept quality, and stance quality. Why: graph mutation is the point where bad meaning becomes durable, so it needs one narrow, replayable, auditable commit point.
2. **Richer stance and original-idea compilation** - expand the compiler prompts and envelopes beyond factual claim, hypothesis, and first concept candidates. Ideas that are not online yet must still be representable when they have origin, rationale, uncertainty, expected observations, and disconfirming signals. They must not be forced through a false "web support or reject" rule, and they must not be promoted as verified external facts. Why: an expert is a developing perspective, not a fact book; Deepr needs clean state for theories, taste, tradeoffs, stance, and exploration.
