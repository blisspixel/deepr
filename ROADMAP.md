...
1. **Maker-checker completion** - caller wiring (build the real cross-vendor checker through the provider adapters that translate model names, budget-gate it via the spend policy), bounded escalation (a 2nd different-vendor checker on a refutation/high-stakes claim before holding), and "verified by N cross-vendor checkers" in the handoff contract. Core + absorb seam shipped; design: [multi-backend-patterns.md](docs/design/multi-backend-patterns.md).
...
3. **Last Phase 4d sub-slices** - the change-detection **conditional-GET** slice (ETag/304 to skip *before* retrieval cost) and the overlap-guard **verb wiring** (hold the per-(expert, verb) lock across each scheduled verb body, best with the `sync_cmd` body extraction).
...
5. **Semantic belief recall** - the one genuine recall gap: an embedding index over belief claims (local-first, cost-gated) so paraphrased/cross-domain beliefs surface. See the Phase 4 graph-memory section.

After that, the larger tracks resume: Phase 5 (security/ops hardening, team features, hosted-MCP completion), Phase 4c (expert crews), Phase 4b (autonomous campaigns).
